...
static AFX_EXTENSION_MODULE ZControlsDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialize ZControls.DLL.\n");


		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(ZControlsDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ZControlsDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalize ZControls.DLL.\n");
	
		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(ZControlsDLL);
	}
