...
KShortestPath::KShortestPath(void)
{
}
...
void KShortestPath::ExtractNodeDepartureLink(KLinkCollection& a_colLink, KIDKeyIntegerValueList& a_colNodeDepartureLink)
{    
...
bool KShortestPath::IsDestination( std::set<Integer>& a_setDestination, Integer a_nxNodeID )
{
...
void KShortestPath::MuLinkBaseExecute(  Integer              a_nxOiginNodeID, 
                                        std::set<Integer>&   a_setDestination, 
                                        KLinkCollection&     a_colLink, 
                                        KCostLinkCollection& a_oMinCostLinkCollection, 
                                        KNodeArrivedMinLink& a_oDestinationNodeArrivedMinLink)
{
    ...
    // 시작점 추가    
    PushHeapFromNode2Links ( oOriginHeapNode, a_colLink, colNodeDepartureLink, oMinHeap, 0.0 );           
    ...
    while ( ! oMinHeap.empty() /*|| (nxDestinationCount == a_oDestinationNodeArrivedMinLink.GetCount())*/ )
    {
        ...
        if ( NULL == pMinCostLink) { // 최초 방문
            a_oMinCostLinkCollection.InsertOrReplaceLink(nxTopPreLinkID, nxTopPPreLinkID, dTopNodeCost);

            PushHeapFromNode2Links (topHeapNode, a_colLink, colNodeDepartureLink, oMinHeap, dTopNodeCost);
            if ( isDestination ) {
...
void KShortestPath::ExtractPath( Integer a_nxDestinationNodeID, KCostLinkCollection& a_oMinCostLinkCollection, KNodeArrivedMinLink& a_oDestinationNodeArrivedMinLink, std::vector<Integer>& ar_vecPathLink )
{	
	ar_vecPathLink.clear();

	KMinLinkInfo* pMinLinkInfo = a_oDestinationNodeArrivedMinLink.GetMinLink(a_nxDestinationNodeID);
...
Integer KShortestPath::MuLinkBaseExecuteClosedNode( Integer a_nxOiginNodeID, std::set<Integer>& a_setDestination, KLinkCollection& a_colLink,
