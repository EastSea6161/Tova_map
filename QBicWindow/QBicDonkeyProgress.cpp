#include "stdafx.h"
#include "QBicDonkeyProgress.h"

#define IDC_DCANCEL  10
#define IDC_TEXT     11
#define IDC_PROGRESS 12
#define IDC_DEVIDER  13
...
const CString QBicDonkeyProgress::DEFAULT_TITLE = _T("Progress");

/////////////////////////////////////////////////////////////////////////////
// KDonkeyProgress
QBicDonkeyProgress::QBicDonkeyProgress(int nLang /*=1*/)
{
	m_nLang = nLang;
    CommonConstruct();    
}

QBicDonkeyProgress::QBicDonkeyProgress(CWnd* pParent, LPCTSTR pszTitle, BOOL bModal, BOOL bProgress, BOOL bSmooth /* = FALSE */, 
	int nLang /*=1*/)
{
	m_nLang = nLang;
    ...
    Create(pParent, pszTitle, bModal, bProgress, bSmooth);

	if (0 == m_nLang)
	{
		SetText(_T("데이터 처리 중입니다...\n잠시만 기다려 주세요..."));
	}
	else if(1 == m_nLang)
	{
		SetText(_T("Please wait... processing data..."));
	}
}

void QBicDonkeyProgress::CommonConstruct()
{
    m_nSlowPeekRequestNo = 0;
    ...
    m_bPersistantPosition = FALSE;   // saves and restores position automatically
}

QBicDonkeyProgress::~QBicDonkeyProgress()
{
    DestroyWindow();
}

BOOL QBicDonkeyProgress::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bModal, BOOL bProgress, BOOL bSmooth /* = FALSE */)
{
    BOOL bSuccess;
	...
	#if (WINVER >= 0x0600)
		ncm.cbSize -= 4; //<== ADD HERE!! : Windows XP에서 문제 발생하여 추가(2012.03.06, nombara)
	#endif

	BOOL f = SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &ncm, 0);
    // Original : VERIFY(SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &ncm, 0));
	VERIFY(f);
    ...
    DWORD dwProgressStyle ;
    if ( bProgress )
    {
        dwProgressStyle = WS_CHILD|WS_VISIBLE;
    }
    else
    {
        dwProgressStyle = WS_CHILD;
    }
#ifdef PBS_SMOOTH    
    if (bSmooth)
    ...
    bSuccess = m_CancelButton.Create(m_strCancelLabel, WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_PUSHBUTTON, TempRect, this, IDC_DCANCEL);
    if (!bSuccess)
        return FALSE;
    ...
    SetWindowSize(m_nNumTextLines, 350, bProgress);
...
BOOL QBicDonkeyProgress::GoModal(LPCTSTR pszTitle /*=_T("Progress")"*/, BOOL bSmooth /*=FALSE*/)
{
...
void QBicDonkeyProgress::SetWindowSize(int nNumTextLines, int nWindowWidth /*=390*/, BOOL bProgress)
{
    int nMargin = 10;
    ...
    CRect deviderRect;
    deviderRect.SetRect(TextRect.left-EdgeSize.cx, TextRect.bottom + EdgeSize.cy*3, TextRect.right, TextRect.bottom + EdgeSize.cy*3+1/*CancelRect.bottom - EdgeSize.cy * 4*/);
    ...
    //m_Devider : 디바이더는 생성시 해야 이쁘게 표현 됨...
    m_Devider.Create(_T(""), WS_CHILD|WS_VISIBLE|SS_ETCHEDHORZ, deviderRect, this, IDC_DEVIDER);       

    if ( ! bProgress )
    {
        CRect rect(deviderRect.left, deviderRect.top - 20, deviderRect.left, deviderRect.top + 20);
        m_Picture.Create(_T(""), WS_VISIBLE | WS_CHILD, rect, this);

        m_Picture.Load(_T(".\\images\\Ix01\\donkey.gif"));
        m_Picture.Draw();
    }    
}

void QBicDonkeyProgress::Clear() 
{ 
    SetText(_T(""));
    SetPos(0);
    m_bCancelled = FALSE; 
...
void QBicDonkeyProgress::SetRange(int nLower, int nUpper, int nStep /* = 1 */)    
{
    if (!::IsWindow(GetSafeHwnd())) 
        return;
...
int QBicDonkeyProgress::OffsetPos(int nPos)
{ 
    if (!::IsWindow(GetSafeHwnd())) 
        return m_nPrevPos;
...
int QBicDonkeyProgress::SetStep(int nStep)
{
...
int QBicDonkeyProgress::SetPos(int nPos)                    
{
#ifdef PBM_SETRANGE32
...
//void KDonkeyProgress::SetText(LPCTSTR fmt, ...)
//{
//    if (!::IsWindow(GetSafeHwnd())) 
//        return;
//
...
void QBicDonkeyProgress::SetText(const CString& strMsg)
{
    m_Text.SetWindowText(strMsg);
}

void QBicDonkeyProgress::SendTextEvent(const CString& strMsg)
{
    m_Text.SetWindowText(strMsg);
    PeekAndPump();
}

BEGIN_MESSAGE_MAP(QBicDonkeyProgress, CWnd)
    //{{AFX_MSG_MAP(KDonkeyProgress)
    ON_WM_ERASEBKGND()
    //}}AFX_MSG_MAP
    ON_BN_CLICKED(IDC_DCANCEL, OnCancel)
END_MESSAGE_MAP()


/////////////////////////////////////////////////////////////////////////////
// KDonkeyProgress message handlers

BOOL QBicDonkeyProgress::OnEraseBkgnd(CDC* pDC) 
{
...
void QBicDonkeyProgress::OnCancel() 
{
    Close();
}

void QBicDonkeyProgress::Close()
{
    if (m_bPersistantPosition)
        SaveCurrentSettings();
...
BOOL QBicDonkeyProgress::DestroyWindow() 
{
    if (m_bPersistantPosition)
        SaveCurrentSettings();
...
void QBicDonkeyProgress::PeekAndPump(BOOL bCancelOnESCkey /*= TRUE*/)
{
...
void QBicDonkeyProgress::SlowPeekAndPump(const CString& strMsg, BOOL bCancelOnESCkey /* = TRUE */)
{
    m_nSlowPeekRequestNo ++;

    if ( m_nSlowPeekRequestNo >= 50 )
    {
        SetText(strMsg);
        PeekAndPump(bCancelOnESCkey);
        m_nSlowPeekRequestNo = 0;
    }
}

// Retores the previous window size from the registry
void QBicDonkeyProgress::GetPreviousSettings()
{
...
void QBicDonkeyProgress::SaveCurrentSettings()
{   
    if (!IsWindow(m_hWnd))
        return;
