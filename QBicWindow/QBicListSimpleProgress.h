...
class QBicListSimpleProgressParameter;
typedef unsigned (__stdcall *ThreadCallBackFunction)(void*);
/**
*@brief 리스트 진행
*@details 리스트 진행 다이얼로그
*@author 큐빅웨어
*@date 2016/05/17
*@version 1.0.1
*@remark -2019.06.11 : v1.0.1 언어설정 기능 추가 
*/
class AFX_EXT_CLASS QBicListSimpleProgress : public CXTResizeDialog
{
	DECLARE_DYNAMIC(QBicListSimpleProgress)

private:
	QBicListSimpleProgress(ThreadCallBackFunction a_fnPtr, void* pParameter, CWnd* pParent = NULL, int nLang = 1);   
public:
	virtual ~QBicListSimpleProgress();

	// 대화 상자 데이터입니다.
	enum { IDD = IDD_QW_0510_ProgressListDlg };
...
protected:
	...
	void* m_pParam;
	QBicListSimpleProgressParameter* m_pSimpleProgressParameter;
...
protected:
	...
	friend class QBicListSimpleProgressThread;
public:
...
typedef std::shared_ptr<QBicListSimpleProgress> QBicListSimpleProgressPtr;
/**
*@brief 심플 프로그레스 스레드 리스트
*@details 
*@author 큐빅웨어
*@date 2016/05/17
*@version 1.0.1
*@remark -2019.06.11 : v1.0.1 언어설정 기능 추가 
*/
class AFX_EXT_CLASS QBicListSimpleProgressThread
{
private:
	QBicListSimpleProgressThread()
	{
	}
...
private:
	static QBicListSimpleProgressPtr m_spProgress;
public:
	/**
	*@brief 스레드 제거
	...
	*@param nLang Progress 다이얼로그의 언어설정, 0=한국어, 1=영어
	*@return 
	*/
	...
	*@return 프로그레스
	*/
	static QBicListSimpleProgressPtr SimpleProgress();    
	/**
	*@brief 유저 중지 상태 확인
	*@return 값 반환
	*/
