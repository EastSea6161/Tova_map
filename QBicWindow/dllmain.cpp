...
static AFX_EXTENSION_MODULE QBicWindowDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialize QBicWindow.DLL.\n");


		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(QBicWindowDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(QBicWindowDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalize QBicWindow.DLL.\n");
	
		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(QBicWindowDLL);
	}
