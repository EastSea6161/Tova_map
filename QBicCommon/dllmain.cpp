...
static AFX_EXTENSION_MODULE QBicCommonDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialize QBicCommon.DLL.\n");


		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(QBicCommonDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(QBicCommonDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalize QBicCommon.DLL.\n");
	
		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(QBicCommonDLL);
	}
