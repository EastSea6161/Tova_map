// ShortestPathNHighwayDlg.cpp : 구현 파일입니다.
//
...
#include "ChildFrm.h"
#include "MapView.h"
#include "ShortestPathNHighwayDlg.h"
...
IMPLEMENT_DYNAMIC(KShortestPathNHighwayDlg, KResizeDialogEx)

KShortestPathNHighwayDlg::KShortestPathNHighwayDlg(CWnd* pParent /*=NULL*/)
	: KResizeDialogEx(KShortestPathNHighwayDlg::IDD, pParent)
	, m_nResultCode(0)
	, m_bStartZone(FALSE)
	, m_bEndZone(FALSE)
{
	...
	m_nxStartNodeID = -1;
	m_nxEndNodeID   = -1;
	m_nxPrevStartNodeID = m_nxStartNodeID;
...
KShortestPathNHighwayDlg::~KShortestPathNHighwayDlg()
{
	CloseWindowProc();
...
void KShortestPathNHighwayDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KShortestPathNHighwayDlg, KResizeDialogEx)
	...
	ON_COMMAND(ID_SELECT_CANCEL, &KShortestPathNHighwayDlg::OnSelectPathCancel)
	ON_COMMAND(ID_SELECT_ALL,    &KShortestPathNHighwayDlg::OnSelectAllPath)
	ON_CBN_SELCHANGE(IDC_COMBO1,                 &KShortestPathNHighwayDlg::OnCbnSelchangeCriterion)
	ON_BN_CLICKED(IDC_BUTTON1, &KShortestPathNHighwayDlg::OnBnClickedBtnSearch)
	ON_BN_CLICKED(IDCANCEL,    &KShortestPathNHighwayDlg::OnBnClickedCancel)
	ON_CBN_SELCHANGE(IDC_CBO_COLORRAMP,			 &KShortestPathNHighwayDlg::OnCbnSelchangeCboColorramp)
	ON_BN_CLICKED(IDC_BUTTON4, &KShortestPathNHighwayDlg::OnBnClickedBtnDraw)
	ON_BN_CLICKED(IDC_BUTTON5, &KShortestPathNHighwayDlg::OnBnClickedBtnClear)
	ON_CBN_SETFOCUS(IDC_COMBO1, &KShortestPathNHighwayDlg::OnCbnSetfocusCriterion)
	ON_BN_SETFOCUS(IDC_CHECK_ZONESTART, &KShortestPathNHighwayDlg::OnBnSetfocusCheckZonestart)
	ON_BN_SETFOCUS(IDC_CHECK_ZONEEND, &KShortestPathNHighwayDlg::OnBnSetfocusCheckZoneend)
	ON_CBN_SETFOCUS(IDC_CBO_COLORRAMP, &KShortestPathNHighwayDlg::OnCbnSetfocusCboColorramp)
	ON_WM_COPYDATA()

	ON_BN_CLICKED(IDC_BUTTON6, &KShortestPathNHighwayDlg::OnBnClickedButtonStartNodeSearch)
	ON_BN_CLICKED(IDC_BUTTON7, &KShortestPathNHighwayDlg::OnBnClickedButtonEndNodeSearch)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_CHECK1, &KShortestPathNHighwayDlg::OnBnClickedCheckSelectAll)
	ON_BN_CLICKED(IDC_CHECK_ZONESTART, &KShortestPathNHighwayDlg::OnBnClickedCheckZonestart)
	ON_BN_CLICKED(IDC_CHECK_ZONEEND, &KShortestPathNHighwayDlg::OnBnClickedCheckZoneend)
END_MESSAGE_MAP()


KMapView* KShortestPathNHighwayDlg::GetMapView(KTarget* a_pTarget)
{
...
void KShortestPathNHighwayDlg::CloseWindowProc()
{
...
void KShortestPathNHighwayDlg::NotifyProcess(LPCTSTR a_strSubjectName, Integer a_nxObjectID)
{
    try
    {
...
void KShortestPathNHighwayDlg::ActivatedFrameWndNotifyProcess()
{
	if(nullptr != m_spImTasIDFind)
	{
...
void KShortestPathNHighwayDlg::DeActivatedFrameWndNotifyProcess()
{
	if(nullptr != m_spImTasIDFind)
	{
		if(!m_spImTasIDFind->IsUserClosed())
...
BOOL KShortestPathNHighwayDlg::OnInitDialog()
{
...
void KShortestPathNHighwayDlg::ResizeComponent()
{
	...
	SetResize(IDC_EDIT4,                  SZ_BOTTOM_LEFT,   SZ_BOTTOM_RIGHT);
	SetResize(IDC_BUTTON7,                SZ_BOTTOM_RIGHT,  SZ_BOTTOM_RIGHT);
...
void KShortestPathNHighwayDlg::InitReportHeader()
{
...
void KShortestPathNHighwayDlg::InitializeCriterionList()
{
...
void KShortestPathNHighwayDlg::InitialColorRampCtrl()
{
	CBitmap bmp;
...
void KShortestPathNHighwayDlg::SetColorRampInfo(int nSelectIndex)
{
	switch(nSelectIndex)
	{
	case 0:
...
void KShortestPathNHighwayDlg::InitializeNode()
{
	CWaitCursor wc;
...
bool KShortestPathNHighwayDlg::GetNodeInfo(void)
{
	TxLogDebugStartMsg();
...
bool KShortestPathNHighwayDlg::GetZoneNodeInfo(void)
{
	TxLogDebugStartMsg();
...
void KShortestPathNHighwayDlg::OnReportCheckItem(NMHDR* pNotifyStruct, LRESULT* pResult)
{
...
void KShortestPathNHighwayDlg::OnReportItemClick(NMHDR* pNotifyStruct, LRESULT* pResult)
{
    XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
...
void KShortestPathNHighwayDlg::ZoomToNodeFeature( Integer &a_nxNodeID )
{
	try
	{
...
void KShortestPathNHighwayDlg::OnReportColumnRButtonClick(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void KShortestPathNHighwayDlg::OnSelectPathCancel()
{
	PathCheckboxControl( FALSE );
}


void KShortestPathNHighwayDlg::OnSelectAllPath()
{
	PathCheckboxControl( TRUE );
}


void KShortestPathNHighwayDlg::PathCheckboxControl(BOOL a_bCheck)
{
	CXTPReportRecords*	  pRecords    = NULL;
	CXTPReportRecord*     pRecord     = NULL;
...
void KShortestPathNHighwayDlg::OnCbnSelchangeCriterion()
{
	UpdateData(TRUE);
...
void KShortestPathNHighwayDlg::UpdateEndNodeID( std::set<CString> &a_setSelectedID )
{
...
void KShortestPathNHighwayDlg::OnBnClickedBtnSearch()
{
	...
	if(m_nxPrevStartNodeID != m_nxStartNodeID)
	{
		m_nxPrevStartNodeID = m_nxStartNodeID;
		m_bResearch = true;
	}
...
CString KShortestPathNHighwayDlg::GetInitTotalID(KTDNodeDataMap &a_mapNodeData, std::set<CString> &a_setID)
{
	a_setID.clear();
...
unsigned __stdcall KShortestPathNHighwayDlg::ShortestPathThreadCaller(void* p)
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	QBicSimpleProgress* pProgressWindow = pParameter->GetProgressWindow();
	
	if (KmzSystem::GetLanguage() == KEMKorea) {
		pProgressWindow->SetStatus(_T("최단 경로 탐색(도로) 모듈 실행 중..."));
	}
	else {
		pProgressWindow->SetStatus(_T("Run Path Find(Highway) Module..."));
	}

	KShortestPathNHighwayDlg* pDlg = (KShortestPathNHighwayDlg*)pParameter->GetParameter();
	pDlg->GetSearchShortestPath();
	return 0;
}

void KShortestPathNHighwayDlg::GetSearchShortestPath()
{
	TxLogDebugStartMsg();
...
// 
// 	// load library
// 	m_hAssignment = ::LoadLibrary(_T("FAssignment.dll"));
// 	
// 	if( NULL == m_hAssignment )
// 	{
...
// 	}
// 
// 	AssignmentProcPtr = (FHIGHWAYSHORTESTPATH)(GetProcAddress(m_hAssignment, "FHIGHWAYSHORTESTPATH"));
// 
...
// 	}
// 
// 	AddStatusMessage(_T("> 경로 탐색 Start ..."));
// 
//     int nResult = 0;
//     
//     try
//     {
//        nResult = AssignmentProcPtr((void*)this, (void*)ReceiveStatusMessageFromDll);
//     }
//     catch (KExceptionPtr ex)
//     {
//         TxExceptionPrint(ex);
...
void KShortestPathNHighwayDlg::ReceiveStatusMessageFromDll(void* pParam1, int nStatus, char* strMsg)
{
	AddStatusMessage(CString(strMsg));
}


void KShortestPathNHighwayDlg::AddStatusMessage(CString a_strMsg)
{
...
bool KShortestPathNHighwayDlg::ReadOutPutFile()
{
	TxLogDebugStartMsg();
...
bool KShortestPathNHighwayDlg::ReadNodeFile()
{
	TxLogDebugStartMsg();
...
bool KShortestPathNHighwayDlg::ReadLinkFile()
{
	TxLogDebugStartMsg();
...
void KShortestPathNHighwayDlg::InsertLinkReportData(Integer a_nxStartNodeID, std::set<CString> &a_setSelectedID )
{
	//- 종점 정보 
...
bool KShortestPathNHighwayDlg::CreateArgument( KIOColumns &a_oUserColumnsLink)
{
	TxLogDebugStartMsg();
...
bool KShortestPathNHighwayDlg::ClearFiles()
{
	TxLogDebugStartMsg();
...
bool KShortestPathNHighwayDlg::WriteShortestPathArgument(CStdioFileEx& of)
{
	CString strOut = _T("");
...
int KShortestPathNHighwayDlg::GetSelCriterionIndex( KIOColumns &a_oUserColumnsLink )
{
	UpdateData(TRUE);
...
Integer KShortestPathNHighwayDlg::GetSelStartNodeId()
{
	Integer nxNodeId      = -1;
...
void KShortestPathNHighwayDlg::ModelessOKFeedback( UINT nDialogID)
{
	if (KImChampIDFind::IDD == nDialogID)
	{
		if (m_spImTasIDFind)
		{
			try
			{
				...
				if(m_nxPrevStartNodeID != nxOrgZoneID)
				{
					InitDestination();
				}
			}
...
void KShortestPathNHighwayDlg::SetNodeViewData(KTDNodeDataMap &a_mapNodeData, CString a_strZoneID, CString &a_strNodeViewData, KIDCaption &a_oIDCaption)
{
	try
	{
...
void KShortestPathNHighwayDlg::OnBnClickedCancel()
{
	CloseWindowProc();
...
void KShortestPathNHighwayDlg::OnBnClickedBtnClear()
{
	PathCheckboxControl( FALSE );
...
void KShortestPathNHighwayDlg::AllClearShortestPath()
{
	try
	{
		KMapView* pMapView = GetMapView(m_pTarget);
...
void KShortestPathNHighwayDlg::OnBnClickedBtnDraw()
{
	try
	{
		DrawShortestPath();
...
void KShortestPathNHighwayDlg::DrawShortestPath()
{
...
void KShortestPathNHighwayDlg::MakeLinkVector(std::vector<Integer/*LinkID*/> &a_vecLink, Integer a_nxNodeID)
{
...
void KShortestPathNHighwayDlg::OnCbnSelchangeCboColorramp()
{
	this->SetFocus();
...
void KShortestPathNHighwayDlg::ConvertEmptyNodeName(CString &a_strNodeName)
{
	a_strNodeName = a_strNodeName.IsEmpty() == TRUE ? _T(" - ") : a_strNodeName;
}


void KShortestPathNHighwayDlg::SetNodeViewData(const Integer &a_nxNodeID, CString &a_strNodeName, CString &a_strNodeViewData)
{
...
void KShortestPathNHighwayDlg::OnCbnSetfocusCriterion()
{
	//SendMessage(DM_SETDEFID, IDC_BUTTON1, 0);
}


void KShortestPathNHighwayDlg::OnBnSetfocusCheckZonestart()
{
	//SendMessage(DM_SETDEFID, IDC_BUTTON1, 0);
}


void KShortestPathNHighwayDlg::OnBnSetfocusCheckZoneend()
{
	//SendMessage(DM_SETDEFID, IDC_BUTTON4, 0);
}


void KShortestPathNHighwayDlg::OnCbnSetfocusCboColorramp()
{
	//SendMessage(DM_SETDEFID, IDC_BUTTON4, 0);
}


BOOL KShortestPathNHighwayDlg::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
    try
    {
...
BOOL KShortestPathNHighwayDlg::PreTranslateMessage(MSG* pMsg)
{
...
void KShortestPathNHighwayDlg::ClickedEditStartNode()
{
	SelectStartNodeList();
}


void KShortestPathNHighwayDlg::ClickedEditEndNode()
{
	SelectEndNodeList();
}


void KShortestPathNHighwayDlg::OnBnClickedButtonStartNodeSearch()
{
	SelectStartNodeList();
}


void KShortestPathNHighwayDlg::OnBnClickedButtonEndNodeSearch()
{
	SelectEndNodeList();
}


void KShortestPathNHighwayDlg::SelectStartNodeList()
{
	try
	{
...
void KShortestPathNHighwayDlg::SelectEndNodeList()
{
	try
	{
		...
		GetDlgItemText(IDC_EDIT4, strSelect);
...
HBRUSH KShortestPathNHighwayDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void KShortestPathNHighwayDlg::OnBnClickedCheckSelectAll()
{
	try
	{
		BOOL bSelectAll(FALSE);
...
void KShortestPathNHighwayDlg::OnBnClickedCheckZonestart()
{
	try
	{
...
void KShortestPathNHighwayDlg::InitDestination()
{
	try
	{
		m_bResearch = true;
...
void KShortestPathNHighwayDlg::OnBnClickedCheckZoneend()
{
	try
	{
		CWaitCursor cw;
		...
		if (IsDlgButtonChecked(IDC_CHECK_ZONEEND) == BST_CHECKED)
		{
			strIDs = ControlSearchedID(true,  m_setSearchedID,  setPart); // 존 추출
		}
		else
		{
			strIDs = ControlSearchedID(false, m_setSearchedID, setPart);
		}
...
CString KShortestPathNHighwayDlg::ControlSearchedID(bool a_bZone, std::set<CString> &a_setSourceID, std::set<CString> &a_setPartID)
{
	CString strReturn(_T(""));
...
void KShortestPathNHighwayDlg::ModelessCancleFeedback(UINT nDialogID)
{
