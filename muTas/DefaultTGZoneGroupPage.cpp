// DefaultTGZoneGroupPage.cpp : 구현 파일입니다.
//
...
IMPLEMENT_DYNAMIC(KDefaultTGZoneGroupPage, CXTPPropertyPage)

KDefaultTGZoneGroupPage::KDefaultTGZoneGroupPage(KTarget* a_pTarget, KGenerationInfo* a_pGenerationInfo)
	: CXTPPropertyPage(KDefaultTGZoneGroupPage::IDD)
	, m_pTarget(a_pTarget)
...
{
	m_pKExpressionRoot = nullptr;
}

KDefaultTGZoneGroupPage::~KDefaultTGZoneGroupPage()
{
	if(nullptr != m_pKExpressionRoot)
	{
...
void KDefaultTGZoneGroupPage::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDefaultTGZoneGroupPage, CXTPPropertyPage)
	ON_BN_CLICKED(IDC_ADD, &KDefaultTGZoneGroupPage::OnBnClickedAdd)
	ON_BN_CLICKED(IDC_DELETE, &KDefaultTGZoneGroupPage::OnBnClickedDelete)
	...
	ON_NOTIFY(NM_RCLICK, IDC_REPORT, &KDefaultTGZoneGroupPage::OnReportItemRClick)
	ON_WM_PAINT()
	ON_WM_SIZE()
END_MESSAGE_MAP()


// KDefaultTGZoneGroupPage 메시지 처리기입니다.


BOOL KDefaultTGZoneGroupPage::OnInitDialog()
{
	...
	SetIcon( hIcon, FALSE );
	TxLogDebugStartMsg();
	m_pKCodeManager = m_pTarget->CodeManager();
	m_pKZone = m_pTarget->Tables()->FindTable(TABLE_ZONE);
	InitZoneGroup();
...
void KDefaultTGZoneGroupPage::InitDefaultData()
{
	...
	{
		m_pGenerationInfo->DeleteExpression();
		return;
	}
...
void KDefaultTGZoneGroupPage::InitZoneGroup( void )
{
	try
	{
		if(NULL == m_pKZone)
...
void KDefaultTGZoneGroupPage::InitReportControl( void )
{
	try
	{
		// Add Columns
...
void KDefaultTGZoneGroupPage::UpdateReportControl( BOOL a_bExpandAll /*= TRUE*/ )
{
	try
	{
		if((m_wndReport.GetSafeHwnd() == NULL) || (NULL == m_pGenerationInfo))
		{
			return;
		}
...
void KDefaultTGZoneGroupPage::UpdateReportColumns( std::list<TGroupColumnInfo>& a_lstRemainColumns )
{
	try
	{
...
BOOL KDefaultTGZoneGroupPage::CheckReportColumns( std::list<TGroupColumnInfo>& a_lstRemainColumns )
{
	try
	{
		std::list<TGroupColumnInfo>::iterator itColumns;
...
void KDefaultTGZoneGroupPage::UpdateButtonState( void )
{
	try
	{
		BOOL bAdd, bDelete;
...
void KDefaultTGZoneGroupPage::UpdateComboList( void )
{
	try
	{
...
void KDefaultTGZoneGroupPage::AddField( CXTPReportRecord* a_pParent )
{
	try
	{
		// combo box에서 값을 가져온다.
		...
		std::set<int> setCode;
		GetSubCode(pColumn->Name(), mapCodeGroup, setCode);
		...
		if (mapCodeGroup.size() < 1)
		{
			...
			for(itCode = codeList.begin(); itCode != itEnd; ++itCode)
			{
				...
				pSubValue->CreateExpressionObject(m_pGenerationInfo->GenerationModel());
			}
		}
		else
		{
...
bool KDefaultTGZoneGroupPage::GetParentsData( CXTPReportRecord* a_pParent, std::map<CString, int>& a_mapCode )
{
	try
	{
...
void KDefaultTGZoneGroupPage::GetSubCode( CString a_strColumnName, std::map<CString, int> a_mapPCodeInfo, std::set<int>& a_setCode )
{
	CString strSQL(_T(""));
	try
	{
		...
		while(iter != end)
		{
			if (nIndex == 0)
			{
				strWhere.Format(_T(" %s = '%d' "), iter->first, iter->second);
			}
			else
			{
		...
		strSQL.Format(_T("Select %s From %s Where %s "), a_strColumnName, TABLE_ZONE, strWhere);

		KDBaseConPtr spDBaseConnection = m_pKZone->GetDBaseConnection();
...
void KDefaultTGZoneGroupPage::OnBnClickedAdd()
{
	try
	{
...
void KDefaultTGZoneGroupPage::OnBnClickedDelete()
{
	try
	{
		CXTPReportRow* pRow = m_wndReport.GetFocusedRow();
...
void KDefaultTGZoneGroupPage::OnItemButtonClick( NMHDR * a_pNotifyStruct, LRESULT* a_pResult )
{
...
void KDefaultTGZoneGroupPage::OnReportSelChanged( NMHDR* a_pNMHDR, LRESULT* a_pResult )
{
	try
	{
		CXTPReportRow* pRow = m_wndReport.GetFocusedRow();
...
LRESULT KDefaultTGZoneGroupPage::OnExpressionEdit( WPARAM a_wParam, LPARAM a_lParam )
{
	try
	{
		...
		if(KEMGenerationUnit == m_pGenerationInfo->GenerationModel())
		{
		...
		else if(KEMGenerationRegression == m_pGenerationInfo->GenerationModel())
		{
		...
		else if(KEMGenerationCrossClassification == m_pGenerationInfo->GenerationModel())
		{
...
void KDefaultTGZoneGroupPage::OnPaint()
{
	CPaintDC dc(this);
}

BOOL KDefaultTGZoneGroupPage::OnApply()
{
	if(nullptr != m_pKExpressionRoot)
	{
		KGGroupExpressionContainer* pContainer = 
			(KGGroupExpressionContainer*)m_pGenerationInfo->ExpressionContainer();
		if(nullptr == pContainer)
		{
			pContainer = (KGGroupExpressionContainer*)(m_pGenerationInfo->CreateExpression());
		}
...
void KDefaultTGZoneGroupPage::OnSize(UINT nType, int cx, int cy)
{
...
void KDefaultTGZoneGroupPage::ResizeComponent()
{
	try
	{
...
void KDefaultTGZoneGroupPage::OnReportItemRClick( NMHDR * pNotifyStruct, LRESULT * result )
{
...
void KDefaultTGZoneGroupPage::ClearCopyData()
{
...
BOOL KDefaultTGZoneGroupPage::InsertCopyData()
{
	UpdateData(TRUE);
...
BOOL KDefaultTGZoneGroupPage::InsertCopySubData( std::list<TGroupColumnInfo>& a_lstRemainColumns )
{
	std::list<TGroupColumnInfo>::iterator itColumns;
	CXTPReportRecord* pColRecord = NULL;
	CXTPReportRecord* pValRecord = NULL;
