...
class KDlgImportSedPop : public KWhiteDlgEx
{
	DECLARE_DYNAMIC(KDlgImportSedPop)

public:
	KDlgImportSedPop(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~KDlgImportSedPop();

	// 대화 상자 데이터입니다.
	enum { IDD = IDD_201_01_13_IMPORT_TABLE_SEDPOP };
...
private:
	void         RegisterZoneRelationTable();
	int          IsZoneRelationTable(CString a_strTableName);
...
private:
	...
	bool         CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg);
	...
	bool         IsSedPopDBID(KODKey a_oODKey, Integer a_nxAge);
...
protected :
	...
	std::map<KODKey, std::set<Integer>> m_mapSedPopPK;
	std::set<CString>       m_setKeyFieldName;

	std::map<CString, int>              m_mapZoneRelationTable;// 테이블 네임, 테이블 내 Zone 관련된 컬럼갯수
