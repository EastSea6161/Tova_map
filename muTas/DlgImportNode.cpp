...
IMPLEMENT_DYNAMIC(KDlgImportNode, KWhiteDlgEx)

KDlgImportNode::KDlgImportNode(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportNode::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportNode::~KDlgImportNode()
{
}

void KDlgImportNode::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportNode, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportNode::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportNode::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportNode::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportNode::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportNode::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportNode::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportNode::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportNode::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportNode::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportNode::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportNode 메시지 처리기입니다.
void KDlgImportNode::ResizeComponent()
{
	try 
	{
...
void KDlgImportNode::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportNode::GetSelectedImportType()
{
...
BOOL KDlgImportNode::OnInitDialog()
{
...
void KDlgImportNode::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportNode::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportNode::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportNode::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportNode::InitComboSeparator()
{
	try
	{
...
void KDlgImportNode::InitComboEncoding()
{
	try
	{
...
void KDlgImportNode::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
// 		pColumn->SetAlignment( DT_CENTER );
		...
		//pImport->GetEditOptions()->m_bConstraintEdit = TRUE;
		//pImport->GetEditOptions()->m_bAllowEdit = FALSE;
...
void KDlgImportNode::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportNode::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportNode::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	...
	try
	{
		...
		for (int i= 0; i< nColumnCount; i++) {
			...
			bool bRed(false);
			CString strField(_T("")); {
                if (strColNm.CompareNoCase(COLUMN_NODE_ID)   == 0  ||
                    strColNm.CompareNoCase(COLUMN_NODE_TYPE) == 0  ||
                    strColNm.CompareNoCase(COLUMN_NODE_X)    == 0  ||
                    strColNm.CompareNoCase(COLUMN_NODE_Y)    == 0   ) 
                {
			...
			if(KEMImportUpdate == emImportType) {
				// TABLE_NODE
#pragma region Field Visible 설정 - node
				if (strColNm.CompareNoCase(COLUMN_NODE_TYPE) == 0  ||
					strColNm.CompareNoCase(COLUMN_NODE_X)    == 0  ||
					strColNm.CompareNoCase(COLUMN_NODE_Y)    == 0   ) {
...
#pragma endregion Field Visible 설정 - node
			}
...
int KDlgImportNode::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportNode::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportNode::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportNode::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportNode::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportNode::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportNode::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportNode::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportNode::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportNode::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportNode::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportNode::AllNullCheck()
{
	try 
	{
...
void KDlgImportNode::OnBnClickedOk()
{
	UpdateData( TRUE );

	if (NULL != m_pTable)
	{
		m_nCurRecCount = m_pTable->RecordCount();
	}
	...
	try
	{
		...
		if (true) { // 데이터 검증 및 중간 바이너리 작성
			...
			if (RThreadInfo.IsOK() == false) {
				...
				// 추가는 새로운 레코드 개수, 갱신은 현재 레코드 개수, 추가는 현재 + 추가되는 레코드 개수
				if (theApp.IsFreeVersion())
				{
					bool isOverNodeCount = false;
					if (KEMImportInsert == emImportType)
					{
						isOverNodeCount = oPara.TKeyInt32[0] > CKmzApp::MAX_NODE_OF_FREEVERSION;
					}
					else if (KEMImportUpdate == emImportType)
					{
						isOverNodeCount = oPara.TKeyInt32[1] > CKmzApp::MAX_NODE_OF_FREEVERSION;
					}
					else if (KEMImportAdd == emImportType)
					{
						isOverNodeCount = (m_nCurRecCount + oPara.TKeyInt32[1]) > CKmzApp::MAX_NODE_OF_FREEVERSION;
					}
		...
		if (true) { // Read 중간바이너리 & DB Import
            ...
            TxFeatureLayerPtr spLayerNode = nullptr; 
            TxFeatureLayerPtr spLayerZone = nullptr; 
                            
            KDlgNodeZoneTypeEdit oDlg(m_pTarget); {
                LoadLayerInfo(m_pTarget, spMapServer, spLayerNode, spLayerZone);
                oDlg.Init(spMapServer, spLayerNode, spLayerZone);
            }
	...
	if (GetSelectedImportType() == KEMImportUpdate || GetSelectedImportType() == KEMImportAdd) {
...
unsigned __stdcall KDlgImportNode::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportNode* pDlg = (KDlgImportNode*)pPara->TWindow;
...
void KDlgImportNode::Execute(ThreadPara* pPara)
{
	try 
	{
...
void KDlgImportNode::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
		if (KEMImportInsert == emImportType)
			ImportNodeData(pPara);
		else if (KEMImportUpdate == emImportType)
...
void KDlgImportNode::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KEMImportInsert == a_emImportType) {
			if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_NODE) == false) { // true: 데이터무, false : 데이터 존재
				//a_strMsg            = _T("Insert Node 수행시 관련된 테이블의 데이터가 삭제되며,\r\n");
				if (KmzSystem::GetLanguage() == KEMKorea) {
					a_strMsg = _T("Insert Node 수행시 관련된 테이블의 데이터가 삭제되며,\r\n");
				}
			...
			for (; iter != itEnd; ++iter) {
				...
				if (KmzSystem::GetLanguage() == KEMKorea) {
					if (0 == nNodeType) {
						a_strMsg.AppendFormat(_T("- (%d:Zone): %d건\r\n"), nNodeType, nCount);
					}
					else {
					...
					if (0 == nNodeType) {
						a_strMsg.AppendFormat(_T("- (%d:Zone): %d\r\n"), nNodeType, nCount);
					}
					else {
			...
			if (KmzSystem::GetLanguage() == KEMKorea) {
				...
				if (pPara->TKeyInt32[1] > 0) { //성공 건수가 있을 경우
					a_strMsg.AppendFormat(_T("#%d. Node Type 별 건수\r\n"), nIdx++);
				...
				if (pPara->TKeyInt32[1] > 0) { //성공 건수가 있을 경우
					a_strMsg.AppendFormat(_T("#%d. The number of records by node type :\r\n"), nIdx++);
...
void KDlgImportNode::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportNode::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		...
		for (int i= 0; i< nSizeVec; i++) {
			...
			if ( strIOColumnName.CompareNoCase(COLUMN_NODE_TYPE) == 0 && emDataType == KEMIODataTypeInteger) {
				int nNodeType = _ttoi(strImportValue);
				
				auto itFindNodeType = m_mapReadNodeType.find(nNodeType);
				if (itFindNodeType != m_mapReadNodeType.end()) {
					int &nTempCount = itFindNodeType->second;
					nTempCount++;
				} else {
					m_mapReadNodeType.insert(std::make_pair(nNodeType, 1));
				}
			}
		}
	} catch (int& ex) {
...
bool KDlgImportNode::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportNode::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportNode::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		bool bPK(false);
		// 조회 key value 추출
		if ( strIOColumnName.CompareNoCase(COLUMN_NODE_ID) == 0 ) {
			bPK = true;
		}
...
bool KDlgImportNode::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	return IsNodeID(nxID);
}

bool KDlgImportNode::IsNodeID(Integer a_nxValue)
{
...
bool KDlgImportNode::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportNode::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
	int  nIdCount      = (int)(a_mapKeyFieldValue.size());
	bool bWrongIdCount = false;

	if( nIdCount != 1) // node_id
		bWrongIdCount = true;

	return bWrongIdCount;
}

bool KDlgImportNode::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		...
		int nKeyCnt(0);
		nKeyCnt = 1; // TABLE_NODE 은 PK 개수가 하나
...
#include <string>
void KDlgImportNode::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportNode::ImportNodeData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	TxFeatureLayerPtr spZoneLayer    = nullptr; // 노드 Import는 존 정보도 Import 해야 한다.

	try 
	{
		LoadLayerInfo(m_pTarget, spMapServer, spFeatureLayer, spZoneLayer);
		if (nullptr == spMapServer)
			throw 1;
		if (nullptr == spFeatureLayer || nullptr == spZoneLayer)
			throw 2;
	} catch (int& ex) {
		CString strError(_T(""));
		if (1 == ex) {
	...
	try
	{
		...
		while(!ifs.eof()) {
			...
			ITxGeometryPtr spGeometry = TxPointPtr(new TxPoint(dX, dY));
			spFeatureLayer->Insert(node_id, spGeometry, oRecord);
...
void KDlgImportNode::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportNode::AddNodeData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
	...
	try 
	{
		LoadLayerInfo(m_pTarget, spMapServer, spFeatureLayer, spZoneLayer, false);
		if (nullptr == spMapServer)
			throw 1;
		if (nullptr == spFeatureLayer || nullptr == spZoneLayer)
			throw 2;
...
void KDlgImportNode::LoadLayerInfo(KTarget* a_pTarget, ITxMapServerPtr &spMapServer, TxFeatureLayerPtr &spFeatureLayer, TxFeatureLayerPtr &spZoneLayer, bool bInitExtent)
{
	try 
	{
...
CString KDlgImportNode::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportNode::ErrorFileOpen()
{
	try
	{
...
void KDlgImportNode::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		CString strColumnName(_T(""));
#pragma region Node 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
...
#pragma endregion Node 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportNode::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
		#pragma region Node 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
		...
		#pragma endregion Node 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportNode::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportNode::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportNode::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportNode::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportNode::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportNode::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportNode::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportNode::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportNode::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
bool KDlgImportNode::CheckBeforeFreeVersionImport()
{
	int nEncoding = GetSelectedEncoding();
	bool bContainsCol = IsDlgButtonChecked(IDC_CHECK1) == BST_CHECKED;
	...
	while (oReader.ReadLine(strLine))
	{
		...
		if (nLine > CKmzApp::MAX_NODE_OF_FREEVERSION)
		{
			return false;
		}
	}
