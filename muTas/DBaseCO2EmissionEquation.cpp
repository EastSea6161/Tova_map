...
KDBaseCO2EmissionEquation::KDBaseCO2EmissionEquation(void)
{
}
...
bool KDBaseCO2EmissionEquation::IsEmptyAssignResult( KTarget* a_pTarget )
{
	CString strQuery;
	bool    bResult = true;
...
void KDBaseCO2EmissionEquation::GetTravelTime( KTarget* a_pTarget, std::map<int, CString>& a_mapTravelTime )
{
...
void KDBaseCO2EmissionEquation::GetModeVolumeColumn( KTarget* a_pTarget, int a_nRunSeq, std::vector<CString>& a_vecModeVolColumn )
{
...
void KDBaseCO2EmissionEquation::GetLinkSpeed( KTarget* a_pTarget, int a_nIdxTravelTime, std::map<Integer, double>& a_mapLinkSpeed )
{
...
void KDBaseCO2EmissionEquation::GetLinkSpeed( KTarget* a_pTarget, KIOColumn* a_pLinkIOColumn, std::map<Integer, double>& a_mapLinkSpeed )
{
	a_mapLinkSpeed.clear();
...
void KDBaseCO2EmissionEquation::GetLinkVolume( KTarget* a_pTarget, CString a_strColumnName, std::map<Integer, double>& a_mapLinkVolume )
{
...
void KDBaseCO2EmissionEquation::UpdateResult( KTarget* a_pTarget, CString a_strColumnName, std::map<Integer, double> a_mapResult )
{
	CString strQuery;
	KDBaseConPtr spDBaseConnection = a_pTarget->GetDBaseConnection();
...
void KDBaseCO2EmissionEquation::GetLinkLength(KTarget* a_pTarget, std::map<Integer, double>& a_mapLinkLength)
{
	...
	try
	{
		strQuery = _T("SELECT link_id, ifnull(length, 0.0) FROM LINK ORDER BY link_id");
