...
IMPLEMENT_DYNAMIC(KZoneSelectDlg, KDialogEx)

KZoneSelectDlg::KZoneSelectDlg(CWnd* pParent /*=NULL*/)
	: KDialogEx(KZoneSelectDlg::IDD, pParent)
{

}

KZoneSelectDlg::~KZoneSelectDlg()
{
}

void KZoneSelectDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KZoneSelectDlg, KDialogEx)
    ON_BN_CLICKED(IDOK, &KZoneSelectDlg::OnBnClickedOk)
END_MESSAGE_MAP()


// KZoneSelectDlg 메시지 처리기입니다.


void KZoneSelectDlg::OnBnClickedOk()
{
...
BOOL KZoneSelectDlg::OnInitDialog()
{
...
void KZoneSelectDlg::InitReportHeader()
{
	CXTPReportColumn* pColumn = NULL;

	pColumn = m_wndZoneReportCtrl.AddColumn(new CXTPReportColumn(0, _T("Select"),  40, FALSE));
	pColumn->SetHeaderAlignment(DT_CENTER);
    if (KmzSystem::GetLanguage() == KEMKorea) {
        pColumn->SetCaption(_T("선택"));
    }

	pColumn = m_wndZoneReportCtrl.AddColumn(new CXTPReportColumn(1, _T("Zone ID"), 40));
	pColumn->SetHeaderAlignment(DT_CENTER);
    if (KmzSystem::GetLanguage() == KEMKorea) {
        pColumn->SetCaption(_T("존 노드 ID"));
    }

	pColumn = m_wndZoneReportCtrl.AddColumn(new CXTPReportColumn(2, _T("Name"),    60));
	pColumn->SetHeaderAlignment(DT_CENTER);
    if (KmzSystem::GetLanguage() == KEMKorea) {
...
void KZoneSelectDlg::UpdateReportData()
{
	try
	{
...
void KZoneSelectDlg::SetInitialSelectedZone( std::vector<Integer> a_vecInitialSelectedZone )
{
...
std::vector<Integer> KZoneSelectDlg::GetSelectedZone()
{
