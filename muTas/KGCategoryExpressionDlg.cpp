...
IMPLEMENT_DYNAMIC(KGCategoryExpressionDlg, CXTPResizeDialog)

KGCategoryExpressionDlg::KGCategoryExpressionDlg(CWnd* pParent /*=NULL*/)
	: CXTPResizeDialog(KGCategoryExpressionDlg::IDD, pParent)
{
    m_pGCategoryExpression = new KGCategoryExpression();
}

KGCategoryExpressionDlg::~KGCategoryExpressionDlg()
{
...
void KGCategoryExpressionDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KGCategoryExpressionDlg, CXTPResizeDialog)
    ON_BN_CLICKED(IDOK, &KGCategoryExpressionDlg::OnBnClickedOk)
    ON_CBN_SELCHANGE(IDC_COMBO_AGE_GROUP, &KGCategoryExpressionDlg::OnCbnSelchangeComboAgeGroup)
	ON_BN_CLICKED(IDC_BUTTON_EDIT, &KGCategoryExpressionDlg::OnBnClickedButtonEdit)
END_MESSAGE_MAP()



void KGCategoryExpressionDlg::SetTarget( KTarget* a_pTarget )
{
	if (a_pTarget == NULL)
	{
		return;
...
// KGCategoryExpressionDlg 메시지 처리기입니다.
void KGCategoryExpressionDlg::Expression(KGCategoryExpression* pExpression)
{
    m_pOriginCategoryExpression = pExpression;
    *m_pGCategoryExpression = *m_pOriginCategoryExpression;
}

BOOL KGCategoryExpressionDlg::OnInitDialog()
{
...
void KGCategoryExpressionDlg::loadCategoryClass()
{
	try
	{
...
int KGCategoryExpressionDlg::GetCategoryIndex(int category_id)
{
    int nIndex = -1;
    for ( size_t i=0; i<m_vecCategoryClass.size(); i++ )
    {
...
void KGCategoryExpressionDlg::loadCategoryAgeGroup(TCategoryClass* pCategory)
{
	try
	{
		KDBaseConPtr spDBaseConnection = KDBase::GetProjectDBConnection();
...
void KGCategoryExpressionDlg::loadCategoryValue()
{
...
void KGCategoryExpressionDlg::OnCbnSelchangeComboAgeGroup()
{
    SelchangeComboAgeGroup();
}

void KGCategoryExpressionDlg::SelchangeComboAgeGroup()
{
...
void KGCategoryExpressionDlg::OnBnClickedOk()
{
    UpdateData(TRUE);
    ...
    m_pGCategoryExpression->m_strInfo     = m_pCategoryClass->name;
    ...
    *m_pOriginCategoryExpression = *m_pGCategoryExpression;
    CXTPResizeDialog::OnOK();
}


void KGCategoryExpressionDlg::OnBnClickedButtonEdit()
{
...
BOOL KGCategoryExpressionDlg::InvalidateInputInfo()
{
	UpdateData(TRUE);
