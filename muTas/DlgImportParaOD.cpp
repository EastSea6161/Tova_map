...
// KDlgImportParaOD 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportParaOD, KWhiteDlgEx)

KDlgImportParaOD::KDlgImportParaOD(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportParaOD::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportParaOD::~KDlgImportParaOD()
{
}

void KDlgImportParaOD::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportParaOD, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportParaOD::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportParaOD::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportParaOD::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportParaOD::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportParaOD::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportParaOD::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportParaOD::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportParaOD::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportParaOD::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportParaOD::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportParaOD 메시지 처리기입니다.
void KDlgImportParaOD::ResizeComponent()
{
	try 
	{
...
void KDlgImportParaOD::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportParaOD::GetSelectedImportType()
{
...
BOOL KDlgImportParaOD::OnInitDialog()
{
...
void KDlgImportParaOD::RegisterZoneRelationTable()
{
	KTableRelationInfo::RelationTablesZone(m_mapZoneRelationTable);
}

int KDlgImportParaOD::IsZoneRelationTable(CString a_strTableName)
{
...
void KDlgImportParaOD::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportParaOD::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportParaOD::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportParaOD::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportParaOD::InitComboSeparator()
{
	try
	{
...
void KDlgImportParaOD::InitComboEncoding()
{
	try
	{
...
void KDlgImportParaOD::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportParaOD::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportParaOD::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportParaOD::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
int KDlgImportParaOD::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportParaOD::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportParaOD::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportParaOD::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportParaOD::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportParaOD::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportParaOD::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportParaOD::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportParaOD::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportParaOD::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportParaOD::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportParaOD::AllNullCheck()
{
	try 
	{
...
void KDlgImportParaOD::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportParaOD::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportParaOD* pDlg = (KDlgImportParaOD*)pPara->TWindow;
...
void KDlgImportParaOD::Execute(ThreadPara* pPara)
{
	try 
	{
...
void KDlgImportParaOD::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportParaOD::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_PARAMETER_OD) == false) { // true: 데이터무, false: 데이터 존재
...
void KDlgImportParaOD::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportParaOD::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportParaOD::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportParaOD::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportParaOD::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportParaOD::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
...
bool KDlgImportParaOD::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportParaOD::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportParaOD::IsODZoneID(KODKey a_oODKey)
{
...
bool KDlgImportParaOD::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
...
bool KDlgImportParaOD::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
...
#include <string>
void KDlgImportParaOD::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportParaOD::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportParaOD::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportParaOD::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportParaOD::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportParaOD::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportParaOD::ErrorFileOpen()
{
	try
	{
...
void KDlgImportParaOD::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region Para_OD 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
...
#pragma endregion Para_OD 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportParaOD::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
...
bool KDlgImportParaOD::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportParaOD::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportParaOD::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportParaOD::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportParaOD::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportParaOD::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportParaOD::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportParaOD::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportParaOD::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
