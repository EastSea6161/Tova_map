// DlgImportAccessStopNode.cpp : 구현 파일입니다.
//
...
// KDlgImportAccessStopNode 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportAccessStopNode, KWhiteDlgEx)

KDlgImportAccessStopNode::KDlgImportAccessStopNode(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportAccessStopNode::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportAccessStopNode::~KDlgImportAccessStopNode()
{
}

void KDlgImportAccessStopNode::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportAccessStopNode, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportAccessStopNode::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportAccessStopNode::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportAccessStopNode::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportAccessStopNode::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportAccessStopNode::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportAccessStopNode::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportAccessStopNode::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportAccessStopNode::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportAccessStopNode::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportAccessStopNode::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportAccessStopNode 메시지 처리기입니다.
void KDlgImportAccessStopNode::ResizeComponent()
{
	try 
	{
...
void KDlgImportAccessStopNode::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportAccessStopNode::GetSelectedImportType()
{
...
BOOL KDlgImportAccessStopNode::OnInitDialog()
{
...
void KDlgImportAccessStopNode::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportAccessStopNode::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportAccessStopNode::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportAccessStopNode::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportAccessStopNode::InitComboSeparator()
{
	try
	{
...
void KDlgImportAccessStopNode::InitComboEncoding()
{
	try
	{
...
void KDlgImportAccessStopNode::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportAccessStopNode::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportAccessStopNode::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportAccessStopNode::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	...
	try
	{
		...
		for (int i= 0; i< nColumnCount; i++) {
            ...
            bool bRed(false);
            CString strField(_T("")); {
                if (strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_ID) == 0  ||
                    strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_NODE_ID) == 0  ||
                    strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_MODE)    == 0  ) 
                {
...
#pragma region Field Visible 설정 - Access_Stop_Node
				if (strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_NAME) == 0) {
					pRecord->AddItem(new KColumnReportRecordItem(1));
					pRecord->SetVisible(FALSE);
					continue;
				}
#pragma endregion Field Visible 설정 - Access_Stop_Node

			} else if (KEMImportInsert == emImportType) {
				if (strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_NAME) == 0)	{
					m_ctrlReportColumn.RemoveRecordEx(pRecord);
					continue;
				}
			} else if (KEMImportAdd == emImportType) {
				if (strColNm.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_NAME) == 0)	{
...
int KDlgImportAccessStopNode::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportAccessStopNode::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportAccessStopNode::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportAccessStopNode::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportAccessStopNode::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportAccessStopNode::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportAccessStopNode::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportAccessStopNode::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportAccessStopNode::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportAccessStopNode::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportAccessStopNode::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportAccessStopNode::AllNullCheck()
{
	try 
	{
...
void KDlgImportAccessStopNode::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportAccessStopNode::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportAccessStopNode* pDlg = (KDlgImportAccessStopNode*)pPara->TWindow;
...
void KDlgImportAccessStopNode::Execute(ThreadPara* pPara)
{
	try 
	{
 		KDBaseImportTable::NodeRecord(m_pTable, m_mapNodeRecord);
 		KDBaseImportTable::GetZoneID (m_pTable, m_setZoneID);
		KDBaseImportTable::AccessStopNodeRecord(m_pTable, m_mapAccessStopNodeID);
...
void KDlgImportAccessStopNode::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
		if (KEMImportInsert == emImportType) {
			...
			KDBaseImportTable::UpdateAccessStopNodeNameData(m_pTable);
		} else if (KEMImportUpdate == emImportType) {
...
void KDlgImportAccessStopNode::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_ACCESS_STOP_NODE) == false) { // false : 데이터 존재
...
void KDlgImportAccessStopNode::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportAccessStopNode::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		...
		if( !ConfirmPrimaryKey(a_vecColMatch, a_row, mapFieldValue, KEMImportInsert) ) {
			a_bEverErrLine = true;
			...
			if (CheckEtcFilter(a_vecColMatch, a_row, strErrorMsg)) {
				a_bEverErrLine = true;
...
bool KDlgImportAccessStopNode::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportAccessStopNode::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
		...
		if( !ConfirmPrimaryKey(a_vecColMatch, a_row, mapFieldValue, KEMImportAdd) )	{
			//strErrorMsg.Format(_T("Line : %d =>\t ID 값 추출 실패"), a_nCurrentLine);
			if (KmzSystem::GetLanguage() == KEMKorea) {
				strErrorMsg.Format(_T("Line : %d =>\t ID 값 추출 실패"), a_nCurrentLine);
			}
...
bool KDlgImportAccessStopNode::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		bool bPK(false);
		// 조회 key value 추출
		if (strIOColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_ID) == 0 || 
			strIOColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_NODE_ID) == 0 || 
			strIOColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_MODE)    == 0 ) {
			bPK = true;
		}
...
bool KDlgImportAccessStopNode::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
		...
		for (int i= 0; i< nSizeVec; i++) {
			...
			if ( KImportCommon::IsNodeRelationColumn(strIOColumnName) )	{ // 데이터 체크 (TABLE_LINK)
				...
				if (IsNodeID(nxValue) == false)	{
					...
					if (KmzSystem::GetLanguage() == KEMKorea) {
						strErr.Format(_T("노드 테이블에 존재하지 않는 Node_ID : %I64d"), nxValue);
					}
					else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
						strErr.Format(_T("The Node ID %I64d does NOT exist in Node Table"), nxValue);
					}
					ThrowException(strErr);
				}
			} else if (KImportCommon::IsZoneRelationColumn(strIOColumnName))	{
				if( emDataType != KEMIODataTypeInteger )
					continue;

				nxValue = _ttoi64(strImportValue);
				if (IsZoneID(nxValue) == false)	{
					CString strErr;
					//strErr.Format(_T("존 테이블에 존재하지 않는 Zone_ID : %I64d"), nxValue);
					if (KmzSystem::GetLanguage() == KEMKorea) {
						strErr.Format(_T("존 테이블에 존재하지 않는 Zone_ID : %I64d"), nxValue);
					}
					else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
						strErr.Format(_T("The Zone ID %I64d does NOT exist in Zone Table"), nxValue);
					}
					ThrowException(strErr);
				}
			}
...
bool KDlgImportAccessStopNode::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for(; iter != itEnd; ++iter) {
		...
		if (strField.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_ID) == 0) {
			nxZoneID = iter->second;
		} else if (strField.CompareNoCase(COLUMN_ACCESS_STOP_NODE_NODE_ID) == 0) {
			nxNodeID = iter->second;
		} else if (strField.CompareNoCase(COLUMN_ACCESS_STOP_NODE_MODE) == 0) {
			nxMode   = iter->second;
		}
	}
	...
	return IsAccessStopNodeDBID(oODKey, (int)nxMode);

}

bool KDlgImportAccessStopNode::IsAccessStopNodeDBID(KODKey a_oODKey, int a_nMode)
{
	auto iter  = m_mapAccessStopNodeID.find( a_oODKey );
	auto itEnd = m_mapAccessStopNodeID.end();

	if(iter == itEnd) {
		return false;
	} else {
		std::set<int> &setMode = iter->second;
		if (setMode.find(a_nMode) == setMode.end())	{
			return false;
		}
	}
...
bool KDlgImportAccessStopNode::IsNodeID(Integer a_nxValue)
{
	auto iter = m_mapNodeRecord.find( a_nxValue );
	auto end  = m_mapNodeRecord.end();
	if(iter == end)	{
		return false;
	}
	return true;
...
bool KDlgImportAccessStopNode::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportAccessStopNode::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
	int  nIdCount      = (int)(a_mapKeyFieldValue.size());
	bool bWrongIdCount = false;

	if (nIdCount != 3) // zone_id, node_id, mode
		bWrongIdCount = true;

	return bWrongIdCount;
}

bool KDlgImportAccessStopNode::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
		...
		int nKeyCnt(0);
		nKeyCnt = 3; // TABLE_ACCESS_STOP_NODE 은 PK 개수가 셋
		...
		for( ; iter != itEnd; ++iter ) {
			...
			if (strField.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_ID) == 0) {
				...
				nxThirdID  = iter->second;
			}
		}

		KODKey oODKey(nxFirstID, nxSecondID);
		auto iterB  = m_mapDuplicateTripleID.find(oODKey);
		auto itEndB = m_mapDuplicateTripleID.end();
		
		if (iterB != itEndB) {
			std::set<Integer> &setThirdID = iterB->second;

			if (setThirdID.find(nxThirdID) == setThirdID.end()) {
				setThirdID.insert(nxThirdID);
			} else {
				return true;
			}
		} else 	{
			std::set<Integer> setThirdID;
			setThirdID.insert(nxThirdID);

			m_mapDuplicateTripleID.insert(std::make_pair(oODKey, setThirdID));
		}
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
...
#include <string>
void KDlgImportAccessStopNode::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportAccessStopNode::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	CString strDelQuery; {
		strDelQuery.Format(_T("delete from %s"), m_pTable->Name());
		spDBase->ExecuteUpdate(strDelQuery);
	}
...
void KDlgImportAccessStopNode::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportAccessStopNode::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);

	int nImportDataCount = (int)pPara->TKeyInt32[1]; // 양호건수가 Add 전체건수가 된다.
...
CString KDlgImportAccessStopNode::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportAccessStopNode::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportAccessStopNode::ErrorFileOpen()
{
	try
	{
...
void KDlgImportAccessStopNode::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		CString strColumnName(_T(""));
#pragma region Access_Stop_Node 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		...
		for (int i= 0; i < nColumnCount; i++) {
			...
			if( strColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_ID) == 0 || 
				strColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_NODE_ID) == 0 || 
				strColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_MODE) == 0 )
			{
...
#pragma endregion Access_Stop_Node 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportAccessStopNode::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
	#pragma region Access_Stop_Node 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
		...
		for (int i = 0; i < nColumnCount; ++i) {
			...
			if (nSelectedIndex == 0) {
				...
				if (strColumnName.CompareNoCase(COLUMN_ACCESS_STOP_NODE_ZONE_NAME) == 0) {
...
#pragma endregion Access_Stop_Node 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportAccessStopNode::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportAccessStopNode::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportAccessStopNode::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportAccessStopNode::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportAccessStopNode::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportAccessStopNode::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportAccessStopNode::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportAccessStopNode::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportAccessStopNode::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
