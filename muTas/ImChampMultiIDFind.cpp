...
IMPLEMENT_DYNAMIC(KImChampMultiIDFind, KResizeDialogEx)

    CString KImChampMultiIDFind::m_strPreSearch = _T("");

KImChampMultiIDFind::KImChampMultiIDFind(CWnd* pParent /*=NULL*/)
    : KResizeDialogEx(KImChampMultiIDFind::IDD, pParent), m_pMapView(NULL), m_nMapSelectType(-1), m_nDrawnMap(false), m_strTiltleName(_T("")), m_nNecessaryCount(0)
{
	m_strReportTitle = _T("Select ID:");
}

KImChampMultiIDFind::~KImChampMultiIDFind()
{
...
void KImChampMultiIDFind::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KImChampMultiIDFind, KResizeDialogEx)
    ON_NOTIFY(NM_DBLCLK, IDC_REPORT, &KImChampMultiIDFind::OnReportRowDblClick)
    ON_NOTIFY(NM_CLICK,  IDC_REPORT, &KImChampMultiIDFind::OnReportItemClick)

    ON_NOTIFY(NM_CLICK,  IDC_REPORT2, &KImChampMultiIDFind::OnReport2ItemClick)
    
    ON_BN_CLICKED(IDC_BUTTON_FIND, &KImChampMultiIDFind::OnBnClickedButtonFind)
    ON_EN_CHANGE(IDC_EDIT_SEARCH,  &KImChampMultiIDFind::OnEnChangeEditSearch)

    ON_BN_CLICKED(IDOK, &KImChampMultiIDFind::OnBnClickedOk)
    ON_BN_CLICKED(IDC_BUTTON_MAP, &KImChampMultiIDFind::OnBnClickedMap)
    ON_BN_CLICKED(IDCANCEL, &KImChampMultiIDFind::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_CHECK1, &KImChampMultiIDFind::OnBnClickedCheckSelectAll)
END_MESSAGE_MAP()


// KImChampMultiIDFind 메시지 처리기입니다.
BOOL KImChampMultiIDFind::OnInitDialog()
{
...
void KImChampMultiIDFind::InitialControl()
{
    try
    {
...
void KImChampMultiIDFind::OnReportRowDblClick( NMHDR* pNMHDR, LRESULT* pResult )
{
    try
    {
...
void KImChampMultiIDFind::OnReportItemClick( NMHDR* pNMHDR, LRESULT* pResult )
{
    try
    {
        XTP_NM_REPORTRECORDITEM* pClickedItem = (XTP_NM_REPORTRECORDITEM*) pNMHDR;
...
void KImChampMultiIDFind::OnReport2ItemClick( NMHDR* pNMHDR, LRESULT* pResult )
{
    try
    {
        XTP_NM_REPORTRECORDITEM* pClickedItem = (XTP_NM_REPORTRECORDITEM*) pNMHDR;
...
void KImChampMultiIDFind::DrawReport( CString a_strFilter )
{
    try
    {
        SetDlgItemText(IDC_STATIC_INFO, _T(""));
...
void KImChampMultiIDFind::DrawReport2( )
{
    try
    {
...
void KImChampMultiIDFind::OnBnClickedButtonFind()
{
    try
    {
        CString strFilter(_T(""));
...
void KImChampMultiIDFind::OnEnChangeEditSearch()
{
    try
    {
...
BOOL KImChampMultiIDFind::PreTranslateMessage(MSG* pMsg)
{
...
void KImChampMultiIDFind::SetIDCaption( std::vector<KIDCaption> a_vecIDCaption )
{
    try
    {
...
void KImChampMultiIDFind::OnBnClickedOk()
{
    try
    {
		if (m_nNecessaryCount > (int)m_setSelectionID.size())
		{
			...
			{
				strMsg.Format(_T("Please select at least %d items."), m_nNecessaryCount);
			}
...
void KImChampMultiIDFind::OnBnClickedCancel()
{
    if (m_nDrawnMap == true && m_pMapView != nullptr)
    {
        m_pMapView->ClearScreenDrawEdit();
...
CString KImChampMultiIDFind::GetSelectedID()
{
...
std::set<CString> KImChampMultiIDFind::GetSelectedIDSet()
{
	return m_setSelectionID;
}

void KImChampMultiIDFind::SetSelectedID( CString a_strSelectID )
{
    try
    {
...
void KImChampMultiIDFind::MapSelectionFeedback( Integer a_nxID)
{
    try
    {
        CString strID(_T(""));
...
void KImChampMultiIDFind::MapSelectionFeedback( std::vector<Integer> a_vecSelection )
{
    try
    {
...
void KImChampMultiIDFind::MapSelectionNoneFeedback()
{

}

void KImChampMultiIDFind::OnBnClickedMap()
{
    try
    {
        if (m_pMapView != nullptr)
...
void KImChampMultiIDFind::SetMapNodeSelectInfo( KMapView* a_pMapView, std::vector<Integer> a_vecID, TPreferenceSymbol a_SymbolStyle )
{
    try
    {
...
void KImChampMultiIDFind::SetMapNodeSelectInfoA( KMapView* a_pMapView, const std::map<Integer, CString> &a_mapID, TPreferenceSymbol a_SymbolStyle )
{
	try
	{
		m_pMapView       = a_pMapView;
...
void KImChampMultiIDFind::OnBnClickedCheckSelectAll()
{
	try
	{
...
void KImChampMultiIDFind::SetTitleName( CString a_strTitleName )
{
	m_strTiltleName = a_strTitleName;
}

void KImChampMultiIDFind::SetReportTitle( CString a_strTitleName )
{
