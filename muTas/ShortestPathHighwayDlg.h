...
class KShortestPathHighwayDlg : public KResizeDialogEx, public KShortestPathBase, public ImChampModelessFeedback, public KTableObserver
{
	DECLARE_DYNAMIC(KShortestPathHighwayDlg)

public:
	KShortestPathHighwayDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~KShortestPathHighwayDlg();

// 대화 상자 데이터입니다.
	enum { IDD = IDD_5010_ShortestpathHighway };
...
protected:
	...
	virtual void ModelessCancleFeedback(UINT nDialogID);
		void SetDestinationEditBox(CString &a_strSearchedIDs);
...
typedef std::shared_ptr<KShortestPathHighwayDlg> KShortestPathHighwayDlgPtr;