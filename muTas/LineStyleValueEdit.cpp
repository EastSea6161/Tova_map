...
// KLineStyleValue 대화 상자입니다.

IMPLEMENT_DYNAMIC(KLineStyleValueEdit, KDialogEx)

KLineStyleValueEdit::KLineStyleValueEdit(CWnd* pParent /*=NULL*/)
	: KDialogEx(KLineStyleValueEdit::IDD, pParent)
{
	m_nPreSelectedStyleID      = -1;
	m_nRealDigit = 0;
}

KLineStyleValueEdit::~KLineStyleValueEdit()
{
}

void KLineStyleValueEdit::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KLineStyleValueEdit, KDialogEx)
	ON_CBN_SELCHANGE(IDC_COMBO_STYLE, &KLineStyleValueEdit::OnCbnSelchangeComboStyle)
	ON_NOTIFY(NM_CLICK, IDC_REPORT, &KLineStyleValueEdit::OnReportItemClick)
	ON_NOTIFY(XTP_NM_REPORT_VALUECHANGED, IDC_REPORT, &KLineStyleValueEdit::OnReportItemValueChanged)
	ON_EN_SETFOCUS(IDC_EDIT_STYLE_NAME, &KLineStyleValueEdit::OnEnSetfocusEditStyleName)
	ON_CBN_SETFOCUS(IDC_COMBO_STYLE, &KLineStyleValueEdit::OnCbnSetfocusComboStyle)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_ADD, &KLineStyleValueEdit::OnBnClickedButtonContentsAdd)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_DELETE, &KLineStyleValueEdit::OnBnClickedButtonContentsDelete)
	ON_BN_CLICKED(IDOK, &KLineStyleValueEdit::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_STYLE_DELETE, &KLineStyleValueEdit::OnBnClickedButtonStyleDelete)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO1, IDC_RADIO2, &KLineStyleValueEdit::OnBnClickedRadioEditMethod)
END_MESSAGE_MAP()

BOOL KLineStyleValueEdit::OnInitDialog()
{
	...
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(1, _T("Width"),   10));
	pColumn->SetHeaderAlignment(DT_CENTER);
	pColumn->SetSortable(FALSE);
    if (KmzSystem::GetLanguage() == KEMKorea) {
	...
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(2, _T("<=  Range  <"),   30));
	pColumn->SetHeaderAlignment(DT_CENTER);
	pColumn->SetSortable(FALSE);
    if (KmzSystem::GetLanguage() == KEMKorea) {
	...
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(3, _T("Delete"), 8));
	pColumn->SetHeaderAlignment(DT_CENTER);
	pColumn->SetSortable(FALSE);
    if (KmzSystem::GetLanguage() == KEMKorea) {
...
void KLineStyleValueEdit::UpdateUIState()
{
...
void KLineStyleValueEdit::OnCbnSelchangeComboStyle()
{
	StyleSelectChangeAction();
}

void KLineStyleValueEdit::OnReportItemClick( NMHDR* pNotifyStruct, LRESULT* pResult )
{
...
void KLineStyleValueEdit::OnReportItemValueChanged( NMHDR* pNotifyStruct, LRESULT* pResult )
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
...
int KLineStyleValueEdit::GetDisplayDecimalPoint(int a_nRealDigit)
{
	int nDecimalPoint(1);
...
void KLineStyleValueEdit::RecalRange()
{
	int nDisplayDecimalPoint = GetDisplayDecimalPoint(m_nRealDigit);
...
void KLineStyleValueEdit::LoadInitialStyle()
{
	m_nRealDigit = 0;
...
void KLineStyleValueEdit::StyleSelectChangeAction()
{
...
#pragma endregion 자릿수 구하기
	...
	for (int i=0; i<nTemplateCount; i++)
	{
		...
		pItem->SetItemData(vecTemplateValue[i].TLineType);
		pItem->SetCaption(strSymbol);

		pItemSize = (CXTPReportRecordItemNumber*)pRecord->AddItem(new CXTPReportRecordItemNumber(1.0, _T("%.1f")));         
		pItemSize->SetEditable(TRUE);
		pItemSize->SetAlignment(xtpColumnTextCenter);

		pItemSize->SetValue(vecTemplateValue[i].TWidth);
		pItemSize->SetItemData((DWORD_PTR)vecTemplateValue[i].TWidth);
...
void KLineStyleValueEdit::OnEnSetfocusEditStyleName()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_UNCHECKED);
...
void KLineStyleValueEdit::OnCbnSetfocusComboStyle()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_CHECKED);
...
void KLineStyleValueEdit::OnBnClickedButtonContentsAdd()
{
	int      nStyle;
	COLORREF clrColor;
	double   dSize;
	double   dMin;
	double   dMax;
	PreStyle(nStyle, clrColor, dSize, dMin, dMax);
	...
	pItem->SetItemData(nStyle);
	pItem->SetCaption(strSymbol);

	pItemSize = (CXTPReportRecordItemNumber*)pRecord->AddItem(new CXTPReportRecordItemNumber(dSize, _T("%.1f")));         
	pItemSize->SetEditable(TRUE);
	pItemSize->SetAlignment(xtpColumnTextCenter);
	pItemSize->SetItemData((DWORD_PTR)dSize);
...
void KLineStyleValueEdit::OnBnClickedButtonContentsDelete()
{
	CXTPReportRecords*	  pRecords = NULL;
	CXTPReportRecord*     pRecord  = NULL;
	CXTPReportRecordItem* pItem    = NULL;
...
void KLineStyleValueEdit::Apply()
{
	KDBaseConPtr spDBaseConnection = KDBase::GetProjectDBConnection(); //KDBase::GetApplicationDBConnection();
	if (spDBaseConnection == NULL)
	{		
	...
	try
	{
		...
		if (!bOverwrite)
		{
			strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplate(ID,Type,Name) Values('%d','%d','%s')"), nID,1,strStyleName);
			spDBaseConnection->ExecuteUpdate(strSQL);
		}		

		strSQL.Format(_T("Delete From LinkTemplateValue Where ID = '%d'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);
...
void KLineStyleValueEdit::OnBnClickedOk()
{
	Apply();
}

void KLineStyleValueEdit::OnBnClickedButtonStyleDelete()
{
	int  nCheckedRadioBtn  = GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2);
	...
	try
	{
		...
		spDBaseConnection->BeginTransaction();		
		strSQL.Format(_T("Delete From LinkTemplate Where ID = '%d' and Type='1'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);		
...
void KLineStyleValueEdit::PreStyle( int& ar_nStyle, COLORREF& ar_clrColor, double& ar_dSize, double& ar_dMin, double& ar_dMax )
{
...
#pragma region style, color, size 정하기
	pRecordLast = pRecords->GetAt(nRecordCount-1); // last
	pItem       = pRecordLast->GetItem(0);

	ar_nStyle   = pItem->GetItemData();
	ar_clrColor = pItem->GetTextColor();

	pItemNumber = (CXTPReportRecordItemNumber*)pRecordLast->GetItem(1);
	ar_dSize    = pItemNumber->GetValue();
#pragma endregion style, color, size 정하기
	...
	else if (nRecordCount == 2)
	{
		...
		CXTPReportRecordItemText* pItemTextLast = (CXTPReportRecordItemText*)pRecordLast->GetItem(2);
...
// 	double dMax = 2.0 * dMin - _ttof(vecValue[0]);
	...
	ar_dMin = dMinWill;
	ar_dMax = dMaxWill; // above 이기 때문에 0이 될것이다.
}


void KLineStyleValueEdit::OnBnClickedRadioEditMethod(UINT nID)
{
	try
	{
