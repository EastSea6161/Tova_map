// PointStyleValueEdit.cpp : 구현 파일입니다.
//
...
IMPLEMENT_DYNAMIC(KPointStyleValueEdit, KDialogEx)

KPointStyleValueEdit::KPointStyleValueEdit(CWnd* pParent /*=NULL*/)
	: KDialogEx(KPointStyleValueEdit::IDD, pParent)
{
...
KPointStyleValueEdit::~KPointStyleValueEdit()
{
}

void KPointStyleValueEdit::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KPointStyleValueEdit, KDialogEx)
	ON_CBN_SELCHANGE(IDC_COMBO_STYLE, &KPointStyleValueEdit::OnCbnSelchangeComboStyle)
	ON_NOTIFY(NM_CLICK, IDC_REPORT, &KPointStyleValueEdit::OnReportItemClick)
	ON_NOTIFY(XTP_NM_REPORT_VALUECHANGED, IDC_REPORT, &KPointStyleValueEdit::OnReportItemValueChanged)
	ON_EN_SETFOCUS(IDC_EDIT_STYLE_NAME, &KPointStyleValueEdit::OnEnSetfocusEditStyleName)
	ON_CBN_SETFOCUS(IDC_COMBO_STYLE, &KPointStyleValueEdit::OnCbnSetfocusComboStyle)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_ADD, &KPointStyleValueEdit::OnBnClickedButtonContentsAdd)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_DELETE, &KPointStyleValueEdit::OnBnClickedButtonContentsDelete)
	ON_BN_CLICKED(IDOK, &KPointStyleValueEdit::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_STYLE_DELETE, &KPointStyleValueEdit::OnBnClickedButtonStyleDelete)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO1, IDC_RADIO2, &KPointStyleValueEdit::OnBnClickedRadioEditMethod)
END_MESSAGE_MAP()


BOOL KPointStyleValueEdit::OnInitDialog()
{
...
void KPointStyleValueEdit::UpdateUIState()
{
...
void KPointStyleValueEdit::OnCbnSelchangeComboStyle()
{
	StyleSelectChangeAction();
}

void KPointStyleValueEdit::OnReportItemClick( NMHDR* pNotifyStruct, LRESULT* pResult )
{
...
void KPointStyleValueEdit::OnReportItemValueChanged( NMHDR* pNotifyStruct, LRESULT* pResult )
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
...
int KPointStyleValueEdit::GetDisplayDecimalPoint(int a_nRealDigit)
{
	int nDecimalPoint(1);
...
void KPointStyleValueEdit::RecalRange()
{
	int nDisplayDecimalPoint = GetDisplayDecimalPoint(m_nRealDigit);
...
void KPointStyleValueEdit::LoadInitialStyle()
{
	m_nRealDigit = 0;
...
void KPointStyleValueEdit::StyleSelectChangeAction()
{
...
#pragma endregion 자릿수 구하기
	...
	for (int i=0; i<nTemplateCount; i++)
	{
		...
		CString strSymbol;strSymbol.AppendChar(vecTemplateValue[i].TSymbol);
		pItem->SetTextColor(RGB(vecTemplateValue[i].TR, vecTemplateValue[i].TG, vecTemplateValue[i].TB));
		pItem->SetItemData(vecTemplateValue[i].TSymbol);
		...
		pItemSize->SetValue(vecTemplateValue[i].TSize);
		pItemSize->SetItemData((DWORD_PTR)vecTemplateValue[i].TSize);
...
void KPointStyleValueEdit::OnEnSetfocusEditStyleName()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_UNCHECKED);
...
void KPointStyleValueEdit::OnCbnSetfocusComboStyle()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_CHECKED);
...
void KPointStyleValueEdit::OnBnClickedButtonContentsAdd()
{
	int      nStyle;
	COLORREF clrColor;
	double   dSize;
	...
	CString strSymbol;strSymbol.AppendChar(nStyle);
...
void KPointStyleValueEdit::OnBnClickedButtonContentsDelete()
{
	CXTPReportRecords*	  pRecords = NULL;
	CXTPReportRecord*     pRecord  = NULL;
	CXTPReportRecordItem* pItem    = NULL;
...
void KPointStyleValueEdit::Apply()
{
	KDBaseConPtr spDBaseConnection = KDBase::GetProjectDBConnection(); //KDBase::GetApplicationDBConnection();
	if (spDBaseConnection == NULL)
	{		
	...
	try
	{
		...
		if (!bOverwrite)
		{
			strSQL.Format(_T("INSERT OR REPLACE INTO NodeTemplate(ID,Type,Name) Values('%d','%d','%s')"), nID,1,strStyleName);
			spDBaseConnection->ExecuteUpdate(strSQL);
		}		

		strSQL.Format(_T("Delete From NodeTemplateValue Where ID = '%d'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);
...
void KPointStyleValueEdit::OnBnClickedOk()
{
	Apply();
}


void KPointStyleValueEdit::OnBnClickedButtonStyleDelete()
{
	int  nCheckedRadioBtn  = GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2);
	...
	try
	{
		...
		spDBaseConnection->BeginTransaction();		
		strSQL.Format(_T("Delete From NodeTemplate Where ID = '%d' and Type='1'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);		
...
void KPointStyleValueEdit::PreStyle( int& ar_nStyle, COLORREF& ar_clrColor, double& ar_dSize, double& ar_dMin, double& ar_dMax )
{
...
#pragma region style, color, size 정하기
	pRecordLast = pRecords->GetAt(nRecordCount-1);
...
void KPointStyleValueEdit::OnBnClickedRadioEditMethod(UINT nID)
{
	try
	{
