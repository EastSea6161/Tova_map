...
IMPLEMENT_DYNAMIC(KDlgImportTransit, KWhiteDlgEx)

KDlgImportTransit::KDlgImportTransit(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportTransit::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportTransit::~KDlgImportTransit()
{
}

void KDlgImportTransit::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportTransit, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportTransit::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportTransit::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportTransit::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportTransit::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportTransit::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportTransit::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportTransit::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportTransit::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportTransit::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportTransit::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportTransit 메시지 처리기입니다.
void KDlgImportTransit::ResizeComponent()
{
	try 
	{
...
void KDlgImportTransit::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportTransit::GetSelectedImportType()
{
...
BOOL KDlgImportTransit::OnInitDialog()
{
...
void KDlgImportTransit::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportTransit::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportTransit::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportTransit::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportTransit::InitComboSeparator()
{
	try
	{
...
void KDlgImportTransit::InitComboEncoding()
{
	try
	{
...
void KDlgImportTransit::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportTransit::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportTransit::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportTransit::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
		...
		for (int i= 0; i< nColumnCount; i++) {
            ...
            CString strField(_T("")); {
                if (strColNm.CompareNoCase(COLUMN_TRANSIT_ID)   == 0  ||
                    strColNm.CompareNoCase(COLUMN_TRANSIT_TYPE) == 0  ||
                    strColNm.CompareNoCase(COLUMN_TRANSIT_DETAIL_TYPE) == 0 ) 
                {
...
// 			pItem = pRecord->AddItem(new CXTPReportRecordItemText(strPK));
			...
			if(KEMImportUpdate == emImportType) {
				// TABLE_TRANSIT
#pragma region Field Visible 설정 - Transit
				if (strColNm.CompareNoCase(COLUMN_TRANSIT_TYPE) == 0)
				{
					pRecord->AddItem(new KColumnReportRecordItem(1));
					pRecord->SetVisible(FALSE);
					continue;
				}
#pragma endregion Field Visible 설정 - Transit
			} else if (KEMImportInsert == emImportType) {
				if (strColNm.CompareNoCase(COLUMN_TRANSIT_TYPE) == 0) {
					m_ctrlReportColumn.RemoveRecordEx(pRecord);
					/*jyk: insert 모드에서의 visible 가능 한 케이스는 오직 AutoGenerate 일 경우에만,,, 이를 제외한 케이스는 visible이 아닌 addrecord 자체를 하지 못하도록 해야함.
					pRecord->AddItem(new KColumnReportRecordItem(1));
					pRecord->SetVisible(FALSE);
					*/
					continue;
...
int KDlgImportTransit::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportTransit::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportTransit::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportTransit::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportTransit::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportTransit::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportTransit::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportTransit::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportTransit::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportTransit::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportTransit::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportTransit::AllNullCheck()
{
	try 
	{
...
void KDlgImportTransit::OnBnClickedOk()
{
	UpdateData( TRUE );
	...
	KDefineTransitCodeCostDlg	oCostDefineDlg(m_pTarget, true);
	oCostDefineDlg.DoModal();
...
unsigned __stdcall KDlgImportTransit::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportTransit* pDlg = (KDlgImportTransit*)pPara->TWindow;
...
void KDlgImportTransit::Execute(ThreadPara* pPara)
{
	try 
	{
...
void KDlgImportTransit::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportTransit::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_TRANSIT) == false) { // true: 데이터무, false : 데이터 존재
					a_strMsg = _T("Insert 수행시 관련된 테이블(노선 경로 자료 포함)의\r\n");
...
void KDlgImportTransit::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportTransit::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportTransit::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportTransit::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportTransit::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportTransit::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	return IsTransitID(nxID);
}

bool KDlgImportTransit::IsTransitID(Integer a_nxValue)
{
	auto iter = m_setTransitID.find( a_nxValue );
	auto end  = m_setTransitID.end();
	if(iter == end) {
		return false;
	}
...
bool KDlgImportTransit::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
	int  nIdCount      = (int)(a_mapKeyFieldValue.size());
	bool bWrongIdCount = false;

	if( nIdCount != 1) // transit_id
		bWrongIdCount = true;

	return bWrongIdCount;
}

bool KDlgImportTransit::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
		...
		int nKeyCnt(0);
		nKeyCnt = 1; // TABLE_TRANSIT 은 PK 개수가 하나
...
#include <string>
void KDlgImportTransit::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportTransit::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	KImportCommon::DeleteTransitInfoRelation(spDBase);
...
void KDlgImportTransit::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportTransit::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportTransit::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportTransit::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportTransit::ErrorFileOpen()
{
	try
	{
...
void KDlgImportTransit::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		CString strColumnName(_T(""));
#pragma region Transit 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		...
		for (int i= 0; i < nColumnCount; i++) {
			...
			if( strColumnName.CompareNoCase(COLUMN_TRANSIT_ID) == 0 || 
				strColumnName.CompareNoCase(COLUMN_TRANSIT_TYPE) == 0 || 
				strColumnName.CompareNoCase(COLUMN_TRANSIT_DETAIL_TYPE) == 0 )
			{
...
#pragma endregion Transit 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportTransit::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region Transit 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
		...
		for (int i = 0; i < nColumnCount; ++i) {
			...
			if ( nSelectedIndex == 0) {
				...
				/*if( strColumnName.CompareNoCase(COLUMN_TRANSIT_TYPE) == 0  ||
					strColumnName.CompareNoCase(COLUMN_TRANSIT_DETAIL_TYPE) == 0 ) {
...
#pragma endregion Transit 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportTransit::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportTransit::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportTransit::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportTransit::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportTransit::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
		KImportCommon::NotifyTransitInfoRelation(m_pTarget);
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportTransit::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportTransit::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportTransit::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportTransit::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
