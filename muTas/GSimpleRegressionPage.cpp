// GSimpleRegressionPage.cpp : 구현 파일입니다.
//

#include "stdafx.h"
#include "KmzApp.h"
#include "GSimpleRegressionPage.h"
#include "afxdialogex.h"
#include "ExpressionElementField.h"
#include "ExpressionElementFunction.h"
#include "ExpressionElementNumber.h"
#include "ExpressionElementOperator.h"
#include "GenerationInfo.h"
#include "PAPurpose.h"
#include "GRegressionExpression.h"
#include "GSimpleExpressionContainer.h"
#include "Target.h"
#include "ImTasDBase.h"
#include "DBaseConnector.h"


const static TCHAR* szOperators[] = {
	_T("+"), _T("-"), _T("*"), _T("/"), 
	_T("^"), _T("("), _T(")")
};

const static TCHAR* szFunctions[] = {
	_T("SQRT"), _T("EXP"), _T("LN"), _T("LOG"), 
	_T("ABS"), _T("POW")
};



// KGSimpleRegressionPage 대화 상자입니다.

IMPLEMENT_DYNAMIC(KGSimpleRegressionPage, CXTPPropertyPage)

KGSimpleRegressionPage::KGSimpleRegressionPage(KGFormulaDefaultDlgPtr* a_spKGFormulaDefaultDlg)
	:	CXTPPropertyPage(KGSimpleRegressionPage::IDD),
		m_strExpression(_T("")),
		m_pGInfo(NULL),
		m_spKGFormulaDefaultDlg(a_spKGFormulaDefaultDlg)
{
	m_pExpression = new KGRegressionExpression;
}


KGSimpleRegressionPage::~KGSimpleRegressionPage()
{
	if(NULL != m_pExpression)
	{
		delete m_pExpression;
	}
}


void KGSimpleRegressionPage::DoDataExchange(CDataExchange* pDX)
{
	CXTPPropertyPage::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_EDIT1, m_strExpression);
	DDX_Control(pDX, IDC_LIST1, m_lstField);
	DDX_Control(pDX, IDC_LIST4, m_lstOperator);
	DDX_Control(pDX, IDC_LIST3, m_lstFunction);
}


BEGIN_MESSAGE_MAP(KGSimpleRegressionPage, CXTPPropertyPage)
	ON_LBN_DBLCLK(IDC_LIST1, &KGSimpleRegressionPage::OnDblclkList1)
	ON_LBN_DBLCLK(IDC_LIST4, &KGSimpleRegressionPage::OnDblclkList2)
	ON_LBN_DBLCLK(IDC_LIST3, &KGSimpleRegressionPage::OnDblclkList3)
    ON_LBN_SELCHANGE(IDC_LIST1, &KGSimpleRegressionPage::OnLbnSelchangeList1)
	ON_BN_CLICKED(IDC_BUTTON1, &KGSimpleRegressionPage::OnBnClickedButtonDefault)
END_MESSAGE_MAP()



void KGSimpleRegressionPage::SetGenerationInfo(KGenerationInfo* a_pGInfo)
{
	m_pGInfo = a_pGInfo;
	ASSERT(m_pGInfo);
	KGBaseExpressionContainer* pBaseCont = m_pGInfo->ExpressionContainer();
	if(NULL == pBaseCont)
	{
		m_pExpression->SetCaptionExpression(m_oParameterInfo.TstrRegresionFomula);
		return;
	}

	ASSERT(pBaseCont->ContainerType() == KEMGExpressionContainerSimple);
	KGSimpleExpressionContainer* pContainer = (KGSimpleExpressionContainer*)pBaseCont;

	KGBaseExpression* pBaseExp = pContainer->ExpressionObject();
	if(NULL == pBaseExp)
	{
		return;
	}

	ASSERT(pBaseExp->GenerationModel() == KEMGenerationRegression);

	*m_pExpression = *(KGRegressionExpression*)(pBaseExp);
}


std::tstring KGSimpleRegressionPage::GetFieldName(LPCTSTR a_strElement)
{
	std::tstring str = a_strElement;
	std::tstring strFieldName = _T("");
	if((str.find(_T("[")) != std::tstring::npos) || (str.find(_T("]")) != std::tstring::npos))
	{
		std::tstring::size_type n = str.find_first_not_of(_T("["));
		str = (n == std::tstring::npos) ? str : str.substr(n, str.length());
		n = str.find_last_not_of(_T("]"));
		str = (n == std::tstring::npos) ? str : str.substr(0, n + 1);
	}

	int nListCount = m_lstField.GetCount();
	for(int i = 0; i < nListCount; ++i)
	{
		CString strItem;
		m_lstField.GetText(i, strItem);
		if(strItem.CompareNoCase(str.c_str()) == 0)
		{
			KIOColumn* pColumn = (KIOColumn*)m_lstField.GetItemData(i);
			strFieldName = pColumn->Name();
			break;
		}
	}

	return strFieldName;
}


std::tstring KGSimpleRegressionPage::GetFieldCaption(LPCTSTR a_strElement)
{
	std::tstring strFieldCaption = a_strElement;
	if((strFieldCaption.find(_T("[")) != std::tstring::npos) || 
		(strFieldCaption.find(_T("]")) != std::tstring::npos))
	{
		std::tstring::size_type n = strFieldCaption.find_first_not_of(_T("["));
		strFieldCaption = (n == std::tstring::npos) ? strFieldCaption : 
			strFieldCaption.substr(n, strFieldCaption.length());
		n = strFieldCaption.find_last_not_of(_T("]"));
		strFieldCaption = (n == std::tstring::npos) ? strFieldCaption : 
			strFieldCaption.substr(0, n + 1);
	}
	return strFieldCaption;
}


// KGFunctionRegressionDlg 메시지 처리기입니다.


//Step 1
BOOL KGSimpleRegressionPage::OnInitDialog()
{
	CXTPPropertyPage::OnInitDialog();

	HICON hIcon = LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_DLG_ICO) );
	SetIcon( hIcon, TRUE );
	SetIcon( hIcon, FALSE );

	InitFieldList();
	InitOperatorList();
	InitFunctionList();

	//m_strExpression = m_pExpression->GetCaption();
    m_strExpression = m_pExpression->GetCaptionExpression();
	UpdateData(FALSE);

// 	if (KmzSystem::IsPassengerSystem())
// 	{
// 		GetDlgItem(IDC_BUTTON1)->ShowWindow(SW_HIDE);
// 	}

	if(nullptr == (*m_spKGFormulaDefaultDlg))
	{
		(*m_spKGFormulaDefaultDlg) = KGFormulaDefaultDlgPtr(new KGFormulaDefaultDlg(m_pTarget, m_pParentSheet));
		(*m_spKGFormulaDefaultDlg)->Create(KGFormulaDefaultDlg::IDD, this);
	}

	return TRUE;  // return TRUE unless you set the focus to a control
	// 예외: OCX 속성 페이지는 FALSE를 반환해야 합니다.
}


void KGSimpleRegressionPage::InitFieldList(void)
{
	KIOTables* pIOTables = m_pTarget->Tables();
	
	ASSERT(NULL != pIOTables);

	KIOTable* pTable = pIOTables->FindTable(_T("sed_vector"));
	ASSERT(NULL != pTable);

	const KIOColumns* pColumns = pTable->Columns();
	ASSERT(NULL != pColumns);

	int nColumnCount = pColumns->ColumnCount();
	for(int i = 0; i < nColumnCount; ++i)
	{
		KIOColumn*      pColumn      = pColumns->GetColumn(i);
		KEMIOColumnType emColumnType = pColumn->ColumnType();

		if(pColumn->CodeType() == KEMIOCodeTypeIsSingleCode || pColumn->CodeType() == KEMIOCodeTypeIsMultiCode)
		{
			continue;
		}

		if((KEMIOColumnTypeFixed == emColumnType) || (KEMIOColumnTypeUserDefine == emColumnType))
		{
			int nIndex = m_lstField.InsertString(-1,  pColumn->Caption());
			m_lstField.SetItemData(nIndex, (DWORD_PTR)pColumn);
		}
	}
}


void KGSimpleRegressionPage::InitOperatorList(void)
{
	int nOperator = sizeof(szOperators) / sizeof(TCHAR*);

	for(int i = 0; i < nOperator; ++i)
	{
		m_lstOperator.InsertString(-1, szOperators[i]);
	}
}


void KGSimpleRegressionPage::InitFunctionList(void)
{
	int nFunction = sizeof(szFunctions) / sizeof(TCHAR*);

	for(int i = 0; i < nFunction; ++i)
	{
		m_lstFunction.InsertString(-1, szFunctions[i]);
	}
}


//Step 2
void KGSimpleRegressionPage::OnDblclkList1()
{
	UpdateData(TRUE);
	int nSel = m_lstField.GetCurSel();
	if(LB_ERR != nSel)
	{
		CString strField;
		m_lstField.GetText(nSel, strField);
		if(m_strExpression.IsEmpty() == false)
		{
			m_strExpression += _T(" ");
		}

		m_strExpression += _T("[");
		m_strExpression += strField;
		m_strExpression += _T("]");
		UpdateData(FALSE);
	}
}


//Step 3
void KGSimpleRegressionPage::OnDblclkList2()
{
	UpdateData(TRUE);
	int nSel = m_lstOperator.GetCurSel();
	if(LB_ERR != nSel)
	{
		CString strOperator;
		m_lstOperator.GetText(nSel, strOperator);
		if(m_strExpression.IsEmpty() == false)
		{
			m_strExpression += _T(" ");
		}
		m_strExpression += strOperator;
		UpdateData(FALSE);
	}
}


//Step 4
void KGSimpleRegressionPage::OnDblclkList3()
{
	UpdateData(TRUE);
	int nSel = m_lstFunction.GetCurSel();
	if(LB_ERR != nSel)
	{
		CString strFunction;
		m_lstFunction.GetText(nSel, strFunction);
		if(m_strExpression.IsEmpty() == false)
		{
			m_strExpression += _T(" ");
		}
		m_strExpression += strFunction;
		UpdateData(FALSE);
	}
}


//Step 5
void KGSimpleRegressionPage::OnLbnSelchangeList1()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}


//Step 6
BOOL KGSimpleRegressionPage::OnApply()
{
	UpdateData(TRUE);

    m_pExpression->ClearElements();
    m_pExpression->SetCaptionExpression(m_strExpression);

	std::tstring str = (LPCTSTR)m_strExpression;

	if( FALSE == InvalidateInputInfo())
	{
		return FALSE;
	}

	typedef boost::tokenizer<
		boost::char_separator<TCHAR>, 
		std::tstring::const_iterator, 
		std::tstring> ttokenizer;

	boost::char_separator<TCHAR> sep(_T(" "));
	ttokenizer tokens(str, sep);
	for(ttokenizer::iterator it = tokens.begin(); it != tokens.end(); ++it)
	{
		std::tstring strToken = *it;
		if(isField(strToken.c_str()) == true)
		{
			std::tstring strName = GetFieldName(strToken.c_str());
			std::tstring strCaption = GetFieldCaption(strToken.c_str());
			m_pExpression->AddField(strName.c_str(), strCaption.c_str());
		}
		else if(isOperator(strToken.c_str()) == true)
		{
			m_pExpression->AddOperator(strToken.at(0));
		}
		else if(isFunction(strToken.c_str()) == true)
		{
			m_pExpression->AddFunction(strToken.c_str());
		}
		else
		{
			m_pExpression->AddNumber(_tstof(strToken.c_str()));
		}
	}

	KGSimpleExpressionContainer* pContainer = 
		(KGSimpleExpressionContainer*)m_pGInfo->ExpressionContainer();
	if(NULL == pContainer)
	{
		pContainer = (KGSimpleExpressionContainer*)(m_pGInfo->CreateExpression());
	}

	KGRegressionExpression* pExpression = (KGRegressionExpression*)(pContainer->ExpressionObject());
	if(NULL == pExpression)
	{
		pExpression = (KGRegressionExpression*)(pContainer->CreateExpressionObject());
	}

	*pExpression = *m_pExpression;

	CXTPPropertyPage::OnApply();
	return TRUE;
}


BOOL KGSimpleRegressionPage::InvalidateInputInfo()
{
	UpdateData(TRUE);

	if (m_strExpression == _T(""))
	{
		if(m_pParentSheet != NULL)
		{
			m_pParentSheet->SetActivePage(this);
		}

		AfxMessageBox(_T("입력한 수식이 없습니다.")); 
		return FALSE;
	}

	return TRUE;
}


bool KGSimpleRegressionPage::isField(LPCTSTR a_strElement)
{
	std::tstring str = a_strElement;
	if((str.find(_T("[")) != std::tstring::npos) || (str.find(_T("]")) != std::tstring::npos))
	{
		std::tstring::size_type n = str.find_first_not_of(_T("["));
		str = (n == std::tstring::npos) ? str : str.substr(n, str.length());
		n = str.find_last_not_of(_T("]"));
		str = (n == std::tstring::npos) ? str : str.substr(0, n + 1);
	}

	int nListCount = m_lstField.GetCount();
	for(int i = 0; i < nListCount; ++i)
	{
		CString strItem;
		m_lstField.GetText(i, strItem);
		if(strItem.CompareNoCase(str.c_str()) == 0)
		{
			return true;
		}
	}

	return false;
}


bool KGSimpleRegressionPage::isOperator(LPCTSTR a_strElement)
{
	std::tstring str = a_strElement;
	static std::tstring strOperator = _T("+-*/^()");
	if(str.length() > 1)
	{
		return false;
	}

	int nOperator = sizeof(szOperators) / sizeof(TCHAR*);
	for(int i = 0; i < nOperator; ++i)
	{
		if(str.compare(szOperators[i]) == 0)
		{
			return true;
		}
	}

	return false;
}


bool KGSimpleRegressionPage::isFunction(LPCTSTR a_strElement)
{
	std::tstring str = a_strElement;
	int nFunction = sizeof(szFunctions) / sizeof(TCHAR*);
	for(int i = 0; i < nFunction; ++i)
	{
		if(_tcsicmp(str.c_str(), szFunctions[i]) == 0)
		{
			return true;
		}
	}

	return false;
}

void KGSimpleRegressionPage::SetTarget( KTarget* a_pTarget )
{
	if (a_pTarget == NULL)
	{
		return;
	}

	m_pTarget = a_pTarget;
}


BOOL KGSimpleRegressionPage::OnSetActive()
{

	/*m_pParentSheet->AddButton(IDS_PROJECTVIEW_NETWORK, IDC_BTN_LOAD);*/
	
	
	/*
	CString strTest = _T("LOAD");

	CButton* pButton1 = new CButton();
	pButton1->Create(strTest, WS_CHILD | BS_PUSHBUTTON | WS_TABSTOP | WS_VISIBLE, CRect(7,200, 57, 214), m_pParentSheet, IDC_BTN_LOAD);*/

	/*strTest = _T("SAVE");

	CButton* pButton2 = new CButton();
	pButton1->Create(strTest, WS_CHILD | BS_PUSHBUTTON | WS_TABSTOP | WS_VISIBLE, CRect(7,200, 57, 214), m_pParentSheet, IDC_BTN_LOAD);

	strTest = _T("DEFAULT");

	CButton* pButton3 = new CButton();
	pButton1->Create(strTest, WS_CHILD | BS_PUSHBUTTON | WS_TABSTOP | WS_VISIBLE, CRect(7,200, 57, 214), m_pParentSheet, IDC_BTN_LOAD);*/

	return CXTPPropertyPage::OnSetActive();
}


void KGSimpleRegressionPage::OnBnClickedButtonDefault()
{	
	if (KmzSystem::IsPassengerSystem())
	{
		SetPassengerDefault();
	}
	else
	{
		SetFreightDefault();
	}
	
	UpdateData(FALSE);
}

void KGSimpleRegressionPage::SetPassengerDefault()
{
	try
	{
		KGBaseExpressionContainer* pBaseCont = m_pDefaultGInfo->ExpressionContainer();
		if (pBaseCont == nullptr)
		{
			return;
		}

		KGSimpleExpressionContainer* pContainer = (KGSimpleExpressionContainer*)pBaseCont;
		if (nullptr == pContainer)
		{
			return;
		}

		KGBaseExpression* pBaseExp = pContainer->ExpressionObject();
		if(NULL == pBaseExp)
		{
			return;
		}

		*m_pExpression	= *(KGRegressionExpression*)(pBaseExp);

		GetDlgItem(IDC_EDIT1)->SetWindowText(m_pExpression->GetCaptionExpression());

	}
	catch (KExceptionPtr ex)
	{
		TxLogDebug(ex->GetErrorMessage());
	}
	catch(...)
	{
		TxLogDebugException();
	}
}

void KGSimpleRegressionPage::SetFreightDefault()
{
	try
	{
		m_strExpression = _T("");

		if(m_oParameterInfo.TstrRegresionFomula == _T(""))
		{
			AfxMessageBox(_T("설정한 수식이 존재하지 않습니다."));
		}
		else
		{
			m_strExpression = m_oParameterInfo.TstrRegresionFomula;
		}
	}
	catch (...)
	{
		TxLogDebugException();
	}
}
