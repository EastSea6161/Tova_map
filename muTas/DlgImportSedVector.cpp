// DlgImportSedVector.cpp : 구현 파일입니다.
//
...
// KDlgImportSedVector 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportSedVector, KWhiteDlgEx)

KDlgImportSedVector::KDlgImportSedVector(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportSedVector::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportSedVector::~KDlgImportSedVector()
{
}

void KDlgImportSedVector::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportSedVector, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportSedVector::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportSedVector::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportSedVector::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportSedVector::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportSedVector::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportSedVector::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportSedVector::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportSedVector::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportSedVector::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportSedVector::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportSedVector 메시지 처리기입니다.
void KDlgImportSedVector::ResizeComponent()
{
	try 
	{
...
void KDlgImportSedVector::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportSedVector::GetSelectedImportType()
{
...
BOOL KDlgImportSedVector::OnInitDialog()
{
...
void KDlgImportSedVector::RegisterZoneRelationTable()
{
	KTableRelationInfo::RelationTablesZone(m_mapZoneRelationTable);
}

int KDlgImportSedVector::IsZoneRelationTable(CString a_strTableName)
{
...
void KDlgImportSedVector::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportSedVector::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportSedVector::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportSedVector::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportSedVector::InitComboSeparator()
{
	try
	{
...
void KDlgImportSedVector::InitComboEncoding()
{
	try
	{
...
void KDlgImportSedVector::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportSedVector::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportSedVector::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportSedVector::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
int KDlgImportSedVector::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportSedVector::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportSedVector::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportSedVector::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportSedVector::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportSedVector::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportSedVector::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportSedVector::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportSedVector::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportSedVector::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportSedVector::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportSedVector::AllNullCheck()
{
	try 
	{
...
void KDlgImportSedVector::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportSedVector::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportSedVector* pDlg = (KDlgImportSedVector*)pPara->TWindow;
...
void KDlgImportSedVector::Execute(ThreadPara* pPara)
{
	try 
	{
		KDBaseImportTable::GetZoneID(m_pTable, m_setZoneID);
		KDBaseImportTable::ZoneRelationTableRecord(m_pTable, m_setSubZoneID);
...
void KDlgImportSedVector::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportSedVector::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_SED_VECTOR) == false) { // true: 데이터무, false : 데이터 존재
...
void KDlgImportSedVector::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportSedVector::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportSedVector::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportSedVector::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportSedVector::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportSedVector::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
...
bool KDlgImportSedVector::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	int nZoneRelationColumnCount = IsZoneRelationTable(m_pTable->Name());
	if( nZoneRelationColumnCount != 1 ) {
		return false; // Sed_Vector 테이블은 1이 나와야 한다.
	}
	...
	return IsSubZoneID(nxID);
}

bool KDlgImportSedVector::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportSedVector::IsSubZoneID(Integer a_nxValue)
{
	auto iter = m_setSubZoneID.find( a_nxValue );
	auto end  = m_setSubZoneID.end();
	if(iter == end) {
		return false;
	}
...
bool KDlgImportSedVector::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
...
bool KDlgImportSedVector::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
...
#include <string>
void KDlgImportSedVector::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportSedVector::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportSedVector::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportSedVector::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportSedVector::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportSedVector::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportSedVector::ErrorFileOpen()
{
	try
	{
...
void KDlgImportSedVector::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region SED_Vector 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
...
void KDlgImportSedVector::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
...
bool KDlgImportSedVector::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportSedVector::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportSedVector::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportSedVector::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportSedVector::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportSedVector::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportSedVector::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportSedVector::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportSedVector::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
