...
class KDlgImportTransitLine : public KWhiteDlgEx
{
	DECLARE_DYNAMIC(KDlgImportTransitLine)

public:
	KDlgImportTransitLine(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~KDlgImportTransitLine();

	// 대화 상자 데이터입니다.
	enum { IDD = IDD_201_01_06_IMPORT_TABLE_TRANSITLINE };
...
private:
	...
	void         CSV2BinaryTransitLineNode(bool a_bFirstLine, std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow &a_row, std::ofstream& ofs, QBicTxtWriter& ofErr, TTransitLine& a_oPastLine );
...
private:
	...
	bool IsErrorNodeData(Integer a_nxNodeID);
...
//★ 2016-10-07, nombara /*NodeBase*/
private:
    class TInputSubPathInfo {
    public:
    ...
    class TInputPath {
    public:
        std::vector<TInputSubPathInfo> TSubPaths;
    };
private:
    std::vector<TColMatch>        m_vecColMatch; /*NodeBase*/
    std::map<__int64, TInputPath> m_mapOutPath;  /*NodeBase*/
    void ExecuteNodeBase(ThreadPara* pPara);
    void ExecuteNodeBaseImport(ThreadPara* pPara);
private:
    void AlightColumn(std::set<CString>& setAlight);
    void LoadCsvNodeBaseData(ThreadPara* pPara, std::map<__int64, TInputPath>& mapOutPath, bool bExceptionFlag);
private:
    void ImportNodeBaseData(ThreadPara* pPara, bool bInsertFlag);
};
