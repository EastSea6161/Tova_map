// DlgImportTransitLineNode.cpp : 구현 파일입니다.
//
...
IMPLEMENT_DYNAMIC(KDlgImportTransitLineNode, KWhiteDlgEx)

KDlgImportTransitLineNode::KDlgImportTransitLineNode(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportTransitLineNode::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
	, FIRST_SEQ(1)
	, SECOND_SEQ(2)
{
	...
	m_strMiddleFile_LinkBase.Format(_T("%s\\%s"), strTargetLocation, _T("importData2.dat"));
	...
	// link base 테이블에 insert할 KIOColumn의 주소정보(Transit_nodes로 부터 column 정보를 담는다.)
	m_setOnlyLinkColumn.clear();
	...
	for (int i= 0; i< nColumnCount; i++) {
		...
		if (strColNm.CompareNoCase(COLUMN_TRANSIT_ID)   == 0  ||
			strColNm.CompareNoCase(COLUMN_TRANSIT_NODE_ID) == 0  ||
			strColNm.CompareNoCase(COLUMN_TRANSIT_NODE_SEQ) == 0  ||
			strColNm.CompareNoCase(COLUMN_TRANSIT_NODE_STATION_YN) == 0   ) 
		{
			m_setOnlyLinkColumn.insert((DWORD_PTR)pColumn);
		}
	}
}

KDlgImportTransitLineNode::~KDlgImportTransitLineNode()
{
}

void KDlgImportTransitLineNode::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportTransitLineNode, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportTransitLineNode::OnBnClickedOk)

	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportTransitLineNode::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportTransitLineNode::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportTransitLineNode::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportTransitLineNode::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO5, &KDlgImportTransitLineNode::OnBnClickedRadioImportType)

	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportTransitLineNode::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportTransitLineNode::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportTransitLineNode::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportTransitLineNode::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportTransitLineNode 메시지 처리기입니다.
void KDlgImportTransitLineNode::ResizeComponent()
{
	try 
	{
...
void KDlgImportTransitLineNode::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportTransitLineNode::GetSelectedImportType()
{
	int nID = GetCheckedRadioButton(IDC_RADIO4, IDC_RADIO5);
...
BOOL KDlgImportTransitLineNode::OnInitDialog()
{
	...
	QBicReportCtrlSetting::Default(m_ctrlReportColumn, TRUE);
...
void KDlgImportTransitLineNode::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportTransitLineNode::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportTransitLineNode::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportTransitLineNode::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportTransitLineNode::InitComboSeparator()
{
	try
	{
...
void KDlgImportTransitLineNode::InitComboEncoding()
{
	try
	{
...
void KDlgImportTransitLineNode::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportTransitLineNode::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportTransitLineNode::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportTransitLineNode::UpdateColumnView()
{
	CheckDlgButton(IDC_CHECK4, BST_CHECKED);
...
void KDlgImportTransitLineNode::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
int KDlgImportTransitLineNode::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportTransitLineNode::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportTransitLineNode::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportTransitLineNode::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportTransitLineNode::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
...
void KDlgImportTransitLineNode::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportTransitLineNode::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportTransitLineNode::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportTransitLineNode::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportTransitLineNode::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportTransitLineNode::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportTransitLineNode::AllNullCheck()
{
	try 
	{
...
void KDlgImportTransitLineNode::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportTransitLineNode::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportTransitLineNode* pDlg = (KDlgImportTransitLineNode*)pPara->TWindow;
...
void KDlgImportTransitLineNode::Execute(ThreadPara* pPara)
{
	try 
	{
        //★ 노드, F-T 링크, 링크, 라인인포 정보 조회
		KDBaseImportTable::NodeRecord      (m_pTable, m_mapNodeTxPoint);
		KDBaseImportTable::LinkRecordODBase(m_pTable, m_mapFromToLink);
		KDBaseImportTable::LinkRecord      (m_pTable, m_setLinkID);
		KDBaseImportTable::TransitRecord   (m_pTable, m_setTransitID);
...
void KDlgImportTransitLineNode::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
		if (KEMImportInsert == emImportType) {
			ImportBaseData(pPara);
			ImportBaseData_link(pPara);
		}
		else if (KEMImportUpdate == emImportType) { // Add
			UpdateBaseData(pPara);
			UpdateBaseData_link(pPara);
		}
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
...
void KDlgImportTransitLineNode::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_TRANSITNODES) == false) { // true: 데이터무, false : 데이터 존재
...
void KDlgImportTransitLineNode::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	QBicFile::DeleteFile(m_strMiddleFile_LinkBase);

	std::ofstream ofs(m_strMiddleFile, std::ios::binary);
	std::ofstream ofs_link(m_strMiddleFile_LinkBase, std::ios::binary);
...
#pragma endregion Load vecColMatch
	...
	TTransitLine oPastLine;	
	TTransitLine oPastLine_link;	

	try
	{
		CString strReadLine(_T(""));
		bool    bEverErrLine(false);
		bool    bEverErrLine_link(false);
		...
		while( oReader.ReadLine(strReadLine) ) {
            ...
            //★ 한줄 읽어서 -> CSVRow : std::vector<CString> 형태로 변경
			CSVRow row; {
                ParseCSVLineString( strReadLine, tcSeparator, row );
            }			
			 
			if (true) { // 디폴트인.. 노드Base
				CSV2BinaryTransitLine(bFirstLine, vecColMatch, nCurrentLine, bEverErrLine, row, ofs, ofErr, oPastLine);
			}

			if (false == bEverErrLine) { // 서브인.. 링크Base
				CSV2BinaryTransitLine_ToLink(bFirstLine, vecColMatch, nCurrentLine, bEverErrLine_link, row, ofs_link, ofErr, oPastLine_link);
			}
		...
		if (true) { // 디폴트인.. 노드Base
			if(bEverErrLine) {
				...
				//ChangeSEQ(oPastLine);
				ChangeStationYN(oPastLine);
				WriteTransitData(vecColMatch, oPastLine, ofs);
			}
		}

		if (true) { // 서브인.. 링크Base
			if(bEverErrLine_link) {
				CString strErrorMsg(_T(""));
				//strErrorMsg.Format(_T("Import 실패\r\n %s 파일에서 내역을 확인 하세요."), IMPORT_ERROR_FILE_NAME);
				if (KmzSystem::GetLanguage() == KEMKorea) {
					strErrorMsg.Format(_T("Import 실패\r\n %s 파일에서 내역을 확인 하세요."), IMPORT_ERROR_FILE_NAME);
				...
				ChangeSEQ(oPastLine_link);
				ChangeStationYN(oPastLine_link);
				WriteTransitData_link(m_setOnlyLinkColumn, vecColMatch, oPastLine_link, ofs_link);
			}
		}
...
void KDlgImportTransitLineNode::CSV2BinaryTransitLine(bool a_bFirstLine, std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow &a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr, TTransitLine& a_oPastLine )
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);

	TTransitLine oTransitLine;
	std::map<int, CString> &mapCSVData = oTransitLine.mapCSVInfo;
	...
	try 
	{
		...
		for (int i= 0; i< nSizeVec; i++) {
			...
			const KIOColumn*  pIOColumn   = oTColMatch.TovaIOColumn;
			KEMIODataType     emDataType  = pIOColumn->DataType();
			strIOColumnName               = pIOColumn->Name();

			if( a_row.size() > (size_t)(oTColMatch.TxtColumnIdx -1) ) 
				strImportValue = a_row[oTColMatch.TxtColumnIdx -1];
			else
				strImportValue = _T("");

			if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_ID) == 0 )	{
				oTransitLine.nxLineID = _ttoi64(strImportValue);
				m_setUpdateTransitLineID.insert(oTransitLine.nxLineID);
			} else if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_SEQ) == 0) {
				oTransitLine.nSeq = _ttoi(strImportValue);
			} else if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_ID) == 0) {
				oTransitLine.nxNodeID = _ttoi64(strImportValue);
			} else if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_STATION_YN) == 0) {
				oTransitLine.nStationLocation = i;
				oTransitLine.nStationYN       = _ttoi(strImportValue);
			} 
			mapCSVData.insert(std::make_pair(i, strImportValue));
		}

		if( CheckNodeData(oTransitLine.nxNodeID) ) {
			a_bEverErrLine = true;
			//strErrorMsg.Format(_T("Line : %d =>\tNode 테이블에 존재하지 않는 Node_ID = %I64d"), a_nCurrentLine, oTransitLine.nxNodeID);
			if (KmzSystem::GetLanguage() == KEMKorea) {
				strErrorMsg.Format(_T("Line : %d =>\tNode 테이블에 존재하지 않는 Node_ID = %I64d"), a_nCurrentLine, oTransitLine.nxNodeID);
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				strErrorMsg.Format(_T("Line : %d =>\tThe Node ID %I64d does NOT exist in Node Table"), a_nCurrentLine, oTransitLine.nxNodeID);
			}
			ofErr.Write(strErrorMsg);
		}
		if ( IsErrorTransitID (oTransitLine.nxLineID) ) {
			a_bEverErrLine = true;
			//strErrorMsg.Format(_T("Line : %d =>\t 대중교통 노선운행 자료 테이블에 존재하지 않는 ID = %I64d"), a_nCurrentLine, oTransitLine.nxLineID);
			if (KmzSystem::GetLanguage() == KEMKorea) {
				strErrorMsg.Format(_T("Line : %d =>\t 대중교통 노선운행 자료 테이블에 존재하지 않는 ID = %I64d"), a_nCurrentLine, oTransitLine.nxLineID);
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				strErrorMsg.Format(_T("Line : %d =>\tThe ID %I64d does NOT exist in Urban Transit Info Table"), a_nCurrentLine, oTransitLine.nxLineID);
			}
			ofErr.Write(strErrorMsg);
		}
		if (CheckTransitSEQ(oTransitLine, a_oPastLine)) {
			a_bEverErrLine = true;
			//strErrorMsg.Format(_T("Line : %d =>\t노선별 라인에 맞지 않는 SEQ"), a_nCurrentLine);
			if (KmzSystem::GetLanguage() == KEMKorea) {
				strErrorMsg.Format(_T("Line : %d =>\tSeq 번호에 오류가 존재함"), a_nCurrentLine);
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				strErrorMsg.Format(_T("Line : %d =>\tError exists in Seq column"), a_nCurrentLine);
			}
			ofErr.Write(strErrorMsg);
		}

		if (FIRST_SEQ == oTransitLine.nSeq) {
			if (false == a_bFirstLine) {
				ChangeStationYN(a_oPastLine); // 여기 들어왔다는 것은 결국 이전 노선의 마지막Seq라는 의미
			}
			ChangeStationYN(oTransitLine); // 현재 노선의 첫번째 Seq
			
			if (CheckPrimaryKey(oTransitLine.nxLineID)) {
				a_bEverErrLine = true;
				//strErrorMsg.Format(_T("Line : %d =>\t Primar Key 중복 ID = %I64d"), a_nCurrentLine, oTransitLine.nxLineID);
				if (KmzSystem::GetLanguage() == KEMKorea) {
					strErrorMsg.Format(_T("Line : %d =>\t Primar Key 중복 ID = %I64d"), a_nCurrentLine, oTransitLine.nxLineID);
				}
				else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
					strErrorMsg.Format(_T("Line : %d =>\t Duplicated Primar Key %I64d exist"), a_nCurrentLine, oTransitLine.nxLineID);
				}
				ofErr.Write(strErrorMsg);
			}
		}

		if (false == a_bEverErrLine && false == a_bFirstLine) {
			WriteTransitData(a_vecColMatch, a_oPastLine, ofs);
		}

		a_oPastLine = oTransitLine;
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportTransitLineNode::CSV2BinaryTransitLine_ToLink(bool a_bFirstLine, std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow &vecRecord,  std::ofstream& ofs,  QBicTxtWriter& ofErr, TTransitLine& a_oPastLine )
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
	...
	try 
	{
		...
		int nCnt = (int)a_vecColMatch.size();
		...
		for (int i=0; i<nCnt; i++) {
			...
			if( vecRecord.size() > (size_t)(oTColMatch.TxtColumnIdx -1) ) 
				strImportValue = vecRecord[oTColMatch.TxtColumnIdx -1];
			else
				strImportValue = _T("");
			...
			if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_ID) == 0 ) {
				...
				oTransitLine.nSeqLocation = i;
				oTransitLine.nSeq         = _ttoi(strImportValue);
			}
			else if (strIOColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_ID) == 0) {
				//★ NodeBase 이기때문에 실제로 Node_id 가 매칭된다.			
				oTransitLine.nLinkLocation    = i;
				oTransitLine.nxNodeID         = _ttoi64(strImportValue);
			}
			else if(strIOColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_STATION_YN) == 0) {
				oTransitLine.nStationLocation = i;
...
// 		if ( IsErrorTransitID (oTransitLine.nxLineID) ) {
// 			a_bEverErrLine = true;
// 			strErrorMsg.Format(_T("Line : %d =>\t TansitInfo 테이블에 존재하지 않는 TransitID = %I64d"), a_nCurrentLine, oTransitLine.nxLineID);
// 			ofErr.Write(strErrorMsg);
// 		}
// 		if (CheckTransitSEQ(oTransitLine, a_oPastLine)) {
// 			a_bEverErrLine = true;
...
// 		}
		...
		if (FIRST_SEQ == oTransitLine.nSeq) { 
			//FindLinkByTNode(a_nCurrentLine, oTransitLine, oTransitLine.strLinkID, a_bEverErrLine, ofErr);
			if (false == a_bFirstLine) {
				//ChangeStationYN(oTransitLine);
				ChangeStationYN(a_oPastLine);
			}
			ChangeStationYN(oTransitLine);
// 			if (CheckPrimaryKey(oTransitLine.nxLineID)) {
// 				a_bEverErrLine = true;
...
// 			}
			...
			Integer nxFNodeID = a_oPastLine.nxNodeID;
			Integer nxTnodeID = oTransitLine.nxNodeID;
			KODKey oLinkInfo(nxFNodeID, nxTnodeID);

			if (CheckTransitNodeToLine(oTransitLine, a_oPastLine, oLinkInfo)) {
				a_bEverErrLine = true;
				//strErrorMsg.Format(_T("Line : %d =>\tLink 테이블에 존재하지 않는 FLink : F_NodeID = %I64d, TNodeID = %I64d"), a_nCurrentLine, nxFNodeID , nxTnodeID);
				if (KmzSystem::GetLanguage() == KEMKorea) {
					strErrorMsg.Format(_T("Line : %d =>\tLink 테이블에 존재하지 않는 FLink : F_NodeID = %I64d, TNodeID = %I64d"), a_nCurrentLine, nxFNodeID, nxTnodeID);
				}
				else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
					strErrorMsg.Format(_T("Line : %d =>\tThe FromLink (%I64d,%I64d) does NOT exist in Link Table"), a_nCurrentLine, nxFNodeID, nxTnodeID);
				}
				ofErr.Write(strErrorMsg);
			}
		}
		...
		if (false == a_bEverErrLine) {
			if (SECOND_SEQ != oTransitLine.nSeq) {
				if (false == a_bFirstLine) {
					ChangeSEQ(a_oPastLine);
					WriteTransitData_link(m_setOnlyLinkColumn, a_vecColMatch, a_oPastLine, ofs);
				}
			}
		}
...
void KDlgImportTransitLineNode::WriteTransitData( std::vector<TColMatch> &a_vecColMatch, TTransitLine &a_oPastLine, std::ofstream& ofs )
{
	CString strColumnName = _T("");

	std::map<int, CString> mapCSVData = a_oPastLine.mapCSVInfo;
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		CString strCSVValue(_T(""));
		auto itFind = mapCSVData.find(i);
		if (itFind != mapCSVData.end()) {
			strCSVValue = itFind->second;
		}

		if(emDataType == KEMIODataTypeDouble)
		{
			double dValue = _ttof(strCSVValue);
			ofs.write(reinterpret_cast<char*>(&dValue), sizeof(double));
		}
		else if(emDataType == KEMIODataTypeInteger)
		{
			Integer nxValue = _ttoi64(strCSVValue);
			ofs.write(reinterpret_cast<char*>(&nxValue), sizeof(Integer));
		}
		else if(emDataType == KEMIODataTypeString)
		{
			const int STR_LENGTH = 200;
			if(strCSVValue.GetLength() > STR_LENGTH - 1)
				strCSVValue.Truncate(STR_LENGTH - 1);
			TCHAR     szStrOut[STR_LENGTH];

			_tcscpy_s(szStrOut, strCSVValue);
			ofs.write( reinterpret_cast<char*>( szStrOut  ), sizeof(TCHAR) * STR_LENGTH );
		}
	}
}

void KDlgImportTransitLineNode::WriteTransitData_link(std::set<DWORD_PTR> &a_setOnlyLinkColumn, std::vector<TColMatch> &a_vecColMatch, TTransitLine &a_oPastLine, std::ofstream& ofs )
{
	CString strColumnName = _T("");
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		DWORD_PTR dwPtr = (DWORD_PTR)pIOColumn;
		if (a_setOnlyLinkColumn.find(dwPtr) == a_setOnlyLinkColumn.end()) {
			continue;
		}
...
bool KDlgImportTransitLineNode::CheckLinkData(Integer a_nxLinkID)
{
...
bool KDlgImportTransitLineNode::CheckNodeData(Integer a_nxNodeID)
{
	if (m_mapNodeTxPoint.find(a_nxNodeID) == m_mapNodeTxPoint.end()) {
		return true;
	}
...
bool KDlgImportTransitLineNode::IsErrorTransitID(Integer a_nxTransitID)
{
	std::set<Integer>::iterator iter = m_setTransitID.find(a_nxTransitID);
	std::set<Integer>::iterator end  = m_setTransitID.end();
...
bool KDlgImportTransitLineNode::CheckTransitSEQ(TTransitLine &a_oCurrntLine, TTransitLine &a_oPastLine)
{
	int nPastSEQ    = a_oPastLine.nSeq;
	int nCurrentSEQ = a_oCurrntLine.nSeq;

	if (a_oCurrntLine.nxLineID != a_oPastLine.nxLineID && a_oCurrntLine.nSeq != FIRST_SEQ) {
		return true;
	} else if (a_oCurrntLine.nxLineID == a_oPastLine.nxLineID && nCurrentSEQ != nPastSEQ + 1 ) {
		return true;
	}
...
bool KDlgImportTransitLineNode::CheckPrimaryKey(Integer a_nxPrimaryKey)
{
...
void KDlgImportTransitLineNode::ChangeStationYN(TTransitLine &a_oPastLine)
{
	std::map<int, CString> &mapCSVData = a_oPastLine.mapCSVInfo;
	mapCSVData[a_oPastLine.nStationLocation] = _T("1");
	a_oPastLine.nStationYN = 1;
}

bool KDlgImportTransitLineNode::CheckTransitNodeToLine(TTransitLine &a_oCurrntLine, TTransitLine &a_oPastLine, KODKey a_oLinkFTNode)
{
	auto iterfind = m_mapFromToLink.find(a_oLinkFTNode);
	auto iterend  = m_mapFromToLink.end();
	...
	{
		Integer nxLinkID = iterfind->second;
		CString strLinkID = _T("");{
			strLinkID.Format(_T("%I64d"), nxLinkID);
		}

		a_oCurrntLine.nxLinkID = nxLinkID;

		std::map<int, CString> &mapCSVData = a_oCurrntLine.mapCSVInfo;
		mapCSVData[a_oCurrntLine.nLinkLocation] = strLinkID;
	}

	return false;
}

void KDlgImportTransitLineNode::ChangeSEQ( TTransitLine& a_oPastLine )
{
	std::map<int, CString> &mapCSVData = a_oPastLine.mapCSVInfo;

	CString strNewSEQ = _T(""); {
		int nOldSEQ = a_oPastLine.nSeq;
		int nNewSEQ = nOldSEQ - 1;

		strNewSEQ.Format(_T("%d"), nNewSEQ);
	}

	mapCSVData[a_oPastLine.nSeqLocation] = strNewSEQ;
}

void KDlgImportTransitLineNode::FindLinkByTNode( int a_nCurrentLine, TTransitLine& a_oCurrntLine, CString& a_strLinkID, bool& a_bChkeErr, CStdioFileEx& ofErr )
{
	TxLogDebugStartMsg();

	CString strQuery(_T(""));
	CString strCntQuery(_T(""));
	CString strLinkID(_T(""));
	int nRecordCount(0);

	Integer nxTNodeID = a_oCurrntLine.nxNodeID;

	strQuery.Format(_T("select link_id from %s where tnode_id = %s order by length desc"), TABLE_LINK, nxTNodeID);
	strCntQuery.Format(_T("SELECT COUNT(*) AS CNT FROM %s where tnode_id = %s"), TABLE_LINK, nxTNodeID);

	try
	{
		KDBaseConnectionPtr spDBase  = m_pTarget->GetDBaseConnection();
		KResultSetPtr		spResult = spDBase->ExecuteQuery(strCntQuery);

		if (spResult->Next()) {
			nRecordCount = spResult->GetValueInt(0);
		}

		if (nRecordCount > 0) {
			KResultSetPtr spResult;
			spResult = spDBase->ExecuteQuery(strQuery);
			if (spResult->Next()) {
				Integer nxLink_ID = spResult->GetValueInt64(0);
				strLinkID.Format(_T("%I64d"), nxLink_ID);

				a_oCurrntLine.nxLinkID = nxLink_ID;
				std::map<int, CString> &mapCSVData = a_oCurrntLine.mapCSVInfo;
			...
			CString strErrorMsg;
			//strErrorMsg.Format(_T("Line : %d =>\t 링크에 맞지 않는 Tnode_ID = %I64d\r\n "), a_nCurrentLine, nxTNodeID);
			if (KmzSystem::GetLanguage() == KEMKorea) {
				strErrorMsg.Format(_T("Line : %d =>\t 링크와 불일치한 Tnode_ID = %I64d\r\n "), a_nCurrentLine, nxTNodeID);
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				strErrorMsg.Format(_T("Line : %d =>\t The ToNode ID %I64d does NOT match with the Link\r\n "), a_nCurrentLine, nxTNodeID);
			}
...
#include <string>
void KDlgImportTransitLineNode::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportTransitLineNode::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	try
	{
		std::vector<KIOColumn*> vecIOColumn; {
			...
			for (int i= 0; i< nRecordCount; i++) {
				...
				KIOColumn*        pIOColumn      = GetTovaIOColumn(pRecord);
				int               nSelectedIndex = GetTxtColumnIndex(pRecord);
...
void KDlgImportTransitLineNode::ImportBaseData_link(ThreadPara* pPara)
{
	TxLogDebugStartMsg();

	std::ifstream ifs(m_strMiddleFile_LinkBase, std::ios::binary);
	...
	CString strQuery(_T("")); {
		strQuery.Format(_T("delete from %s"), TABLE_TRANSITLINKS);
	}
	spDBase->ExecuteUpdate(strQuery);

	CString strPreparedQuery = GeneratePrepareQuery_link(m_setOnlyLinkColumn);
	KPreparedStatementPtr spPrepared = spDBase->PrepareStatement(strPreparedQuery);
	...
	try
	{
		std::vector<KIOColumn*> vecIOColumn; {
			...
			for (int i= 0; i< nRecordCount; i++) {
				...
				DWORD_PTR dwPtrIOColumn = (DWORD_PTR)pIOColumn;
				if (m_setOnlyLinkColumn.find(dwPtrIOColumn) == m_setOnlyLinkColumn.end()) {
					continue;
				}
...
void KDlgImportTransitLineNode::UpdateBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	CString strQuery(_T("")); {
		CString strWhere(_T(""));
		DynamicWhere(strWhere);

		CString strTableName = m_pTable->Name();
		strQuery.Format(_T("delete from %s %s "), strTableName, strWhere);
	}
	spDBase->ExecuteUpdate(strQuery);
	{
		strQuery.Format(_T("delete from %s"), TABLE_ACCESS_LINE_STATION);
...
void KDlgImportTransitLineNode::UpdateBaseData_link(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	CString strQuery(_T("")); {
		...
		strQuery.Format(_T("delete from %s %s "), TABLE_TRANSITLINKS, strWhere);
	}
	spDBase->ExecuteUpdate(strQuery);
...
void KDlgImportTransitLineNode::DynamicWhere( CString &a_strWhere )
{
	a_strWhere = _T("");

	auto iter  = m_setUpdateTransitLineID.begin();
	auto itEnd = m_setUpdateTransitLineID.end();

	int nIdx(0);
	for (; iter != itEnd; ++iter) {
		if(nIdx == 0) {
			a_strWhere.Append(_T(" WHERE transit_id IN ( "));
		} else {
			a_strWhere.Append(_T(", "));
		}

		a_strWhere.AppendFormat(_T("%I64d"), *iter);

		nIdx++;
	}

	if(a_strWhere.CompareNoCase(_T("")) != 0)
	{
		a_strWhere.Append(_T(" ) "));
	}
}

CString KDlgImportTransitLineNode::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportTransitLineNode::GeneratePrepareQuery_link( std::set<DWORD_PTR> &a_setOnlyLinkColumn )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();

	CString strPreparedQueryHeader; {
		strPreparedQueryHeader.Format(_T(" INSERT INTO %s ( "), TABLE_TRANSITLINKS);
	}
	CString strPreparedQueryValues(_T(" Values("));
	...
	for (int i= 0; i< nRecordCount; i++) {
		...
		if(0 < pColumnItem->getColumnIndex())
		{
			...
			DWORD_PTR dwPtrIOColumn = (DWORD_PTR)pColumn;
			if (a_setOnlyLinkColumn.find(dwPtrIOColumn) == a_setOnlyLinkColumn.end()) {
				continue;
			}

			CString strColumnName = pColumn->Name();
			if (strColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_ID) == 0) {
				strColumnName = COLUMN_TRANSIT_LINK_ID;
			}
			...
			strPreparedQueryHeader.Append(strColumnName);
			strPreparedQueryValues.Append(_T("?"));
...
CString KDlgImportTransitLineNode::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportTransitLineNode::ErrorFileOpen()
{
	try
	{
...
void KDlgImportTransitLineNode::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		CString strColumnName(_T(""));
#pragma region Transit_Nodes 테이블 필수입력_Filed Null 체크
		if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_TRANSIT) == true) // true: 데이터무, false : 데이터 존재
		{
			if (KmzSystem::GetLanguage() == KEMKorea) {
				ThrowException(_T("대중교통 노선운행 자료 데이터를 먼저 입력해 주세요."));
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				ThrowException(_T("Please import Urban Transit Info data first."));
			}
		}
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		...
		for (int i= 0; i < nColumnCount; i++) {
			...
			CString strMsg(_T(""));
			pIOColumn = GetTovaIOColumn(pRecord);
			strColumnName = pIOColumn->Name();

			if( strColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_ID)  == 0 || 
				strColumnName.CompareNoCase(COLUMN_TRANSIT_ID)  == 0 || 
				strColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_SEQ) == 0 ||
				strColumnName.CompareNoCase(COLUMN_TRANSIT_NODE_STATION_YN) == 0) {
...
#pragma endregion Transit_Nodes 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportTransitLineNode::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportTransitLineNode::GetTovaIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportTransitLineNode::GetTxtColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportTransitLineNode::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnView();
}

void KDlgImportTransitLineNode::RelationTableNotify()
{
	TxLogDebugStartMsg();

	/*Transit_Nodes Notify*/
	try
	{
		m_pTable->Notify();
	} catch (int& ex) {
	...
	/*Transit_Links Notify*/
	try
	{
		KIOTables* pIOTables = m_pTarget->Tables();
		KIOTable*  pIOTable  = pIOTables->FindTable(TABLE_TRANSITLINKS);
...
void KDlgImportTransitLineNode::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportTransitLineNode::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnView();
...
void KDlgImportTransitLineNode::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnView();
...
void KDlgImportTransitLineNode::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnView();
