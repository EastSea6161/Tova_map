// KInspectionNodeDlg.cpp : 구현 파일입니다.
//
...
// KInspectionNodeDlg 대화 상자입니다.

IMPLEMENT_DYNAMIC(KInspectionNodeDlg, KResizeDialogEx)

KInspectionNodeDlg::KInspectionNodeDlg(CWnd* pParent /*=NULL*/)
	: KResizeDialogEx(KInspectionNodeDlg::IDD, pParent)
    , m_chkDataInspection(TRUE)
    , m_chkCodeInspection(TRUE)
    , m_chkZoneMatchingInspection(TRUE)
    , m_chkIsolatedNode(TRUE)
{

}

KInspectionNodeDlg::~KInspectionNodeDlg()
{
}

void KInspectionNodeDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK3, m_chkZoneMatchingInspection);
    DDX_Check(pDX, IDC_CHECK4, m_chkIsolatedNode);
}


BEGIN_MESSAGE_MAP(KInspectionNodeDlg, KResizeDialogEx)
    ON_NOTIFY(NM_CLICK, IDC_REPORT, OnReportItemClick)
    ON_BN_CLICKED(IDOK, &KInspectionNodeDlg::OnBnClickedOk)
    ON_BN_CLICKED(IDCANCEL, &KInspectionNodeDlg::OnBnClickedCancel)
END_MESSAGE_MAP()


// KInspectionNodeDlg 메시지 처리기입니다.
void KInspectionNodeDlg::OnReportItemClick(NMHDR* pNotifyStruct, LRESULT* pResult)
{
    try
    {
...
void KInspectionNodeDlg::InitReportHeader()
{
    ...
    pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(2, _T("Column"), 15));
    pColumn->SetHeaderAlignment(DT_CENTER);

    pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(3, _T("Type"), 15));
    pColumn->SetHeaderAlignment(DT_CENTER);
...
void KInspectionNodeDlg::InitReportData(std::vector<InspectionResultOneKey>& results)
{
...
BOOL KInspectionNodeDlg::OnInitDialog()
{
...
void KInspectionNodeDlg::OnBnClickedOk()
{
    ...
    KIOTable* pNodeTable = pTables->FindTable(_T("node"));
    if(NULL == pNodeTable)
    {
        AfxMessageBox(_T("노드 정보를 찾을 수 없습니다."));
        return;
    }
    ...
    {
        KDBaseInspection::CheckNodeNullData(pNodeTable, results);
    }

    if (m_chkCodeInspection)
    {
        KDBaseInspection::CheckNodeCodeData(pNodeTable, results);
    }

    if (m_chkZoneMatchingInspection)
    {
        KDBaseInspection::CheckNodeZoneMatching(pNodeTable, results);
    }

    if (m_chkIsolatedNode)
    {
        KDBaseInspection::CheckNodeIsolated(pNodeTable, results);
    }
...
void KInspectionNodeDlg::OnBnClickedCancel()
{
