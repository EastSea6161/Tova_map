// ThemeLinkValueScale.cpp : 구현 파일입니다.
//
...
IMPLEMENT_DYNAMIC(KThemeLinkValueScale, KDialogEx)

KThemeLinkValueScale::KThemeLinkValueScale(CWnd* pParent /*=NULL*/)
	: KDialogEx(KThemeLinkValueScale::IDD, pParent)
...
KThemeLinkValueScale::~KThemeLinkValueScale()
{
...
void KThemeLinkValueScale::DoDataExchange(CDataExchange* pDX)
{
	KDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO_FIELD,   m_cboCategory);
	DDX_Control(pDX, IDC_BUTTON_MARKER_COLOR, m_btnDefaultSymbolColorPicker);
	DDX_Control(pDX, IDC_BUTTON_MARKER_COLOR_OTHER, m_btnOtherSymbolColorPicker);
}


BEGIN_MESSAGE_MAP(KThemeLinkValueScale, KDialogEx)
	ON_WM_CTLCOLOR()	
	ON_WM_SIZE()
	ON_CBN_SELCHANGE(IDC_COMBO_FIELD, &KThemeLinkValueScale::OnCbnSelchangeComboField)	

	ON_BN_CLICKED(IDC_CHECK1,       &KThemeLinkValueScale::OnBnClickedCheckAllOtherValues)
	ON_BN_CLICKED(IDC_BTN_FILTER1,   &KThemeLinkValueScale::OnBnClickedBtnFilter)
	ON_BN_CLICKED(IDC_CHECK_FILTER, &KThemeLinkValueScale::OnBnClickedFilter)
	
	ON_BN_CLICKED(IDC_BUTTON1, &KThemeLinkValueScale::OnBnClickedButtonFormula)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO1, IDC_RADIO2, &KThemeLinkValueScale::OnBnClickedRadioDrawData)
END_MESSAGE_MAP()


void KThemeLinkValueScale::NotifyProcess( LPCTSTR a_strSubjectName, Integer a_nxObjectID )
{
    try
    {
...
BOOL KThemeLinkValueScale::OnInitDialog()
{
	...
	SetDlgItemText(IDC_EDIT_LOW_SIZE,  _T("2.0"));
	SetDlgItemText(IDC_EDIT_HIGH_SIZE, _T("15.0"));
	SetDlgItemText(IDC_EDIT_OUT_SIZE,  _T("2.0"));

	CheckDlgButton(IDC_CHECK_DRAWNODE, BST_CHECKED);
    CheckDlgButton(IDC_CHECK1, BST_CHECKED);

	m_btnDefaultSymbolColorPicker.SetColor( RGB(77,132,194) );
	m_btnDefaultSymbolColorPicker.SetWindowText( _T("") );
	m_btnDefaultSymbolColorPicker.ShowText( TRUE );
	m_btnDefaultSymbolColorPicker.ModifyCPStyle( 0, CPS_XT_NOFILL | CPS_XT_EXTENDED | CPS_XT_MORECOLORS | CPS_XT_SHOW3DSELECTION | CPS_XT_SHOWHEXVALUE );
...
HBRUSH KThemeLinkValueScale::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void KThemeLinkValueScale::OnSize(UINT nType, int cx, int cy)
{
	KDialogEx::OnSize(nType, cx, cy);
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
}

void KThemeLinkValueScale::SetTargetMap(KTarget* a_pTarget, KMapView* a_pMapView)
{
	m_pTarget       = a_pTarget;
...
void KThemeLinkValueScale::SetLinkTable(KIOTable* a_pLinkTable)
{
	m_pLinkTable    = a_pLinkTable;
}


void KThemeLinkValueScale::GetCategoryField(std::vector<KIOColumn*> &a_vecColumnCategory)
{
...
void KThemeLinkValueScale::LoadCategoryFieldRecord(std::vector<KIOColumn*> &a_vecColumnCategory)
{
	try
	{
...
void KThemeLinkValueScale::LoadInitialCategory(std::vector<KIOColumn*> &a_vecColumnCategory)
{
...
void KThemeLinkValueScale::InitExpressionChecker(std::vector<KIOColumn*> &a_vecColumnCategory)
{
	try
	{
		if (nullptr != m_pExpressionChecker)
...
void KThemeLinkValueScale::OnCbnSelchangeComboField()
{
	try
	{
...
void KThemeLinkValueScale::FieldSelectChangeAction()
{
	int nCurSelectedIndex = m_cboCategory.GetCurSel();
	if (nCurSelectedIndex < 0)
		return;
...
void KThemeLinkValueScale::SetEditControlMinMax( double a_dMinVal, double a_dMaxVal )
{
	CString strTemp(_T(""));
...
void KThemeLinkValueScale::ClearLinkThemes()
{
	m_pMapView->ClearLinkThemes(false);
}


void KThemeLinkValueScale::Draw(bool a_bLabel)
{
    bool bResult(false);
    bResult = InvalidateInput();
    ...
    else 
    {
        ...
        if (GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2) == IDC_RADIO1) {
            GetDrawDataByNormal(oTThemeResultSettingInfo, vecOutDrawLinkInfo);
        }
        else {
            GetDrawDataByCalculate(oTThemeResultSettingInfo, vecOutDrawLinkInfo);
        }

        m_pMapView->DrawLinkThemes(vecOutDrawLinkInfo, a_bLabel, true);
    }
}


bool KThemeLinkValueScale::InvalidateInput()
{
	try
	{
		if (GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2) == IDC_RADIO1)
...
void KThemeLinkValueScale::GetDrawDataByNormal(TThemeResultSettingInfo &a_oTThemeResultSettingInfo, std::vector<TDrawLinkInfo> &a_vecOutDrawLinkInfo)
{
	try
	{
		int nCurSelectedIndex = m_cboCategory.GetCurSel();
		...
		COLORREF clrDefault = m_btnDefaultSymbolColorPicker.GetColor();
		...
		CString strSQL;
		strSQL.Format(_T("Select link_id, %s From %s %s"), pIOColumn->Name(), TABLE_LINK, strQueryFilter);
		...
		while(spResult->Next())
		{
			...
			double  dValue   = spResult->GetValueDouble(1);
			double  dWidth   = 0.0;

			bool    bOutRange(false);
			CString strLabel(_T(""));

			if (!Calculate2Width(bChecked, bIncludeZero, a_oTThemeResultSettingInfo, dValue, bOutRange, dWidth, strLabel))
			{
				continue;
			}
			...
			if (bOutRange == false)
			{
				...
				oTDrawLinkInfo.TColorRGB = clrDefault;
				vecDrawLinkInfo.push_back(oTDrawLinkInfo);
			}
			else
			{
				oTDrawLinkInfo.TWidth    = a_oTThemeResultSettingInfo.dOutSize;
...
// 				{
// 					TDrawLinkInfo    oTDrawLinkInfo;
// 					oTDrawLinkInfo.TLinkID = spResult->GetValueInt64(0);
// 					oTDrawLinkInfo.TWidth  = a_oTThemeResultSettingInfo.dOutSize;
// 					oTDrawLinkInfo.TLabel    = _T("");
// 					oTDrawLinkInfo.TColorRGB = clrOther;
// 					a_vecOutDrawLinkInfo.push_back(oTDrawLinkInfo);
...
void KThemeLinkValueScale::GetDrawDataByCalculate(TThemeResultSettingInfo &a_oTThemeResultSettingInfo, std::vector<TDrawLinkInfo> &a_vecOutDrawLinkInfo)
{
	try
	{
...
// 					oTDrawLinkInfo.TWidth  = a_oTThemeResultSettingInfo.dOutSize;
// 					oTDrawLinkInfo.TLabel    = _T("");
// 					oTDrawLinkInfo.TColorRGB = a_oTThemeResultSettingInfo.clrOther;
// 					a_vecOutDrawLinkInfo.push_back(oTDrawLinkInfo);
				}
			}
			else
			{
				...
				if (bOutRange == false)
				{
					oTDrawLinkInfo.TWidth  = dWidth;	
					oTDrawLinkInfo.TLabel  = strLabel;
					oTDrawLinkInfo.TColorRGB = a_oTThemeResultSettingInfo.clrDefault;
					vecDrawLinkInfo.push_back(oTDrawLinkInfo);
				}
				else
				{
...
void KThemeLinkValueScale::GetThemeResultSettingInfo(TThemeResultSettingInfo &a_oTThemeResultSettingInfo)
{
	try
	{
		CString strTemp(_T(""));
		...
		a_oTThemeResultSettingInfo.clrDefault = m_btnDefaultSymbolColorPicker.GetColor();
		a_oTThemeResultSettingInfo.clrOther   = m_btnOtherSymbolColorPicker.GetColor();
	}
	catch (...)
	{
		TxLogDebugException();	
...
bool KThemeLinkValueScale::Calculate2Width(BOOL a_bChecked, BOOL a_bIncludeZero, TThemeResultSettingInfo &a_oTThemeResultSettingInfo, double a_dValue, bool &a_bOutRange, double &a_dWidth, CString &a_strLabel)
{
	try
	{
		...
		{	
			...
			{
				a_dWidth = (dHighSize-dLowSize) / (dHighValue-dLowValue) * (a_dValue-dLowValue) + dLowSize;			
			}
		}
		else
		{	
		...
		if (true == a_bOutRange)
		{
			if (a_bChecked == TRUE)
			{
				a_dWidth = a_oTThemeResultSettingInfo.dOutSize;
			}
			else
			{
				return false;
...
void KThemeLinkValueScale::OnBnClickedCheckAllOtherValues()
{
	ControlCheckOutRange();
}


void KThemeLinkValueScale::ControlCheckOutRange()
{
	try
	{
		if (IsDlgButtonChecked(IDC_CHECK1) == BST_CHECKED)
...
bool KThemeLinkValueScale::Descending( TDrawLinkInfo left, TDrawLinkInfo right )
{
	return left.TWidth > right.TWidth;
}

void KThemeLinkValueScale::InitFilterData( void )
{
	m_vecFilterInfo.clear();
...
void KThemeLinkValueScale::OnBnClickedBtnFilter()
{
...
void KThemeLinkValueScale::OnBnClickedFilter()
{
...
BOOL KThemeLinkValueScale::PreTranslateMessage( MSG* pMsg )
{
...
void KThemeLinkValueScale::UpdateTxtFilter( void )
{
	//update idc_edit1;
...
void KThemeLinkValueScale::GenFilter(std::set<int> &a_setCheckType)
{
	try
	{
		if (m_bCheckedFilter)
...
CString KThemeLinkValueScale::GenQueryFilter( void )
{
...
CString KThemeLinkValueScale::GenNotQueryFilter( void )
{
    CString strQueryFilter(_T(""));
...
void KThemeLinkValueScale::OnBnClickedButtonFormula()
{
	try
	{
...
unsigned __stdcall KThemeLinkValueScale::CalculateResultThreadCaller(void* p)
{
	...
	KThemeLinkValueScale* pDlg = (KThemeLinkValueScale*)pParameter->GetParameter();
...
bool KThemeLinkValueScale::CalculateResult()
{
	try
	{
		m_mapCalculateResult.clear();
...
void KThemeLinkValueScale::OnBnClickedRadioDrawData( UINT nID )
{
	ControlRadioDrawData();
}


void KThemeLinkValueScale::ControlRadioDrawData()
{
	try
	{
		if (GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2) == IDC_RADIO1)
...
void KThemeLinkValueScale::NodeLayerSettingChanged()
{
}
