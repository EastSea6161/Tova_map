// LinkValueClassify.cpp : 구현 파일입니다.
//
...
// KLinkValueClassify 대화 상자입니다.

IMPLEMENT_DYNAMIC(KThemeLinkValueClassify, KDialogEx)

KThemeLinkValueClassify::KThemeLinkValueClassify(CWnd* pParent /*=NULL*/)
	: KDialogEx(KThemeLinkValueClassify::IDD, pParent), ISymbologyThemeLink()
	, m_bCheckedFilter(FALSE)
	, m_nRealDigit(0)
	, m_dMinCalculateVal(0.0)
	, m_dMaxCalculateVal(0.0)
{
	m_clrSelectedLineColor = RGB(167, 76, 178);
...
KThemeLinkValueClassify::~KThemeLinkValueClassify()
{
...
void KThemeLinkValueClassify::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KThemeLinkValueClassify, KDialogEx)
	ON_WM_CTLCOLOR()	
	ON_WM_SIZE()
	ON_WM_DRAWITEM()
	ON_BN_CLICKED(IDC_BUTTON_MARKER, &KThemeLinkValueClassify::OnBnClickedButtonMarker)	
	ON_BN_CLICKED(IDC_RADIO1, &KThemeLinkValueClassify::OnBnClickedRadio)
	ON_BN_CLICKED(IDC_RADIO2, &KThemeLinkValueClassify::OnBnClickedRadio)
	ON_CBN_SELCHANGE(IDC_COMBO_FIELD, &KThemeLinkValueClassify::OnCbnSelchangeComboField)
	ON_CBN_SELCHANGE(IDC_COMBO_STYLE, &KThemeLinkValueClassify::OnCbnSelchangeComboStyle)	
	ON_CBN_SELCHANGE(IDC_COMBO_CLASS, &KThemeLinkValueClassify::OnCbnSelchangeComboClass)
	ON_BN_CLICKED(IDC_CHECK1, &KThemeLinkValueClassify::OnBnClickedCheck1)

	ON_NOTIFY(NM_CLICK, IDC_REPORT,   &KThemeLinkValueClassify::OnReportItemClick)
	ON_NOTIFY(XTP_NM_REPORT_VALUECHANGED, IDC_REPORT, &KThemeLinkValueClassify::OnReportItemValueChanged)
	//XTP_NM_REPORT_SELCHANGED

	ON_CBN_SELCHANGE(IDC_COMBO_DIGIT, &KThemeLinkValueClassify::OnCbnSelchangeComboDigit)
	ON_BN_CLICKED(IDC_BUTTON_STYLE_SAVE, &KThemeLinkValueClassify::OnBnClickedButtonStyleSave)
	ON_BN_CLICKED(IDC_BUTTON_STYLE_EDIT, &KThemeLinkValueClassify::OnBnClickedButtonStyleEdit)

	ON_BN_CLICKED(IDC_BTN_FILTER1, &KThemeLinkValueClassify::OnBnClickedBtnFilter)
	ON_BN_CLICKED(IDC_CHECK_FILTER, &KThemeLinkValueClassify::OnBnClickedFilter)

	ON_BN_CLICKED(IDC_BUTTON1, &KThemeLinkValueClassify::OnBnClickedButtonFormula)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO3, IDC_RADIO4, &KThemeLinkValueClassify::OnBnClickedRadioDrawData)
	ON_BN_CLICKED(IDC_CHECK3, &KThemeLinkValueClassify::OnBnClickedCheckAllOtherValues)
	ON_BN_CLICKED(IDC_CHECK_ALL, &KThemeLinkValueClassify::OnBnClickedCheckAll)
	ON_NOTIFY(XTP_NM_REPORT_CHECKED, IDC_REPORT, &KThemeLinkValueClassify::OnReportCheckItem)
END_MESSAGE_MAP()


void KThemeLinkValueClassify::NotifyProcess( LPCTSTR a_strSubjectName, Integer a_nxObjectID )
{
    try
    {
...
// KLinkValueClassify 메시지 처리기입니다.
BOOL KThemeLinkValueClassify::OnInitDialog()
{
	...
	m_btnLineSymbol.SetFont(&m_oQbicDefaultMarkerFont);
	CString strSymbol;
	strSymbol.AppendChar(m_MarkerIntex);  
	SetDlgItemText(IDC_BUTTON_MARKER, strSymbol);

	CheckDlgButton(IDC_CHECK_DRAWNODE, BST_CHECKED);
	...
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(1, _T("Color"), 20));
    if (KmzSystem::GetLanguage() == KEMKorea) {
        pColumn->SetCaption(_T("색상"));
    }
	pColumn->SetHeaderAlignment(DT_CENTER);
	
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(2, _T("Size"),  25));
	pColumn->SetHeaderAlignment(DT_CENTER);
	...
	pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(3, _T("<=  Range  <"),  55));
	pColumn->SetHeaderAlignment(DT_CENTER);
    if (KmzSystem::GetLanguage() == KEMKorea) {
	...
	SetDlgItemText(IDC_EDIT_OUT_SIZE,  _T("2.0"));

	return TRUE;  // return TRUE unless you set the focus to a control
	// 예외: OCX 속성 페이지는 FALSE를 반환해야 합니다.
}

HBRUSH KThemeLinkValueClassify::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void KThemeLinkValueClassify::OnSize(UINT nType, int cx, int cy)
{
	KDialogEx::OnSize(nType, cx, cy);
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
}

void KThemeLinkValueClassify::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void KThemeLinkValueClassify::SetTargetMap(KTarget* a_pTarget, KMapView* a_pMapView)
{
	m_pTarget       = a_pTarget;
...
void KThemeLinkValueClassify::SetLinkTable(KIOTable* a_pLinkTable)
{
	m_pLinkTable    = a_pLinkTable;
}


void KThemeLinkValueClassify::LoadInitialData()
{
...
void KThemeLinkValueClassify::GetCategoryField(std::vector<KIOColumn*> &a_vecColumnCategory)
{
...
void KThemeLinkValueClassify::InitExpressionChecker(std::vector<KIOColumn*> &a_vecColumnCategory)
{
	try
	{
		if (nullptr != m_pExpressionChecker)
...
void KThemeLinkValueClassify::LoadCategoryFieldRecord(std::vector<KIOColumn*> &a_vecColumnCategory)
{
	try
	{
		...
		KBulkDBase::BulkLinkSelect(pIOTable, oIOColumnsCategory, *m_pCategoryFieldRecord);
	}
	catch (...)
	{
		TxLogDebugException();	
...
void KThemeLinkValueClassify::LoadInitialCategory(std::vector<KIOColumn*> &a_vecColumnCategory)
{
...
void KThemeLinkValueClassify::LoadInitialStyle()
{
	...
	if (spDBaseConnection != NULL)
	{
		...
		KProjectDBase::LinkTemplate(spDBaseConnection, 1, vecLinkTemplate);

		int nPreSelectedIndex = -1;
		for (size_t i=0; i<vecLinkTemplate.size(); i++)
		{
			TLinkTemplate oTemplate = vecLinkTemplate[i];
...
void KThemeLinkValueClassify::OnBnClickedRadio()
{
	StyleRadioSelectChangeAction();
	LoadReportCtrlData();
}

void KThemeLinkValueClassify::StyleRadioSelectChangeAction()
{
	if (m_cboStyle.GetCount() == 0)
	{		
...
void KThemeLinkValueClassify::OnCbnSelchangeComboField()
{
...
void KThemeLinkValueClassify::FieldSelectChangeAction()
{
	try
	{
		int nCurSelectedIndex = m_cboCategory.GetCurSel();
...
void KThemeLinkValueClassify::OnCbnSelchangeComboStyle()
{
...
void KThemeLinkValueClassify::StyleSelectChangeAction()
{
	LoadReportCtrlData();
}

void KThemeLinkValueClassify::OnCbnSelchangeComboClass()
{
	LoadReportCtrlData();
}

void KThemeLinkValueClassify::LoadInitialClass()
{
...
void KThemeLinkValueClassify::OnBnClickedCheck1()
{
	LoadReportCtrlData(false);
}

void KThemeLinkValueClassify::LoadInitialDigit()
{
...
void KThemeLinkValueClassify::OnCbnSelchangeComboDigit()
{
	LoadReportCtrlData(false);
}

void KThemeLinkValueClassify::OnBnClickedButtonMarker()
{
	...
	if ( IDOK == dlg.DoModal() )
	{
		...
		for (size_t i= 0; i< nxCount; i++)
		{
			...
			pItem->SetTextColor(m_clrSelectedLineColor);
		}
		if (nxCount > 0)
		{
...
void KThemeLinkValueClassify::OnReportItemClick( NMHDR* pNotifyStruct, LRESULT* pResult )
{
...
void KThemeLinkValueClassify::OnReportItemValueChanged( NMHDR* pNotifyStruct, LRESULT* pResult )
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
...
void KThemeLinkValueClassify::RecalRange()
{
	int nDisplayDecimalPoint = GetDisplayDecimalPoint(m_nRealDigit);
...
void KThemeLinkValueClassify::LoadReportCtrlData(bool a_bStlyeReset)
{
	try
	{
		bool bDrawned = true;
...
void KThemeLinkValueClassify::LoadSavedStyle(bool a_bStlyeReset)
{
	int nCurSelectedFieldIndex = m_cboCategory.GetCurSel();
	if (nCurSelectedFieldIndex < 0)
	{
	...
	std::vector<TLinkTemplateValue> vecTemplateValue;
	KProjectDBase::LinkTemplateValue(spDBaseConnection, nStyleID, vecTemplateValue);
...
void KThemeLinkValueClassify::ResetSavedStlye(std::vector<TLinkTemplateValue> &a_vecTemplateValue)
{
	try
	{
		BOOL bCheck(FALSE);
		...
		for (size_t i=0; i<nxCount; i++)
		{
			TLinkTemplateValue& oTemplateValue = a_vecTemplateValue[i];
			...
			// Size
			pItem = pRecord->AddItem(new CXTPReportRecordItemNumber(oTemplateValue.TWidth, _T("%.1f")));         
...
void KThemeLinkValueClassify::RefreshOnlySavedRange(std::vector<TLinkTemplateValue> &a_vecTemplateValue)
{
	try
	{
		CXTPReportRecords*        pRecords = m_wndReportCtrl.GetRecords();
...
void KThemeLinkValueClassify::LoadUserStyle(bool a_bStlyeReset)
{
	try
	{
		KIOColumn* pIOColumn = nullptr;
		...
		if (GetCheckedRadioButton(IDC_RADIO3, IDC_RADIO4) == IDC_RADIO3)
		{
			...
			//KBulkDBaseLink::AllLinkValue(m_pTarget, pIOColumn->Name(), vecData);
			KBulkDBaseLink::SrchLinkValue(m_pTarget, pIOColumn->Name(), vecData, strQueryFilter);
		}
		else
		{
...
void KThemeLinkValueClassify::GetDataByCalculate(std::vector<double> &a_vecData)
{
	try
	{
		a_vecData.clear();
...
void KThemeLinkValueClassify::GetDrawDataByCalculate( std::vector<TLinkIDValue> &ar_vecData, std::vector<TLinkIDValue> &ar_vecOutFilterData )
{
	try
	{
		ar_vecData.clear();
		...
		while (iter != itEnd)
		{
			...
			{    
				int     nLinkType(-1);
				AutoType itFind = m_mapLinkIdType.find(nxLinkID);
				if (itFind != m_mapLinkIdType.end())
					nLinkType = itFind->second;

				if (setFilterCheckType.find(nLinkType) != setFilterCheckType.end())
					bNoneData = false;
				else
					bNoneData = true;
			}
			...
			oTLinkIDValue.TLinkID = nxLinkID;
			oTLinkIDValue.TValue  = dValue;

			if (!bNoneData)
			{
				ar_vecData.push_back(oTLinkIDValue);
			}
			else
			{
				ar_vecOutFilterData.push_back(oTLinkIDValue);
			}
...
void KThemeLinkValueClassify::GetMinDecreaseMaxIncrease(int a_nDecimalPoint, double &a_dMinDecrease, double &a_dMaxIncrease)
{
	try
	{
		a_dMinDecrease = 0.5;
...
int KThemeLinkValueClassify::GetDisplayDecimalPoint(int a_nRealDigit)
{
	int nDecimalPoint(1);
...
void KThemeLinkValueClassify::ResetUserStyle(std::vector<TDevideDataInterval> &a_ovecDevideInfo)
{
	try
	{
		BOOL bCheck(FALSE);
		...
		for ( size_t i=0; i< nxCount; i++ )
		{
			...
			// Size
			pItem = pRecord->AddItem(new CXTPReportRecordItemNumber(1.0, _T("%.1f")));         
			pItem->SetEditable(TRUE);
			pItem->SetAlignment(xtpColumnTextCenter);
...
// 			CString strMax0 = QBicStringFormat::FormatString(-100.04, 1);
// 			CString strMax1 = QBicStringFormat::FormatString(-100.05, 1);
// 			CString strMin2 =  QBicStringFormat::FormatString(-100.06, 1);
// 			CString strMin3 =  QBicStringFormat::FormatString(-100.051, 1);
...
void KThemeLinkValueClassify::RefreshOnlyUserRange(std::vector<TDevideDataInterval> &a_ovecDevideInfo)
{
	try
	{
		double dMinDecrease(0.0);
...
void KThemeLinkValueClassify::Draw(bool a_bLabel)
{
    std::vector<TDrawLinkInfo> vecOutDrawLinkInfo;
    bool bResult = InvalidateInput();
	if (bResult)
    {
		MakeDrawData(vecOutDrawLinkInfo);
    }

    // 과거의 지도 상태
    bool bDrawned = m_pMapView->DrawingLinkThemeState();
    ...
    {
        Clear();
        m_pMapView->DrawLinkThemes(vecOutDrawLinkInfo, a_bLabel, false);
//         if(IsDlgButtonChecked(IDC_CHECK_DRAWNODE) == BST_CHECKED) {
//             DrawNode();
//         }      
...
bool KThemeLinkValueClassify::InvalidateInput()
{
	try
	{
		if (GetCheckedRadioButton(IDC_RADIO3, IDC_RADIO4) == IDC_RADIO3)
...
void KThemeLinkValueClassify::MakeDrawData(std::vector<TDrawLinkInfo> &a_vecOutDrawLinkInfo)
{
	try
	{
		a_vecOutDrawLinkInfo.clear();
...
#pragma endregion vecTMinMax - 데이터 삽입
		...
		std::vector<TLinkIDValue> vecData;
		std::vector<TLinkIDValue> vecNotFilterData;

		if (GetCheckedRadioButton(IDC_RADIO3, IDC_RADIO4) == IDC_RADIO3)
		{
			CString strQueryFilter = GenQueryFilter(); //genereate where절.
			//KBulkDBaseLink::AllLinkValue(m_pTarget, pIOColumn->Name(), vecData);
			KBulkDBaseLink::SrchLinkValue(m_pTarget, pIOColumn->Name(), vecData, strQueryFilter);
			...
			{      
				...
				KBulkDBaseLink::SrchLinkValue(m_pTarget, pIOColumn->Name(), vecNotFilterData, strQueryFilter);
			}
		}
		else
		{
		...
		size_t nxLinkCount    = vecData.size();
		...
		{
			TLinkIDValue& oTLinkIDValue = vecData[i];
			oTDrawLinkInfo.TLinkID = oTLinkIDValue.TLinkID;

			double dValue = oTLinkIDValue.TValue;

			bool bOutRange(false);
#pragma region oTDrawLinkInfo - Size, color, label 구하기
			...
			else
			{
				...
				for (size_t k=0; k<nxTMinMaxCount; k++)
				{
					...
					if ( KEMThemeRangeIndexFirst == oTMinMax.emRangeIndex )
					{
						...
						{
							oTDrawLinkInfo.TColorRGB = oTMinMax.TColorRGB;
							oTDrawLinkInfo.TWidth    = oTMinMax.TSize;    
			...
			if (bOutRange)
			{
				...
				oTDrawLinkInfo.TColorRGB = clrOther;
				oTDrawLinkInfo.TWidth    = dOutSize;  
			}
...
#pragma endregion oTDrawLinkInfo - Size, color, label 구하기
			...
			{
				a_vecOutDrawLinkInfo.push_back(oTDrawLinkInfo);
			}
		}
...
// 				{
// 					TLinkIDValue& oLinkIDValue = vecNotFilterData[i];
// 					TDrawLinkInfo oTDrawLinkInfo;
// 					oTDrawLinkInfo.TLinkID   = oLinkIDValue.TLinkID;
// 					oTDrawLinkInfo.TWidth    = dOutSize;
// 					oTDrawLinkInfo.TLabel    = _T("");
// 					oTDrawLinkInfo.TColorRGB = clrOther;
// 					a_vecOutDrawLinkInfo.push_back(oTDrawLinkInfo);
// 				}
			}
		...
		std::sort(vecDrawLinkInfo.begin(), vecDrawLinkInfo.end(), Descending);
		for (size_t i=0; i<vecDrawLinkInfo.size(); i++)
		{
			a_vecOutDrawLinkInfo.push_back(vecDrawLinkInfo[i]);
		}
	}
	catch (...)
	{
...
void KThemeLinkValueClassify::OnBnClickedButtonStyleSave()
{
	KDBaseConPtr spDBaseConnection = KDBase::GetProjectDBConnection(); //KDBase::GetApplicationDBConnection();
	if (spDBaseConnection == NULL)
	{		
	...
	try
	{
		...
		if (!bOverwrite)
		{
			...
			nID = KProjectDBase::NewLinkTemplateID(spDBaseConnection, 1);	
			strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplate(ID,Type,Name) Values('%d','%d','%s')"), nID,1,strNewStyleName.Trim());
		...
		for (int i=0; i<nRecordCount; i++)
		{
			...
			//strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplateValue(ID,Seq,LineType,Width, Min, Max, RGB) Values('%d','%d','%d','%.1f','%.1f','%.1f','%s')"), nID, i, nSymbol, dSize, dMin, dMax, strRGB);
			strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplateValue(ID,Seq,LineType,Width, Min, Max, RGB) Values('%d','%d','%d','%.1f','%s','%s','%s')"), nID, i, nSymbol, dSize, vecValue[0], vecValue[1], strRGB);
...
void KThemeLinkValueClassify::OnBnClickedButtonStyleEdit()
{
	KLineStyleValueEdit dlg;
	dlg.DoModal();
...
bool KThemeLinkValueClassify::Descending( TDrawLinkInfo left, TDrawLinkInfo right )
{
	return left.TWidth > right.TWidth;
}

BOOL KThemeLinkValueClassify::PreTranslateMessage( MSG* pMsg )
{
...
void KThemeLinkValueClassify::OnBnClickedBtnFilter()
{
	KThemeSetFilterDlg oThemeSetFilterDlg(m_pTarget, m_vecFilterInfo, USER_LINKCODE_GROUP_KEY, SYSTEM_LINKCODE_GROUP_KEY);
...
void KThemeLinkValueClassify::OnBnClickedFilter()
{
...
void KThemeLinkValueClassify::InitFilterData( void )
{
	m_vecFilterInfo.clear();

	vector<int>   vecTypeLink;
	...
	try
	{
		...
		//link테이블에서 사용하는 link_type list를 조회
		strQuery.Format(_T("select %s from %s group by %s order by %s asc"), 
			COLUMN_LINK_TYPE, TABLE_LINK, COLUMN_LINK_TYPE, COLUMN_LINK_TYPE);

		KResultSetPtr spResultSet = spDBaseConnection->ExecuteQuery(strQuery);

		while (spResultSet->Next())
		{
			int nTypeLink = spResultSet->GetValueInt(0);
			vecTypeLink.push_back(nTypeLink);
		}

		//define_system_code테이블에서 user_link_tp과, System_link_tp의 매칭코드를 조회
...
void KThemeLinkValueClassify::GenFilter(std::set<int> &a_setCheckType)
{
	try
	{
		if (m_bCheckedFilter)
...
CString KThemeLinkValueClassify::GenQueryFilter( void )
{
	CString strQueryFilter(_T(""));
	...
	if (m_bCheckedFilter)
	{
		...
		strQueryFilter.Format(_T(" where %s in ( "), COLUMN_LINK_TYPE);
...
CString KThemeLinkValueClassify::GenNotQueryFilter( void )
{
    CString strQueryFilter(_T(""));
    ...
    if (m_bCheckedFilter)
    {
        ...
        strQueryFilter.Format(_T(" where %s not in ( "), COLUMN_LINK_TYPE);
        strQueryFilter.Append(strQuery2);
        strQueryFilter.Append(_T(" ) "));
    }
...
void KThemeLinkValueClassify::UpdateTxtFilter( void )
{
	//update idc_edit1;
...
void KThemeLinkValueClassify::OnBnClickedButtonFormula()
{
	try
	{
...
unsigned __stdcall KThemeLinkValueClassify::CalculateResultThreadCaller(void* p)
{
	...
	KThemeLinkValueClassify* pDlg = (KThemeLinkValueClassify*)pParameter->GetParameter();
...
bool KThemeLinkValueClassify::CalculateResult()
{
	try
	{
		m_mapCalculateResult.clear();
		...
		if (m_mapLinkIdType.size() < 1)
		{
			KBulkDBaseLink::AllLinkIdType( m_pTarget->GetDBaseConnection(), m_mapLinkIdType );
		}
...
void KThemeLinkValueClassify::SetEditControlMinMax( double a_dMinVal, double a_dMaxVal )
{
	CString strTemp(_T(""));
...
void KThemeLinkValueClassify::OnBnClickedRadioDrawData( UINT nID )
{
	ControlRadioDrawData();
	LoadReportCtrlData(false);
}
...
void KThemeLinkValueClassify::ControlRadioDrawData()
{
	try
	{
		if (GetCheckedRadioButton(IDC_RADIO3, IDC_RADIO4) == IDC_RADIO3)
...
void KThemeLinkValueClassify::NodeLayerSettingChanged()
{
}


void KThemeLinkValueClassify::OnBnClickedCheckAllOtherValues()
{
	ControlCheckOutRange();
}


void KThemeLinkValueClassify::ControlCheckOutRange()
{
	try
	{
		if (IsDlgButtonChecked(IDC_CHECK3) == BST_CHECKED)
...
void KThemeLinkValueClassify::OnBnClickedCheckAll()
{
	ControlCheckAll();
}


void KThemeLinkValueClassify::ControlCheckAll()
{
	try
	{
		BOOL bCheck(FALSE);
...
void KThemeLinkValueClassify::OnReportCheckItem(NMHDR* pNotifyStruct, LRESULT* pResult)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*)pNotifyStruct;
