// InterRegionalChoiceParaSubPage.cpp : 구현 파일입니다.
//
...
IMPLEMENT_DYNAMIC(KInterRegionalChoiceParaSubPage, KDialogEx)

KInterRegionalChoiceParaSubPage::KInterRegionalChoiceParaSubPage(CWnd* pParent /*=NULL*/)
	: KDialogEx(KInterRegionalChoiceParaSubPage::IDD, pParent)
{

}

KInterRegionalChoiceParaSubPage::~KInterRegionalChoiceParaSubPage()
{
}

void KInterRegionalChoiceParaSubPage::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KInterRegionalChoiceParaSubPage, KDialogEx)
	ON_WM_SIZE()
END_MESSAGE_MAP()


// KInterRegionalChoiceParaSubPage 메시지 처리기입니다.


BOOL KInterRegionalChoiceParaSubPage::OnInitDialog()
{
	...
	InitReportHeader();
	UpdateReportData(m_mapModeRegionalChoiceValue);
...
void KInterRegionalChoiceParaSubPage::OnSize(UINT nType, int cx, int cy)
{
...
void KInterRegionalChoiceParaSubPage::SetParameter( KRegionalChoiceParameter a_oRegionalParameter )
{
	m_oRegionalParameter = a_oRegionalParameter;
	m_mapModeRegionalChoiceValue = m_oRegionalParameter.mapModeRegionalChoiceValue;
}

void KInterRegionalChoiceParaSubPage::GetParameter( KRegionalChoiceParameter& ar_oRegionalParameter )
{
...
void KInterRegionalChoiceParaSubPage::InitReportHeader( void )
{
	try
	{
		...
		pColumn = m_wndReportCtrl.AddColumn(new CXTPReportColumn(1, _T("InVehcle Time"), 40));
		pColumn->SetHeaderAlignment(DT_CENTER);
		if (KmzSystem::GetLanguage() == KEMKorea) {
...
void KInterRegionalChoiceParaSubPage::UpdateReportData(  std::map<int, TRegionalParameter> a_mapModeRegionalChoiceValue  )
{
	...
	try
	{
		...
		AutoType iter = a_mapModeRegionalChoiceValue.begin();
		AutoType end  = a_mapModeRegionalChoiceValue.end();
		while(iter != end)
		{
