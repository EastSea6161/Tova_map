...
void KTocLinkProperties::InitRptHeader( void )
{
    try
    {
        ...
        pColumn = m_ctrlReport.AddColumn(new CXTPReportColumn(nIdxColumn++, _T("Color"), nColumnWidth, FALSE));
        pColumn->SetHeaderAlignment(DT_CENTER);
		if (KmzSystem::GetLanguage() == KEMKorea) {
			pColumn->SetCaption(_T("색상"));
		}

        pColumn = m_ctrlReport.AddColumn(new CXTPReportColumn(nIdxColumn++, _T("Width"), nColumnWidth, FALSE));
