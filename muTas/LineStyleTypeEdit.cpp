// LineStyleTypeEdit.cpp : 구현 파일입니다.
//

#include "stdafx.h"
#include "ImChampDir.h"
#include "LineStyleTypeEdit.h"
#include "afxdialogex.h"

#include "DBaseConnector.h"
#include "ImTasDBase.h"
// KLineStyleTypeEdit 대화 상자입니다.

IMPLEMENT_DYNAMIC(KLineStyleTypeEdit, KDialogEx)

KLineStyleTypeEdit::KLineStyleTypeEdit(CWnd* pParent /*=NULL*/)
	: KDialogEx(KLineStyleTypeEdit::IDD, pParent)
{
	m_nPreSelectedStyleID      = -1;
}

KLineStyleTypeEdit::~KLineStyleTypeEdit()
{
}

void KLineStyleTypeEdit::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KLineStyleTypeEdit, KDialogEx)
	ON_CBN_SELCHANGE(IDC_COMBO_STYLE, &KLineStyleTypeEdit::OnCbnSelchangeComboStyle)
	ON_NOTIFY(NM_CLICK, IDC_REPORT, &KLineStyleTypeEdit::OnReportItemClick)
	ON_EN_SETFOCUS(IDC_EDIT_STYLE_NAME, &KLineStyleTypeEdit::OnEnSetfocusEditStyleName)
	ON_CBN_SETFOCUS(IDC_COMBO_STYLE, &KLineStyleTypeEdit::OnCbnSetfocusComboStyle)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_ADD, &KLineStyleTypeEdit::OnBnClickedButtonContentsAdd)
	ON_BN_CLICKED(IDC_BUTTON_CONTENTS_DELETE, &KLineStyleTypeEdit::OnBnClickedButtonContentsDelete)
	ON_BN_CLICKED(IDOK, &KLineStyleTypeEdit::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_STYLE_DELETE, &KLineStyleTypeEdit::OnBnClickedButtonStyleDelete)
END_MESSAGE_MAP()


// KLineStyleTypeEdit 메시지 처리기입니다.
BOOL KLineStyleTypeEdit::OnInitDialog()
{
...
void KLineStyleTypeEdit::UpdateUIState()
{
...
void KLineStyleTypeEdit::OnCbnSelchangeComboStyle()
{
	StyleSelectChangeAction();
}

void KLineStyleTypeEdit::OnReportItemClick( NMHDR* pNotifyStruct, LRESULT* pResult )
{
...
void KLineStyleTypeEdit::LoadInitialStyle()
{
	...
	KProjectDBase::LinkTemplate(spDBaseConnection, 0, vecLinkTemplate);
...
void KLineStyleTypeEdit::StyleSelectChangeAction()
{
	...
	std::vector<TLinkTemplateType> veTemplateType;

	CString strApplicationDirectory = ImChampDir::GetAppDirectory();
	KDBaseConPtr spDBaseConnection = KDBase::GetProjectDBConnection(); //KDBase::GetApplicationDBConnection();
	if (spDBaseConnection != NULL)
	{		
		KProjectDBase::LinkTemplateType(spDBaseConnection, nStyleID, veTemplateType);
	}
	...
	for (int i=0; i<nTemplateCount; i++)
	{
		...
		pItem->SetItemData(veTemplateType[i].TLineType);
		pItem->SetCaption(_T("━"));
        pItem->SetBold(TRUE);
		...
		pItemSize->SetValue(veTemplateType[i].TWidth);
		pItemSize->SetItemData((DWORD_PTR)veTemplateType[i].TWidth);
...
void KLineStyleTypeEdit::OnEnSetfocusEditStyleName()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_UNCHECKED);
...
void KLineStyleTypeEdit::OnCbnSetfocusComboStyle()
{
	CButton* pButton = NULL;
	pButton = (CButton*)GetDlgItem(IDC_RADIO1);	
	pButton->SetCheck(BST_CHECKED);
...
void KLineStyleTypeEdit::OnBnClickedButtonContentsAdd()
{
	CXTPReportRecord*     pRecord  = NULL;
	CXTPReportRecordItem* pItem    = NULL;
	CXTPReportRecordItemNumber* pItemSize = NULL;
	...
	pItemSize->SetItemData((DWORD_PTR)1.0);
...
void KLineStyleTypeEdit::OnBnClickedButtonContentsDelete()
{
	CXTPReportRecords*	  pRecords = NULL;
	CXTPReportRecord*     pRecord  = NULL;
	CXTPReportRecordItem* pItem    = NULL;
...
void KLineStyleTypeEdit::Apply()
{
	...
	try
	{
		...
		if (!bOverwrite)
		{
			strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplate(ID,Type,Name) Values('%d','%d','%s')"), nID,0,strStyleName);
			spDBaseConnection->ExecuteUpdate(strSQL);
		}		

		strSQL.Format(_T("Delete From LinkTemplateType Where ID = '%d'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);
		...
		for (int i=0; i<nRecordCount; i++)
		{
			...
			int nLineType = pItem->GetItemData();
			...
			double dWidth = _ttof(pItem->GetCaption(NULL));
			strSQL.Format(_T("INSERT OR REPLACE INTO LinkTemplateType(ID,Seq,LineType,Width,RGB) Values('%d','%d','%d','%.1f','%s')"), nID, i, nLineType, dWidth, strRGB);
...
void KLineStyleTypeEdit::OnBnClickedOk()
{
	Apply();
}


void KLineStyleTypeEdit::OnBnClickedButtonStyleDelete()
{
	int  nCheckedRadioBtn  = GetCheckedRadioButton(IDC_RADIO1, IDC_RADIO2);
	...
	try
	{
		...
		spDBaseConnection->BeginTransaction();		
		strSQL.Format(_T("Delete From LinkTemplate Where ID = '%d' and Type='0'"), nID);
		spDBaseConnection->ExecuteUpdate(strSQL);		
