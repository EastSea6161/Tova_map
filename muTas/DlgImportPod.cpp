...
// KDlgImportPod 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportPod, KWhiteDlgEx)

KDlgImportPod::KDlgImportPod(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportPod::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportPod::~KDlgImportPod()
{
}

void KDlgImportPod::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportPod, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportPod::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportPod::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportPod::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportPod::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportPod::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportPod::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportPod::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportPod::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportPod::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportPod::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportPod 메시지 처리기입니다.
void KDlgImportPod::ResizeComponent()
{
	try 
	{
...
void KDlgImportPod::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportPod::GetSelectedImportType()
{
...
BOOL KDlgImportPod::OnInitDialog()
{
...
void KDlgImportPod::RegisterZoneRelationTable()
{
	KTableRelationInfo::RelationTablesZone(m_mapZoneRelationTable);
}

int KDlgImportPod::IsZoneRelationTable(CString a_strTableName)
{
...
void KDlgImportPod::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportPod::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportPod::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportPod::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportPod::InitComboSeparator()
{
	try
	{
...
void KDlgImportPod::InitComboEncoding()
{
	try
	{
...
void KDlgImportPod::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportPod::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportPod::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportPod::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	...
	try
	{
		...
		for (int i= 0; i< nColumnCount; i++) {
            ...
            bool bRed(false);
            CString strField(_T("")); {
                if (strColNm.CompareNoCase(COLUMN_OZONE) == 0  ||
                    strColNm.CompareNoCase(COLUMN_DZONE) == 0 ) 
                {
...
int KDlgImportPod::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportPod::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportPod::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportPod::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportPod::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportPod::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportPod::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportPod::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportPod::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportPod::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportPod::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportPod::AllNullCheck()
{
	try 
	{
...
void KDlgImportPod::OnBnClickedOk()
{
	UpdateData( TRUE );
	...
	try
	{
		...
		if (true) { // 데이터 검증 및 중간 바이너리 작성
			...
			oPara.TKeyInt[1] = 0;      // Success 개수 저장
			oPara.TKeyInt[2] = 0;      // 실패 개수 저장
...
unsigned __stdcall KDlgImportPod::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportPod* pDlg = (KDlgImportPod*)pPara->TWindow;
...
void KDlgImportPod::Execute(ThreadPara* pPara)
{
	try 
	{
		KDBaseImportTable::GetZoneID(m_pTable, m_setZoneID);
		KDBaseImportTable::ODTableRecord(m_pTable, m_setODZoneID);
...
void KDlgImportPod::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportPod::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_PURPOSE_OD) == false) { // true: 데이터무, false : 데이터 존재
				...
				a_strMsg.AppendFormat(_T("#%d. 레코드 개수: %d 건\r\n"), nIdx++, pPara->TKeyInt[0]);

				if (pPara->TKeyInt[2] > 0) { //실패 건수가 있을 경우
					a_strMsg.AppendFormat(_T("#%d. 양호: %d 건, 불량: %d 건\r\n"), nIdx++, pPara->TKeyInt[1], pPara->TKeyInt[2]);
				}
			}
...
void KDlgImportPod::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
#pragma endregion Load vecColMatch
	...
	try
	{
		...
		pPara->TKeyInt[0] = nImportDataCount;      // 레코드 개수 저장
		pPara->TKeyInt[1] = nSuccessCnt;           // Update/Add Mode일 경우 성공 데이터의 개수
		pPara->TKeyInt[2] = nFailedCnt;            // Update/Add Mode일 경우 실패 데이터의 개수
...
void KDlgImportPod::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportPod::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportPod::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportPod::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportPod::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
...
bool KDlgImportPod::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	int nZoneRelationColumnCount = IsZoneRelationTable(m_pTable->Name());
	if( nZoneRelationColumnCount != 2 ) {
		return false; // Purpose_OD 테이블은 2가 나와야 한다.
	}
	...
	for(; iter != itEnd; ++iter )
	{
		...
		if (strField.CompareNoCase(COLUMN_OZONE) == 0) {
			nxOZoneID = iter->second;
		} else if (strField.CompareNoCase(COLUMN_DZONE) == 0) {
			nxDZoneID = iter->second;
		}
	}
	KODKey oODKey(nxOZoneID, nxDZoneID);
	return IsODZoneID(oODKey);
}

bool KDlgImportPod::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportPod::IsODZoneID(KODKey a_oODKey)
{
	auto iter   = m_setODZoneID.find(a_oODKey);
	auto itEnd  = m_setODZoneID.end();
...
bool KDlgImportPod::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
	...
	int nZoneRelationColumnCount = IsZoneRelationTable(m_pTable->Name());
	if( nIdCount != nZoneRelationColumnCount ) // 2: ozone_id, dzone_id  or 1: zone_id
		bWrongIdCount = true;

	return bWrongIdCount;
}

bool KDlgImportPod::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
...
#include <string>
void KDlgImportPod::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportPod::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);

	int nImportDataCount = (int)pPara->TKeyInt[0];
...
void KDlgImportPod::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);

	int     nImportDataCount = (int)pPara->TKeyInt[1]; // 양호건수가 Update 전체건수가 된다.
	double	dValue  (0.0);
	Integer nxValue (0);
	CString strValue(_T(""));
...
void KDlgImportPod::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);

	int nImportDataCount = (int)pPara->TKeyInt[1]; // 양호건수가 Add 전체건수가 된다.
...
CString KDlgImportPod::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportPod::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportPod::ErrorFileOpen()
{
	try
	{
...
void KDlgImportPod::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportPod::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
...
bool KDlgImportPod::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportPod::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportPod::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportPod::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportPod::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportPod::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportPod::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportPod::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportPod::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
