...
// KDlgImportSedPop 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportSedPop, KWhiteDlgEx)

KDlgImportSedPop::KDlgImportSedPop(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportSedPop::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportSedPop::~KDlgImportSedPop()
{
}

void KDlgImportSedPop::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportSedPop, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportSedPop::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportSedPop::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportSedPop::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportSedPop::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportSedPop::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportSedPop::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportSedPop::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportSedPop::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportSedPop::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportSedPop::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportSedPop 메시지 처리기입니다.
void KDlgImportSedPop::ResizeComponent()
{
	try 
	{
...
void KDlgImportSedPop::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportSedPop::GetSelectedImportType()
{
...
BOOL KDlgImportSedPop::OnInitDialog()
{
...
void KDlgImportSedPop::RegisterZoneRelationTable()
{
	KTableRelationInfo::RelationTablesZone(m_mapZoneRelationTable);
}

int KDlgImportSedPop::IsZoneRelationTable(CString a_strTableName)
{
	a_strTableName = a_strTableName.MakeUpper();
	auto iter  = m_mapZoneRelationTable.find(a_strTableName);
	auto itEnd = m_mapZoneRelationTable.end();

	if (iter != itEnd) {
		int nZoneRelationColumnCount = (int)iter->second;
		return nZoneRelationColumnCount;
	}

	return 0;
}

void KDlgImportSedPop::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportSedPop::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportSedPop::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportSedPop::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportSedPop::InitComboSeparator()
{
	try
	{
...
void KDlgImportSedPop::InitComboEncoding()
{
	try
	{
...
void KDlgImportSedPop::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportSedPop::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportSedPop::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportSedPop::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
int KDlgImportSedPop::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportSedPop::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportSedPop::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportSedPop::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportSedPop::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportSedPop::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportSedPop::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportSedPop::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportSedPop::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportSedPop::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportSedPop::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportSedPop::AllNullCheck()
{
	try 
	{
...
void KDlgImportSedPop::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportSedPop::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportSedPop* pDlg = (KDlgImportSedPop*)pPara->TWindow;
...
void KDlgImportSedPop::Execute(ThreadPara* pPara)
{
	try 
	{
		KDBaseImportTable::GetZoneID(m_pTable, m_setZoneID);
		KDBaseImportTable::SedPopDBPK(m_pTable, m_mapSedPopPK);
...
void KDlgImportSedPop::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportSedPop::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_SED_POP) == false) { // true: 데이터무, false : 데이터 존재
...
void KDlgImportSedPop::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportSedPop::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportSedPop::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportSedPop::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportSedPop::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		bool bPK(false);
		// 조회 key value 추출
		if( pIOColumn->PrimaryKey() || KImportCommon::IsZoneRelationColumn(strIOColumnName) == true )
			bPK = true;
...
bool KDlgImportSedPop::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
	...
	try 
	{
		...
		for (int i= 0; i< nSizeVec; i++) {
			...
			if ( KImportCommon::IsZoneRelationColumn(strIOColumnName) )	{ 
				if( emDataType != KEMIODataTypeInteger )
					continue;
				...
				if (IsZoneID(nxValue) == false)	{
					CString strErr;
					//strErr.Format(_T("Zone 테이블에 존재하지 않는 Zone_ID : %I64d"), nxValue);
					if (KmzSystem::GetLanguage() == KEMKorea) {
...
bool KDlgImportSedPop::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for(; iter != itEnd; ++iter) {
		...
		if (strField.CompareNoCase(COLUMN_SED_POP_ZONE_ID) == 0) {
			nxZoneID = iter->second;
		} else if (strField.CompareNoCase(COLUMN_SED_POP_SEX) == 0)	{
			nxSex = iter->second;
		} else if (strField.CompareNoCase(COLUMN_SED_POP_AGE) == 0)	{
			nxAge = iter->second;
		}
	}
	...
	return IsSedPopDBID(oODKey, nxAge);
}

bool KDlgImportSedPop::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportSedPop::IsSedPopDBID(KODKey a_oODKey, Integer a_nxAge)
{
	auto iter  = m_mapSedPopPK.find( a_oODKey );
	auto itEnd = m_mapSedPopPK.end();

	if(iter == itEnd) {
		return false;
	} else {
		std::set<Integer> &setAge = iter->second;
		if (setAge.find(a_nxAge) == setAge.end()) {
			return false;
		}
	}
...
bool KDlgImportSedPop::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
	...
	if (pIOColumns->GetPKColumnCount() != nIdCount) // zone_id, sex, age
		bWrongIdCount = true;

	return bWrongIdCount;
}

bool KDlgImportSedPop::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
		...
		if ( m_strTableName.CompareNoCase(TABLE_SED_POP) == 0) {
			for(; iter != itEnd; ++iter ) {
				CString strField = iter->first;
...
#include <string>
void KDlgImportSedPop::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportSedPop::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	...
	CString strQuery;{
		strQuery.Format(_T("delete from %s"), m_pTable->Name());
...
void KDlgImportSedPop::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportSedPop::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportSedPop::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportSedPop::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportSedPop::ErrorFileOpen()
{
	try
	{
...
void KDlgImportSedPop::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_ZONE) == true) { // true: 데이터무, false : 데이터 존재
			//ThrowException(_T("Node Import를 통해 먼저 Zone Data를 입력 하세요."));
			if (KmzSystem::GetLanguage() == KEMKorea) {
				ThrowException(_T("존 노드 데이터를 먼저 입력해 주세요."));
			}
			else { //if (KmzSystem::GetLanguage() == KEMEnglish) {
				ThrowException(_T("Import Zone Node data first !"));
			}
		} 

		CString strColumnName(_T(""));
#pragma region P_OD 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		...
		for (int i= 0; i < nColumnCount; i++) {
			...
			if (KImportCommon::IsZoneRelationColumn(strColumnName)) {
				//strMsg.Format(_T("\"%s\" Field는 Null일 수 없습니다."), pIOColumn->Caption());
				if (KmzSystem::GetLanguage() == KEMKorea) {
					strMsg.Format(_T("\"%s\" 필수 입력 컬럼은 Null 값일 수 없습니다 !"), pIOColumn->Caption());
				}
...
#pragma endregion P_OD 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportSedPop::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region P_OD 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
		...
		for (int i = 0; i < nColumnCount; ++i) {
			...
			CString strMsg(_T(""));
			if ( nSelectedIndex == 0) {
				if (pIOColumn->PrimaryKey() || KImportCommon::IsZoneRelationColumn(strColumnName) == true) {
					//strMsg.Format(_T("\"%s\" Field는 Null일 수 없습니다."), pIOColumn->Caption());
					if (KmzSystem::GetLanguage() == KEMKorea) {
						strMsg.Format(_T("\"%s\" 필수 입력 컬럼은 Null 값일 수 없습니다 !"), pIOColumn->Caption());
					}
...
#pragma endregion P_OD 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportSedPop::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportSedPop::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportSedPop::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportSedPop::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportSedPop::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportSedPop::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportSedPop::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportSedPop::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportSedPop::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
