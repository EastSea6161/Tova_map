...
class KDlgImportAccessStopNode : public KWhiteDlgEx
{
	DECLARE_DYNAMIC(KDlgImportAccessStopNode)

public:
	KDlgImportAccessStopNode(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~KDlgImportAccessStopNode();

	// 대화 상자 데이터입니다.
	enum { IDD = IDD_201_01_07_IMPORT_TABLE_ACCESSSTOPNODE };
...
private:
	...
	bool		 IsAccessStopNodeDBID(KODKey a_oODKey, int a_nMode);
...
protected :
	...
	std::map<KODKey, std::set<int>>     m_mapAccessStopNodeID;
