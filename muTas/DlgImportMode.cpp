...
// KDlgImportMode 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportMode, KWhiteDlgEx)

KDlgImportMode::KDlgImportMode(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportMode::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportMode::~KDlgImportMode()
{
}

void KDlgImportMode::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportMode, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportMode::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportMode::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportMode::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportMode::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportMode::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportMode::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportMode::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportMode::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportMode::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportMode::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportMode 메시지 처리기입니다.
void KDlgImportMode::ResizeComponent()
{
	try 
	{
...
void KDlgImportMode::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportMode::GetSelectedImportType()
{
...
BOOL KDlgImportMode::OnInitDialog()
{
...
void KDlgImportMode::RegisterZoneRelationTable()
{
	KTableRelationInfo::RelationTablesZone(m_mapZoneRelationTable);
}

int KDlgImportMode::IsZoneRelationTable(CString a_strTableName)
{
...
void KDlgImportMode::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportMode::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportMode::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportMode::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportMode::InitComboSeparator()
{
	try
	{
...
void KDlgImportMode::InitComboEncoding()
{
	try
	{
...
void KDlgImportMode::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportMode::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportMode::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportMode::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
int KDlgImportMode::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportMode::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportMode::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportMode::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportMode::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportMode::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportMode::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportMode::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportMode::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportMode::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportMode::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportMode::AllNullCheck()
{
	try 
	{
...
void KDlgImportMode::OnBnClickedOk()
{
	UpdateData( TRUE );
...
unsigned __stdcall KDlgImportMode::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportMode* pDlg = (KDlgImportMode*)pPara->TWindow;
...
void KDlgImportMode::Execute(ThreadPara* pPara)
{
	try 
	{
...
void KDlgImportMode::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportMode::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, TABLE_MODE_OD) == false) { // true: 데이터무, false : 데이터 존재
...
void KDlgImportMode::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportMode::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportMode::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportMode::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportMode::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportMode::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
...
bool KDlgImportMode::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
...
bool KDlgImportMode::IsZoneID(Integer a_nxValue)
{
...
bool KDlgImportMode::IsODZoneID(KODKey a_oODKey)
{
...
bool KDlgImportMode::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
...
bool KDlgImportMode::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
...
#include <string>
void KDlgImportMode::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportMode::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportMode::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportMode::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportMode::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportMode::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportMode::ErrorFileOpen()
{
	try
	{
...
void KDlgImportMode::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region M_OD 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
...
#pragma endregion M_OD 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportMode::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region M_OD 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
...
#pragma endregion M_OD 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportMode::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportMode::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportMode::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportMode::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportMode::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
...
void KDlgImportMode::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportMode::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportMode::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportMode::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
