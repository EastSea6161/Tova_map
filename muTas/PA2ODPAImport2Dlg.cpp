...
IMPLEMENT_DYNAMIC(KPA2ODPAImport2Dlg, KDialogEx)

KPA2ODPAImport2Dlg::KPA2ODPAImport2Dlg(
	BOOL a_bContainHeader, KEMCSVEncoding a_emCSVEncoding, TCHAR a_cSeparator,
	CString a_strFileName, CWnd* pParent /*=NULL*/)
	: KDialogEx(KPA2ODPAImport2Dlg::IDD, pParent)
...
KPA2ODPAImport2Dlg::~KPA2ODPAImport2Dlg(void)
{
}

void KPA2ODPAImport2Dlg::DoDataExchange( CDataExchange* pDX )
{
...
BEGIN_MESSAGE_MAP(KPA2ODPAImport2Dlg, KDialogEx)
	ON_BN_CLICKED(ID_BACK, &KPA2ODPAImport2Dlg::OnBnClickedBack)
	ON_BN_CLICKED(IDOK, &KPA2ODPAImport2Dlg::OnBnClickedOk)
END_MESSAGE_MAP()

bool KPA2ODPAImport2Dlg::GoBack( void )
{
	return m_bGoBack;
}

BOOL KPA2ODPAImport2Dlg::OnInitDialog()
{
...
void KPA2ODPAImport2Dlg::OnBnClickedBack()
{
	m_bGoBack = true;
	KDialogEx::OnOK();
}

void KPA2ODPAImport2Dlg::GenerateHeaderColumn( void )
{
	TxLogDebugStartMsg();
...
void KPA2ODPAImport2Dlg::ParseCSVLineString( CString& strLine, std::vector<CString>& row )
{
	bool bQuote = false;
	int nSub = 0;
...
void KPA2ODPAImport2Dlg::HomeBasedInfo( std::list<KPA2ODInfo*> a_lstHomeBasedPA2ODInfo )
{
	m_lstHomeBasedPA2ODInfo = a_lstHomeBasedPA2ODInfo;
}


void KPA2ODPAImport2Dlg::NotHBasedInfo( std::list<KPA2ODInfo*> a_lstNotHBasedPA2ODInfo )
{
	m_lstNotHBasedPA2ODInfo = a_lstNotHBasedPA2ODInfo;
}

void KPA2ODPAImport2Dlg::InitPreviewReport( void )
{
...
void KPA2ODPAImport2Dlg::UpdatePreviewReport( void )
{
...
void KPA2ODPAImport2Dlg::PreviewDataLoad( void )
{
	TxLogDebugStartMsg();
	bool bResult = true;
...
void KPA2ODPAImport2Dlg::InitZoneReport( void )
{
	CXTPReportColumn* pCXTPColumn = NULL;
...
void KPA2ODPAImport2Dlg::UpdateZoneReport( void )
{
...
void KPA2ODPAImport2Dlg::InitHBReport( void )
{
	CXTPReportColumn* pCXTPColumn = NULL;

	pCXTPColumn = m_wndHBReport.AddColumn( new CXTPReportColumn( 0, _T("Field"), 50) );
	pCXTPColumn->GetEditOptions()->m_bAllowEdit = FALSE;
	pCXTPColumn->SetHeaderAlignment(DT_CENTER);
	pCXTPColumn->SetAlignment(DT_CENTER);
	...
	pCXTPColumn = m_wndHBReport.AddColumn( new CXTPReportColumn( 1, _T("Import"), 50) );
	pCXTPColumn->SetHeaderAlignment(DT_CENTER);
	pCXTPColumn->GetEditOptions()->AddConstraint(_T("Null"), nComboIndex++);
...
void KPA2ODPAImport2Dlg::UpdateHBReport( void )
{
...
void KPA2ODPAImport2Dlg::InitNHBReport( void )
{
	CXTPReportColumn* pCXTPColumn = NULL;

	pCXTPColumn = m_wndNHBReport.AddColumn( new CXTPReportColumn( 0, _T("Field"), 50) );
	pCXTPColumn->GetEditOptions()->m_bAllowEdit = FALSE;
	pCXTPColumn->SetHeaderAlignment(DT_CENTER);
	pCXTPColumn->SetAlignment(DT_CENTER);
	...
	pCXTPColumn = m_wndNHBReport.AddColumn( new CXTPReportColumn( 1, _T("Import"), 50) );
	pCXTPColumn->SetHeaderAlignment(DT_CENTER);
	pCXTPColumn->GetEditOptions()->AddConstraint(_T("Null"), nComboIndex++);
...
void KPA2ODPAImport2Dlg::UpdateNHBReport( void )
{
...
void KPA2ODPAImport2Dlg::OnBnClickedOk()
{
	try
	{
		CWaitCursor cw;
...
void KPA2ODPAImport2Dlg::WriteBinaryFile( void )
{
	ImChampDir::SetCurrentDirectoryToApp();
	CString strPAInputFile = _T("PAData.dat");
...
void KPA2ODPAImport2Dlg::LineData2Binary( std::vector<CString> a_vecLineData, std::ofstream& ofs )
{
	__int64 nxValue;
	double  dValue;
	//WriteZone
