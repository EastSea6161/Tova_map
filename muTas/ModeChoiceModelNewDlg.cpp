// ModeChoiceModelNewDlg.cpp : 구현 파일입니다.
//
#include "stdafx.h"
...
IMPLEMENT_DYNAMIC(KModeChoiceModelNewDlg, KDialogEx)

KModeChoiceModelNewDlg::KModeChoiceModelNewDlg(CWnd* pParent /*=NULL*/)
	: KDialogEx(KModeChoiceModelNewDlg::IDD, pParent)
	, m_pTarget(NULL)
...
KModeChoiceModelNewDlg::~KModeChoiceModelNewDlg()
{
	TxLogDebugStartMsg();
	clearMInfoList();
	clearPurposeSet();
	DeleteTempSavedFunctionTree();
...
void KModeChoiceModelNewDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KModeChoiceModelNewDlg, KDialogEx)
	ON_BN_CLICKED(IDC_DEMANDSTRATUM, &KModeChoiceModelNewDlg::OnBnClickedDemandstratum)
	ON_BN_CLICKED(IDC_DECISIONTREE, &KModeChoiceModelNewDlg::OnBnClickedDecisiontree)
	ON_BN_CLICKED(IDC_DEFINEVARIABLE, &KModeChoiceModelNewDlg::OnBnClickedDefinevariable)
	ON_BN_CLICKED(IDC_ADJUSTMENT, &KModeChoiceModelNewDlg::OnBnClickedAdjustment)
	ON_BN_CLICKED(IDC_OUTPUTDESIGN, &KModeChoiceModelNewDlg::OnBnClickedOutputdesign)
	ON_BN_CLICKED(IDC_RUN, &KModeChoiceModelNewDlg::OnBnClickedRun)
	ON_BN_CLICKED(IDC_CLOSE, &KModeChoiceModelNewDlg::OnBnClickedClose)
	ON_BN_CLICKED(IDC_OUTPUTTABLE, &KModeChoiceModelNewDlg::OnBnClickedOutputtable)
	ON_BN_CLICKED(IDC_OUTPUTGRAPH, &KModeChoiceModelNewDlg::OnBnClickedOutputgraph)
	ON_WM_CTLCOLOR()
	//ON_WM_ACTIVATE() - markup button focus 있을때 외부 클릭 시 버튼 사라짐 문제 때문에 기능 삭제
	ON_BN_CLICKED(IDC_SELECT_ELASTICITY, &KModeChoiceModelNewDlg::OnBnClickedSelectElasticity)
	ON_BN_CLICKED(IDC_REGION, &KModeChoiceModelNewDlg::OnBnClickedRegion)
	ON_BN_CLICKED(IDC_BUTTON2, &KModeChoiceModelNewDlg::OnBnClickedSelectvariable)
	ON_BN_CLICKED(IDC_RUNELASTICITY, &KModeChoiceModelNewDlg::OnBnClickedRunelasticity)
	ON_BN_CLICKED(IDC_LOGVIEW, &KModeChoiceModelNewDlg::OnBnClickedLogview)
	ON_WM_COPYDATA()
END_MESSAGE_MAP()


void KModeChoiceModelNewDlg::SetTarget(KTarget* pTarget)
{
    TxLogDebugStartMsg();
	m_pTarget = pTarget;
    TxLogDebugEndMsg();
...
void KModeChoiceModelNewDlg::ModeChoiceModelThreadProc(void* pParam)
{
	if(NULL == pParam)
	{
		g_pProgressDlg->Stop();
	...
	KModeChoiceModelNewDlg* pDlg = (KModeChoiceModelNewDlg*)pParam;
...
void KModeChoiceModelNewDlg::initPurposeSet(void)
{
    TxLogDebugStartMsg();
	clearPurposeSet();
...
void KModeChoiceModelNewDlg::clearPurposeSet(void)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::clearMInfoList(void)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::initOutputInfo(void)
{
    TxLogDebugStartMsg();
	clearOutputInfo();
...
void KModeChoiceModelNewDlg::initOutputInfo2()
{
	TxLogDebugStartMsg();
	clearOutputInfo();
	...
	try
	{
		AutoType iter  = m_mapTempSavedFunctionTree.begin();
		AutoType itEnd = m_mapTempSavedFunctionTree.end();
...
void KModeChoiceModelNewDlg::clearOutputInfo(void)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::preparePurposeODIndex(std::map<CString, int>& purposeIndexMap)
{
    TxLogDebugStartMsg();
	//int nIndex = 0; ptj
	int nIndex = 1;
...
void KModeChoiceModelNewDlg::preparePurposeODData(KODKeyDoubleRecords& records)
{
    TxLogDebugStartMsg();
...
KMultiIDDoubleRecord* KModeChoiceModelNewDlg::preparePurposeODData(void)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::prepareParameterODIndex(std::map<CString, int>& paramIndexMap)
{
...
KODKeyDoubleRecords* KModeChoiceModelNewDlg::prepareParameterODData(CString strFilter)
{
    TxLogDebugStartMsg();
...
KODKeyDoubleRecords* KModeChoiceModelNewDlg::prepareParameterODDataByODGroup(std::map<int, TGroupGenerationODInfo> &a_mapGroupGenerationODInfo, CString strFilter)
{
	KDBaseConPtr spDBaseConnection = m_pTarget->GetDBaseConnection();

	CString strSQL = _T(" select od_Group_Key, Output_Column from Group_Generation_OD order by od_Group_Key ");
	...
	try
	{
		...
		while(spResult->Next())
		{
			TGroupGenerationODInfo oTGroupGenerationODInfo;

			oTGroupGenerationODInfo.nODGroupCode    = spResult->GetValueInt   (0);
			oTGroupGenerationODInfo.strOutputColumn = spResult->GetValueString(1);
			oTGroupGenerationODInfo.nIndex          = nIndex++;

			a_mapGroupGenerationODInfo.insert(std::make_pair(oTGroupGenerationODInfo.nODGroupCode, oTGroupGenerationODInfo));
		}
	}
	catch (...)
	{
	...
	AutoType iter  = a_mapGroupGenerationODInfo.begin();
	AutoType itEnd = a_mapGroupGenerationODInfo.end();
	...
	while (iter != itEnd)
	{
		TGroupGenerationODInfo &oTGroupGenerationODInfo = iter->second;

		KIOColumn* pColumnByODGroup = pColumns->GetColumn(oTGroupGenerationODInfo.strOutputColumn);

		reqColumns.AddNewColumn(pColumnByODGroup);
	...
	KODKeyDoubleRecords* pRecords = new KODKeyDoubleRecords(nReqColumnCount);
	KBulkDBase::BulkODSelectByModeChoice(pTable, strFilter, reqColumns, *pRecords);

	return pRecords;
}


KMultiIDDoubleRecord* KModeChoiceModelNewDlg::prepareModeODData(std::map<KMode*, int>& modeIndexMap)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::prepareModeIndexMap(std::map<KMode*, int>& modeIndexMap)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::PrepareFunctionIdIndexMap(std::map<int, int> &a_functionIdIndexMap)
{
	TxLogDebugStartMsg();

	try
	{
		a_functionIdIndexMap.clear();
		...
		{
			int nFunctionID = iter->first;
			a_functionIdIndexMap.insert(std::make_pair(nFunctionID, nIndex++));
...
void KModeChoiceModelNewDlg::LoadXmlDataModeChoiceVarDefault(std::set<int> &a_setUsedFunctionID)
{
	try
	{
		DeleteTempSavedFunctionTree();
		...
		while (nullptr != pElementVariable)
		{
			...
			AutoType itFind = a_setUsedFunctionID.find(nVariableID);
			if (itFind != a_setUsedFunctionID.end())
			{
				m_mapTempSavedFunctionTree.insert(std::make_pair(nVariableID, pLoadSimpleVariable));
			}
...
KMode* KModeChoiceModelNewDlg::FindMode(int a_nModeID)
{
	try
	{
...
void KModeChoiceModelNewDlg::GenerateModeFunction(LPCTSTR strFunction, KModeFunction& function)
{
...
void KModeChoiceModelNewDlg::InitExpressionChecker(void)
{
...
CString KModeChoiceModelNewDlg::GetFieldName(LPCTSTR strCaption)
{
	CString strRet = _T("");
...
void KModeChoiceModelNewDlg::DeleteTempSavedFunctionTree()
{
	try
	{
		AutoType iter  = m_mapTempSavedFunctionTree.begin();
		...
		m_mapTempSavedFunctionTree.clear();
	}
	catch (...)
	{
		TxLogDebugException();	
...
KDTreeNodeCollection* KModeChoiceModelNewDlg::generateModeTree(KModeChoiceInfo* pInfo, 
...
KDTreeNodeCollection* KModeChoiceModelNewDlg::generateModeTree(int nCode, 
...
bool KModeChoiceModelNewDlg::addModeODColumns(std::map<KMode*, int>& modeODIndexMap, KIOColumns& columnCollection)
{
    TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::recordModeChoiceResult(LPCTSTR strStartDate, LPCTSTR strEndDate)
{
    TxLogDebugStartMsg();
	KDBaseConPtr spDBaseConnection = m_pTarget->GetDBaseConnection();
...
// KModeChoiceModelNewDlg 메시지 처리기입니다.


BOOL KModeChoiceModelNewDlg::OnInitDialog()
{
	TxLogDebugStartMsg();
	...
	modelXamlKR.Format(
		...
		_T("  </Border.Background>")
		_T("  <StackPanel HorizontalAlignment='Center' VerticalAlignment='Center' TextBlock.FontFamily='나눔고딕' TextBlock.FontSize='%f'>")
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		SetDlgItemText(IDC_STATIC_TEXT1, _T(""));
		SetDlgItemText(IDC_STATIC_TEXT2, _T(""));
		SetDlgItemText(IDC_STATIC_TEXT3, _T(""));
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		m_btnDemand.SetWindowText(KR_UIXAML_M_DEMAND_DEFAULT);
	}
	else {
...
// 	m_btnDecisionTree.EnableWindow(FALSE);
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		m_btnVariable.SetWindowText(KR_UIXAML_M_VARIABLE_FALSE);
	}
	else {
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		m_btnOutputDesign.SetWindowText(KR_UIXAML_M_OUTPUT_FALSE);
	}
	else {
...
void KModeChoiceModelNewDlg::OnBnClickedDemandstratum()
{
    TxLogDebugStartMsg();
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		if (KmzSystem::GetLanguage() == KEMKorea) {
			m_btnDemand.SetWindowText(KR_UIXAML_M_DEMAND_SELECTED);
			m_btnVariable.SetWindowText(KR_UIXAML_M_VARIABLE_DEFAULT);
		}
		else {
...
void KModeChoiceModelNewDlg::LoadDefaultPurpoeGroupODInfo()
{
...
void KModeChoiceModelNewDlg::LoadODGroupDefineVariableID(int a_nGroupKey, int a_nPurposeID, std::map<int, int> &a_mapODGroupSavedFunctionID)
{
	try
	{
...
void KModeChoiceModelNewDlg::OnBnClickedDecisiontree()
{
//     TxLogDebugStartMsg();
...
// 			{
// 				KPurpose* pPurpose = pInfo->Purpose();
// 				strEmptyTreeCheckMsg.Format(_T("%s에 대한 Decision Tree 정보가 없습니다."), pPurpose->PurposeName());
// 				nEmptyCount++;
// 			}
// 		}
// 
...
void KModeChoiceModelNewDlg::OnBnClickedDefinevariable()
{
    TxLogDebugStartMsg();
	KMDefineVariableGroupDlg dlg(m_mapDefaultODGroup);
	...
	if(dlg.DoModal() == IDOK)
	{
		std::set<int> setUsedFunctionID;
		dlg.UsedFunctionIDSet(setUsedFunctionID);
		LoadXmlDataModeChoiceVarDefault(setUsedFunctionID);
		initOutputInfo2();

		if (KmzSystem::GetLanguage() == KEMKorea) {
			m_btnVariable.SetWindowText(KR_UIXAML_M_VARIABLE_SELECTED);
			m_btnOutputDesign.SetWindowText(KR_UIXAML_M_OUTPUT_DEFAULT);
		}
		else {
...
void KModeChoiceModelNewDlg::OnBnClickedAdjustment()
{
    TxLogDebugStartMsg();
	KMAdjustmentDlg dlg;
	dlg.Target(m_pTarget);
...
void KModeChoiceModelNewDlg::OnBnClickedOutputdesign()
{
    TxLogDebugStartMsg();
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		if (KmzSystem::GetLanguage() == KEMKorea) {
			m_btnOutputDesign.SetWindowText(KR_UIXAML_M_OUTPUT_SELECTED);
		}
		else {
...
void KModeChoiceModelNewDlg::RunButtonControl()
{
	UpdateData(TRUE);
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		if( strButtonState.CompareNoCase(KR_UIXAML_M_DEMAND_SELECTED) != 0 )
		{
			bButtonRun = FALSE;
		}
	}
...
// 	}
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		if( strButtonState.CompareNoCase(KR_UIXAML_M_VARIABLE_SELECTED) != 0 )
		{
			bButtonRun = FALSE;
		}
	}
	...
	if (KmzSystem::GetLanguage() == KEMKorea) {
		if( strButtonState.CompareNoCase(KR_UIXAML_M_OUTPUT_SELECTED) != 0 )
		{
			bButtonRun = FALSE;
		}
	}
...
void KModeChoiceModelNewDlg::OnBnClickedRun()
{
 	TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::OnBnClickedClose()
{
	OnCancel();
}


void KModeChoiceModelNewDlg::OnBnClickedOutputtable()
{
	TxLogDebugStartMsg();
	KIOTables* pIOTables = m_pTarget->Tables();
	KIOTable* pODTable = pIOTables->FindTable(_T("mode_od"));
...
void KModeChoiceModelNewDlg::OnBnClickedOutputgraph()
{
	TxLogDebugStartMsg();
	CChildFrame* pChildFrame = (CChildFrame*)((CMainFrameWnd*)AfxGetMainWnd())->MDIGetActive();
...
HBRUSH KModeChoiceModelNewDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    HBRUSH hbr = KDialogEx::OnCtlColor(pDC, pWnd, nCtlColor);

    // TODO:  여기서 DC의 특성을 변경합니다.       
    UINT ID = pWnd->GetDlgCtrlID();
    if( ID == IDC_SELECT_ELASTICITY || ID == IDC_STATIC_TEXT1 || ID == IDC_STATIC_TEXT2 || ID == IDC_STATIC_TEXT3 )
    {        
...
void KModeChoiceModelNewDlg::KRedrawWindow()
{
    RedrawWindow(0, 0, RDW_ALLCHILDREN|RDW_INVALIDATE|RDW_ERASE);
}


void KModeChoiceModelNewDlg::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
...
void KModeChoiceModelNewDlg::OnBnClickedSelectElasticity()
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
...
void KModeChoiceModelNewDlg::OnBnClickedRegion()
{
	TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::OnBnClickedSelectvariable()
{
	TxLogDebugStartMsg();
...
void KModeChoiceModelNewDlg::CalculateModel()
{	
	TxLogDebugStartMsg();
	bool  bResult = false;
	...
	std::map<int, int>     functionIdIndexMap;
	...
	PrepareFunctionIdIndexMap(functionIdIndexMap);
	bResult = CalculateNonGroupValue(purposeODRecords, purposeODIndexMap, paramODIndexMap, modeODIndexMap, functionIdIndexMap, modeODRecords);
...
bool KModeChoiceModelNewDlg::CalculateNonGroupValue( KODKeyDoubleRecords& purposeODRecords, 
												std::map<CString, int>& purposeODIndexMap, 
												std::map<CString, int>& paramODIndexMap, 
												std::map<KMode*, int>& modeODIndexMap, 
												...
												KODKeyDoubleRecords& modeODRecords/*out*/ )
{
	...
	std::map<int, TGroupGenerationODInfo> mapGroupGenerationODInfo;
	KODKeyDoubleRecords* pParameterODByODGroup = prepareParameterODDataByODGroup(mapGroupGenerationODInfo);
	...
	int nFunctionCount = (int)a_functionIdIndexMap.size();
	WriteFunctionCountArgument(of, nFunctionCount);

 	std::vector<KModelChoiceArgInfo> vecChoiceArgInfo;
 
	std::map<int, std::map<int, TFunctionInfo>> mapFunctionInfo; // functionid, modeindex, vector
	MakeFunctionInfo(modeODIndexMap, mapFunctionInfo);
 	...
 	for(itInfo = m_MInfoVector.begin(); itInfo != itInfoEnd; ++itInfo)
 	{
		...
		AutoType itFind  = m_mapDefaultODGroup.find(pPurpose->PurposeID());
		TDefVarDefaultODGroup &oTDefVarGroupResultInfo = itFind->second;

		bResult = CalculateModeTree(mapFunctionInfo, oTDefVarGroupResultInfo, paramODIndexMap, modeODIndexMap, *pParameterOD, of, nPurposeODIndex, 
			a_functionIdIndexMap, mapGroupGenerationODInfo, *pParameterODByODGroup);

 		//bResult = CalculateModeTree(pInfo, paramODIndexMap, modeODIndexMap, *pParameterOD, nPurposeODIndex, of, nPurposeODIndex);
	}
	ReleaseFunctionInfo(mapFunctionInfo);
	...
	while (iter != itEnd)
	{
		int               nVariableID     = iter->first;
		KMSimpleVariable* pSimpleVariable = iter->second;

		CalculateFunctionTree(nVariableID, pSimpleVariable, modeODIndexMap, a_functionIdIndexMap, of);
	...
	KBulkDBase2File::ExportDB2ZoneIDFile(m_pTarget->GetDBaseConnection());

	pParameterOD->RemoveAll();
	pParameterODByODGroup->RemoveAll();
...
void KModeChoiceModelNewDlg::MakeFunctionInfo(std::map<KMode*, int> &a_modeIndexMap, std::map<int, std::map<int, TFunctionInfo>> &a_mapFunctionInfo)
{
	try
	{
		// 정보 저장
		...
		AutoType iterB  = m_mapTempSavedFunctionTree.begin();
		AutoType itEndB = m_mapTempSavedFunctionTree.end();

		while (iterB != itEndB)
		{
			int               nFunctionID    = iterB->first;
			KMSimpleVariable* pSVariable     = iterB->second; 
			KModeTree*        pModeTree      = pSVariable->ModeTree();
			int               nModeNodeCount = pModeTree->ModeNodeCount();

			std::map<int, TFunctionInfo> mapModeIndexFunctionInfo;

			for(int i = 0; i < nModeNodeCount; ++i)
			{
				KModeTreeNodeMode* pModeNode = pModeTree->ModeNode(i);
				KMode* pMode = pModeNode->Mode();
				std::map<KMode*, int>::iterator itModeIndex = a_modeIndexMap.find(pMode);
				int nModeIndex = itModeIndex->second;
				...
				if(pModeNode->HasChild() == false)
				{
					...
					mapModeIndexFunctionInfo.insert(std::make_pair(nModeIndex, oFunction));
				}
				else
				{
					int nSubNodeCount = pModeNode->NodeCount();
					for(int j = 0; j < nSubNodeCount; ++j)
					{
						...
						std::map<KMode*, int>::iterator itSubModeIndex = a_modeIndexMap.find(pSubMode);
						...
						mapModeIndexFunctionInfo.insert(std::make_pair(nSubModeIndex, oFunction));
					}
				}
			}

			a_mapFunctionInfo.insert(std::make_pair(nFunctionID, mapModeIndexFunctionInfo));
...
void KModeChoiceModelNewDlg::ReleaseFunctionInfo(std::map<int, std::map<int, TFunctionInfo>> &a_mapFunctionInfo)
{
	try
	{
		AutoType iter  = a_mapFunctionInfo.begin();
		AutoType itEnd = a_mapFunctionInfo.end();

		while (iter != itEnd)
		{
			std::map<int, TFunctionInfo> &mapModeIndexFunctionInfo = iter->second;

			AutoType iterB  = mapModeIndexFunctionInfo.begin();
			AutoType itEndB = mapModeIndexFunctionInfo.end();

			while (iterB != itEndB)
			{
				TFunctionInfo &oFunction = iterB->second;
...
bool KModeChoiceModelNewDlg::CalculateModeTree( std::map<int, std::map<int, TFunctionInfo>> &a_mapFunctionInfo,
												TDefVarDefaultODGroup &a_oTDefVarGroupResultInfo, std::map<CString, int>& paramIndexMap,
												std::map<KMode*, int>& modeIndexMap, KODKeyDoubleRecords& a_oParameterOD, 
												CStdioFileEx& of, int a_nPurposeIndex, std::map<int, int> &a_functionIdIndexMap, 
												std::map<int, TGroupGenerationODInfo> &a_mapGroupGenerationODInfo, KODKeyDoubleRecords &a_pParameterODByODGroup)
{
	...
	std::set<int> setModeIndex;
	AutoType iterA  = modeIndexMap.begin();
	AutoType itEndA = modeIndexMap.end();

	while (iterA != itEndA)
	{
		int modeIndex = iterA->second;

		setModeIndex.insert(modeIndex);
...
// 		}
		...
		try
		{
			if (false == a_oTDefVarGroupResultInfo.bUseODGroup)
			{
				nFunctionID = a_oTDefVarGroupResultInfo.nNoneGroupFunctionID;
			}
			else
			{
				int nIndex(-1);
				AutoType itFindD = a_mapGroupGenerationODInfo.find(a_oTDefVarGroupResultInfo.nODGroupCode);
				if (itFindD != a_mapGroupGenerationODInfo.end())
				{
					TGroupGenerationODInfo &oTGroupGenerationODInfo = itFindD->second;
					nIndex = oTGroupGenerationODInfo.nIndex;
				}
				else
				{
					throw 1;
				...
				KSDoubleRecord* recordCode = a_pParameterODByODGroup.GetRecord(oKey);
				if (nullptr == recordCode)
				{
					throw 1;
				}
				int nCode = (int)recordCode->GetAt(nIndex);

				std::map<int, int> &mapFunctionID = a_oTDefVarGroupResultInfo.mapFunctionID;

				AutoType itFind = mapFunctionID.find(nCode);
				if (itFind != mapFunctionID.end())
					nFunctionID = itFind->second;
				else
					throw 1;
			}

			AutoType itFindB = a_mapFunctionInfo.find(nFunctionID);
			if (itFindB == a_mapFunctionInfo.end())
				throw 1;

			AutoType itFindC = a_functionIdIndexMap.find(nFunctionID);
			if (itFindC != a_functionIdIndexMap.end())
				nFunctionIndex = itFindC->second;
			else
				throw 1;
		}
		catch (...)
		...
		AutoType itFind = a_mapFunctionInfo.find(nFunctionID);
		std::map<int, TFunctionInfo> &mapModeIndexFunctionInfo = itFind->second;

		ofs.write(reinterpret_cast<char*>( &nxOZoneID ), sizeof(Integer) );
		ofs.write(reinterpret_cast<char*>( &nxDZoneID ), sizeof(Integer) ); 
		ofs.write(reinterpret_cast<char*>( &nFunctionIndex ), sizeof(int) );

		AutoType itModeIdx    = setModeIndex.begin();
		AutoType itEndModeIdx = setModeIndex.end();

		while (itModeIdx != itEndModeIdx)
		{
			int nAllModeIdx = *itModeIdx;

			AutoType itFind = mapModeIndexFunctionInfo.find(nAllModeIdx);
			if (itFind != mapModeIndexFunctionInfo.end())
			{
				TFunctionInfo oFunction				= itFind->second;
...
// bool KModeChoiceModelNewDlg::CalculateModeTree( KModeChoiceInfo* pInfo, std::map<CString, int>& paramIndexMap,
// 	std::map<KMode*, int>& modeIndexMap, KODKeyDoubleRecords& a_oParameterOD, 
...
bool KModeChoiceModelNewDlg::CalculateFunctionTree(int a_nVariableID, KMSimpleVariable* a_pSimpleVariable, 
											std::map<KMode*, int> &a_modeIndexMap, std::map<int, int> &a_functionIdIndexMap,
											CStdioFileEx& of)
{
	bool bResult(true);
	...
	try
	{
		KIOTables*         pIOTables    = m_pTarget->Tables();
		KIOTable*          pTable       = pIOTables->FindTable(TABLE_PARAMETER_OD);
		KMSimpleVariable*  pSVariable   = a_pSimpleVariable;
		...
		//Argument 입력
		AutoType itFind = a_functionIdIndexMap.find(a_nVariableID);
		AutoType itEnd  = a_functionIdIndexMap.end();
		if (itFind != itEnd)
		{
			bResult = WriteFunctionIndexArgument(of, itFind->second);
...
void KModeChoiceModelNewDlg::GetParaColumnName(std::vector<TUseColumnInfo>& vecUseColumnInfo)
{
	TxLogDebugStartMsg();
	std::vector<KModeChoiceInfo*>::iterator itInfo, itInfoEnd = m_MInfoVector.end();
	for(itInfo = m_MInfoVector.begin(); itInfo != itInfoEnd; ++itInfo)
...
bool KModeChoiceModelNewDlg::WriteZoneAgument( CStdioFileEx& of )
{
	KDBaseConPtr spDBaseConnection = m_pTarget->GetDBaseConnection();
...
bool KModeChoiceModelNewDlg::WritePurposeCountAgument( CStdioFileEx& of )
{
	try
	{
...
bool KModeChoiceModelNewDlg::WriteModeCountAgument( CStdioFileEx& of, int a_nModeCount )
{
	try
	{
		CString strOut = _T("");
...
bool KModeChoiceModelNewDlg::WriteFunctionCountArgument( CStdioFileEx& of, int a_nFunctionCount )
{
	try
	{
		CString strOut = _T("");
		strOut.Format(_T("noFunction\t%d\r\n"), a_nFunctionCount);
		of.WriteString(strOut);
	}
	catch (...)
	{
...
bool KModeChoiceModelNewDlg::WritePurposeInfoAgument( CStdioFileEx& of, int a_nPurposeIndex )
{
	try
	{
		CString strOut = _T("");
...
bool KModeChoiceModelNewDlg::WriteElisticityPurposeInfoAgument( CStdioFileEx& of, int a_nPurpposeIndex )
{
	try
	{
		CString strOut = _T("");
...
bool KModeChoiceModelNewDlg::WriteNotNestedInfoAgument( CStdioFileEx& of, std::vector<int> a_vecNotNestedArgInfo )
{
	try
	{
...
bool KModeChoiceModelNewDlg::WriteNestdInfoAgument( CStdioFileEx& of, std::vector<TNestedArgInfo>& a_vecNestedArgInfo )
{
	try
	{
...
bool KModeChoiceModelNewDlg::WriteFunctionIndexArgument( CStdioFileEx& of, int a_nFunctionIndex )
{
	try
	{
		CString strOut = _T("");
		strOut.Format(_T("FunctionIndex\t%d\r\n"), a_nFunctionIndex);
		of.WriteString(strOut);
	}
	catch (...)
	{
...
void KModeChoiceModelNewDlg::WritePurposeODBinary()
{
...
unsigned __stdcall KModeChoiceModelNewDlg::ModeChoiceThreadCaller( void* pParam )
{
	...
	KModeChoiceModelNewDlg* pDlg = (KModeChoiceModelNewDlg*)pParameter->GetParameter();;
...
void KModeChoiceModelNewDlg::AddStatusMessage( CString a_strMsg )
{
...
void KModeChoiceModelNewDlg::ReceiveStatusMessageFromDll( void* pParam1, int nStatus, char* strMsg )
{
	AddStatusMessage(CString(strMsg));
}


void KModeChoiceModelNewDlg::OnBnClickedRunelasticity()
{
	TxLogDebugStartMsg();
...
unsigned __stdcall KModeChoiceModelNewDlg::ElasticityThreadCaller( void* pParam )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)pParam;
...
void KModeChoiceModelNewDlg::CalculateElasticityModel()
{
	TxLogDebugStartMsg();
	bool  bResult = false;
	CTime timeStart, timeEnd;
...
bool KModeChoiceModelNewDlg::CalculateNonGroupElasticity( KODKeyDoubleRecords& purposeODRecords, std::map<CString, int>& purposeODIndexMap, 
...
bool KModeChoiceModelNewDlg::CalculateElasticityTree( KModeChoiceInfo* pInfo, std::map<CString, int>& paramIndexMap, std::map<KMode*, int>& modeIndexMap, KODKeyDoubleRecords& a_oParameterOD, int a_nPurposeODIndex, CStdioFileEx& of, int a_nPurposeIndex )
{
	bool bResult = true;
...
CString KModeChoiceModelNewDlg::ChangeRealFunction( CString a_strFunction )
{
...
bool KModeChoiceModelNewDlg::WriteZoneInfoAgument(CStdioFileEx& of)
{
...
bool KModeChoiceModelNewDlg::ReadOutputFile(KODKeyDoubleRecords& a_oModeODResult, std::map<KMode*, int>& modeODIndexMap)
{
...
void KModeChoiceModelNewDlg::OnBnClickedLogview()
{
...
BOOL KModeChoiceModelNewDlg::OnCopyData( CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct )
{
	try
	{
