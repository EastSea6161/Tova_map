// DlgImportTerminalSchedule.cpp : 구현 파일입니다.
//
...
// KDlgImportTerminalSchedule 대화 상자입니다.

IMPLEMENT_DYNAMIC(KDlgImportTerminalSchedule, KWhiteDlgEx)

KDlgImportTerminalSchedule::KDlgImportTerminalSchedule(KTarget* a_pTarget, KIOTable* a_pTable, CWnd* pParent /*=NULL*/)
	: KWhiteDlgEx(KDlgImportTerminalSchedule::IDD, pParent)
	, m_pTarget(a_pTarget)
	, m_pTable(a_pTable)
	, m_strTableName(_T(""))
{
...
KDlgImportTerminalSchedule::~KDlgImportTerminalSchedule()
{
}

void KDlgImportTerminalSchedule::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(KDlgImportTerminalSchedule, KWhiteDlgEx)
	ON_BN_CLICKED(IDOK,       &KDlgImportTerminalSchedule::OnBnClickedOk)
	ON_NOTIFY(XTP_NM_REPORT_RECORDS_DROPPED,      IDC_REPORT1,     &KDlgImportTerminalSchedule::OnReportRecordsDropped)
	ON_NOTIFY(XTP_NM_REPORT_DROP,                 IDC_REPORT7,     &KDlgImportTerminalSchedule::OnReportDropSampleColumn)
	ON_BN_CLICKED(IDC_CHECK4, &KDlgImportTerminalSchedule::OnBnClickedCheckDefault)
	ON_BN_CLICKED(IDCANCEL, &KDlgImportTerminalSchedule::OnBnClickedCancel)
	ON_CONTROL_RANGE(BN_CLICKED,IDC_RADIO4, IDC_RADIO6, &KDlgImportTerminalSchedule::OnBnClickedRadioImportType)
	ON_EN_CHANGE(IDC_FILENAME2, &KDlgImportTerminalSchedule::OnEnChangeFilename)
	ON_CBN_SELCHANGE(IDC_COMBO4, &KDlgImportTerminalSchedule::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &KDlgImportTerminalSchedule::OnCbnSelchangeCombo5)
	ON_BN_CLICKED(IDC_CHECK1, &KDlgImportTerminalSchedule::OnBnClickedCheck1)
END_MESSAGE_MAP()


// KDlgImportTerminalSchedule 메시지 처리기입니다.
void KDlgImportTerminalSchedule::ResizeComponent()
{
	try 
	{
...
void KDlgImportTerminalSchedule::InitBaseDataTypeUI()
{
	UpdateData(TRUE);
...
KEMImportType KDlgImportTerminalSchedule::GetSelectedImportType()
{
...
BOOL KDlgImportTerminalSchedule::OnInitDialog()
{
...
void KDlgImportTerminalSchedule::OnEnChangeFilename()
{
	UpdateData( TRUE );
...
void KDlgImportTerminalSchedule::CheckDefaultSeperator(CString a_strFile)
{
	try
	{
...
bool KDlgImportTerminalSchedule::CheckImportHeader(CString a_strFile)
{
	try
	{
		QBicTxtReader oReader; {
...
int KDlgImportTerminalSchedule::CheckEncoding(CString a_strFile, bool a_bAlarmErr)
{
	try
	{
...
void KDlgImportTerminalSchedule::InitComboSeparator()
{
	try
	{
...
void KDlgImportTerminalSchedule::InitComboEncoding()
{
	try
	{
...
void KDlgImportTerminalSchedule::InitializeColumnField( void )
{
	TxLogDebugStartMsg();
...
void KDlgImportTerminalSchedule::UpdateReportDataSampleColumn(CString a_strColName, int a_nIndex)
{
	try 
	{
...
void KDlgImportTerminalSchedule::InitializeSampleColumnReport()
{
	try 
	{
...
void KDlgImportTerminalSchedule::UpdateColumnRecord( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	...
	try
	{
		...
		for (int i= 0; i< nColumnCount; i++) {
            ...
            bool bRed(false);
            CString strField(_T("")); {
                if (strColNm.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_SEQ) == 0  ||
                    strColNm.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_O_TERMINAL) == 0  ||
                    strColNm.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_D_TERMINAL) == 0  ||
                    strColNm.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_MODE) == 0   ) 
                {
...
int KDlgImportTerminalSchedule::MatchingColumnIndex(CString strColumnCaption)
{
	int nIndex = 0;
...
void KDlgImportTerminalSchedule::InitializePreview( void )
{
	TxLogDebugStartMsg();
...
int KDlgImportTerminalSchedule::GetSelectedEncoding()
{
	int nEncoding; {
...
TCHAR KDlgImportTerminalSchedule::GetSelectedSeparator()
{
	TCHAR tcSeparator; {
		int nCurSel = m_cboSeparator.GetCurSel();
		int nCode   = (int)m_cboSeparator.GetItemData(nCurSel);
...
bool KDlgImportTerminalSchedule::PriviewLoadCSV( void )
{
	TxLogDebugStartMsg();
	m_Rows.clear();
	m_ColumnRow.clear();
...
void KDlgImportTerminalSchedule::ParseCSVLineString( CString& strLine, TCHAR tcSeparator, CSVRow& row )
{
	bool bQuote(false);
	int  nSub(0);
...
void KDlgImportTerminalSchedule::OnBnClickedCancel()
{
	KWhiteDlgEx::OnCancel();
}

void KDlgImportTerminalSchedule::OnReportRecordsDropped(NMHDR *pNotifyStruct, LRESULT *pResult)
{
...
void KDlgImportTerminalSchedule::OnReportDropSampleColumn(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	try
	{
		XTP_NM_REPORTDRAGDROP *pItemNotify = reinterpret_cast<XTP_NM_REPORTDRAGDROP *>(pNotifyStruct);
...
void KDlgImportTerminalSchedule::OnBnClickedCheckDefault()
{
	ControlDefaultCheck();
}

void KDlgImportTerminalSchedule::ControlDefaultCheck()
{
	try 
	{
...
void KDlgImportTerminalSchedule::AllNullCheck()
{
	try 
	{
...
void KDlgImportTerminalSchedule::OnBnClickedOk()
{
	UpdateData( TRUE );
	...
	KDefineRegionMode oRegionModeDlg(m_pTarget, true);
	oRegionModeDlg.DoModal();

	OnOK();
}

unsigned __stdcall KDlgImportTerminalSchedule::ThreadRun( void* p )
{
	QBicSimpleProgressParameter* pParameter = (QBicSimpleProgressParameter*)p;
	if (pParameter != nullptr) {
		ThreadPara* pPara = (ThreadPara*)pParameter->GetParameter(); {
			KDlgImportTerminalSchedule* pDlg = (KDlgImportTerminalSchedule*)pPara->TWindow;
...
void KDlgImportTerminalSchedule::Execute(ThreadPara* pPara)
{
	try 
	{
 		KDBaseImportTable::TerminalScheduleRecord(m_pTable, m_setTerminalScheduleID);
...
void KDlgImportTerminalSchedule::ExecuteImport(ThreadPara* pPara)
{
	try 
	{
		KEMImportType emImportType = GetSelectedImportType();
...
void KDlgImportTerminalSchedule::GetStatisticsMsg(KEMImportType a_emImportType, ThreadPara* pPara, CString &a_strMsg)
{
	a_strMsg = _T("");
	try 
	{
		if (KmzSystem::GetLanguage() == KEMKorea) {
			if (KEMImportInsert == a_emImportType) {
				if (KImportCommon::IsEmptyTable(m_pTarget, m_pTable->Name()) == false) { // true: 데이터무, false : 데이터 존재
...
void KDlgImportTerminalSchedule::ReadCSVFile(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportTerminalSchedule::CSV2BinaryNew(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
...
bool KDlgImportTerminalSchedule::CSV2Binary_Update(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine,  bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs, QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportTerminalSchedule::CSV2BinaryAdd(std::vector<TColMatch> &a_vecColMatch, int a_nCurrentLine, bool& a_bEverErrLine, CSVRow a_row,  std::ofstream& ofs,  QBicTxtWriter& ofErr )
{
	try
	{
		std::map<CString, Integer> mapFieldValue;
...
bool KDlgImportTerminalSchedule::ConfirmPrimaryKey(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	for (int i= 0; i< nSizeVec; i++) {
		...
		bool bPK(false);
		// 조회 key value 추출
		if (strIOColumnName.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_SEQ) == 0) {
			bPK = true;
		}
...
bool KDlgImportTerminalSchedule::CheckEtcFilter(std::vector<TColMatch> &a_vecColMatch, CSVRow &a_oImportRow, CString &a_strErrMsg)
{
	CString strIOColumnName(_T(""));
	CString strImportValue(_T(""));
	Integer nxValue(0);
	...
	try 
	{
		...
		for (int i= 0; i< nSizeVec; i++) {
			...
			if ( KImportCommon::IsNodeRelationColumn(strIOColumnName) )	{
				if( emDataType != KEMIODataTypeInteger )
					continue;
...
bool KDlgImportTerminalSchedule::IsFindTableData(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	...
	return IsScheduleID(nxID);
}

bool KDlgImportTerminalSchedule::IsNodeID(Integer a_nxValue)
{
...
bool KDlgImportTerminalSchedule::IsScheduleID(Integer a_nxValue)
{
	auto iter = m_setTerminalScheduleID.find( a_nxValue );
	auto end  = m_setTerminalScheduleID.end();
	if(iter == end) {
		return false;
	}
...
bool KDlgImportTerminalSchedule::IsWrongIDCount(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	// 추출 ID 갯수 확인
...
bool KDlgImportTerminalSchedule::CheckDuplicateID(std::map<CString, Integer> &a_mapKeyFieldValue, KEMImportType a_emImportType)
{
	try 
	{
		auto iter  = a_mapKeyFieldValue.begin();
		...
		int nKeyCnt(0);
		nKeyCnt = 1; // TABLE_TERMINAL_SCHEDULE 은 PK 개수가 셋
...
#include <string>
void KDlgImportTerminalSchedule::WriteOutputStream(CString a_strImportValue, KEMIODataType a_emDataType, std::ofstream& ofs)
{
	try
	{
		double	dValue  = 0.0;
...
void KDlgImportTerminalSchedule::ImportBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
...
void KDlgImportTerminalSchedule::UpdateUserData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
void KDlgImportTerminalSchedule::AddBaseData(ThreadPara* pPara)
{
	TxLogDebugStartMsg();
	std::ifstream ifs(m_strMiddleFile, std::ios::binary);
...
CString KDlgImportTerminalSchedule::GeneratePrepareQuery( void )
{
	CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
	int nRecordCount = pRecords->GetCount();
...
CString KDlgImportTerminalSchedule::GenerateUpdatePrepareQuery(std::vector<KIOColumn*> &a_vecIOColumn)
{
...
void KDlgImportTerminalSchedule::ErrorFileOpen()
{
	try
	{
...
void KDlgImportTerminalSchedule::ValidateImport( void )
{
	TxLogDebugStartMsg();
	try
	{
		CString strColumnName(_T(""));
#pragma region Terminal_Schedule 테이블 필수입력_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		...
		for (int i= 0; i < nColumnCount; i++) {
			...
			if( strColumnName.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_SEQ) == 0        || 
				strColumnName.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_O_TERMINAL) == 0 || 
				strColumnName.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_D_TERMINAL) == 0 ||
				strColumnName.CompareNoCase(COLUMN_TERMINAL_SCHEDULE_MODE) == 0  ) {
...
#pragma endregion Terminal_Schedule 테이블 필수입력_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportTerminalSchedule::ValidateUpdateMode( void )
{
	TxLogDebugStartMsg();
	UpdateData(TRUE);
	try
	{
		...
		CString strColumnName(_T(""));
#pragma region Terminal_Schedule 테이블 Id_Filed Null 체크
		CXTPReportRecords* pRecords = m_ctrlReportColumn.GetRecords();
		int nColumnCount			= pRecords->GetCount();
		int nUpdateColums(0);
...
#pragma endregion Terminal_Schedule 테이블 Id_Filed Null 체크
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
bool KDlgImportTerminalSchedule::AlertTableChange()
{
	UpdateData(TRUE);
...
KIOColumn* KDlgImportTerminalSchedule::GetDefIOColumn( CXTPReportRecord* a_pRecord )
{
...
int KDlgImportTerminalSchedule::GetImportColumnIndex( CXTPReportRecord* a_pRecord )
{
	int nColumnIndex = 0;
...
void KDlgImportTerminalSchedule::OnBnClickedRadioImportType(UINT nID)
{
	InitBaseDataTypeUI();
	UpdateColumnRecord();
}

void KDlgImportTerminalSchedule::RelationTableNotify()
{
	TxLogDebugStartMsg();
	try
	{
		...
		// regional_mode:TRANSIT_REGIONAL_MODE_CODE_GROUP_KEY 코드가 레코드 내에 존재한다면, 코드메니저에 등록하자
		// 주의 사항은 NotifyNodeRelation() 이후에 해야 한다.
		//AddTransitRegionModeCode();
	} catch (int& ex) {
		CString strError(_T(""));
		strError.Format(_T("Error : %d"), ex);
		TxLogDebug((LPCTSTR)strError);
...
void KDlgImportTerminalSchedule::AddStatusMessage( CString a_strMsg )
{
...
void KDlgImportTerminalSchedule::OnCbnSelchangeCombo4()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportTerminalSchedule::OnCbnSelchangeCombo5()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportTerminalSchedule::OnBnClickedCheck1()
{
	if( PriviewLoadCSV() ) {
		InitializeColumnField();
		UpdateColumnRecord();
...
void KDlgImportTerminalSchedule::AddTransitRegionModeCode()
{
	try
	{
		// #1. 터미널 스케쥴 테이블에서 mode 코드를 확인한다.
		...
		pTTypeCodeGroup->GetCodes(mapRegionMode);

		auto iter  = setDBInCode.begin();
		auto itEnd = setDBInCode.end();
		for (; iter != itEnd; ++iter) {
			const int &nModeCode = *iter;

			auto itFind = mapRegionMode.find(nModeCode);
			if (itFind != mapRegionMode.end()) {
				continue; // 기존에 등록 되어있음.
			}

			CString strCodeName(_T(""));
			strCodeName.Format(_T("%d:(x)"), nModeCode);
			// 기존에 등록 되어있지 않으면, 등록
			pTTypeCodeGroup->AddCode(nModeCode, strCodeName);
		}
	}
	catch (KExceptionPtr ex)
	{
...
void KDlgImportTerminalSchedule::GetRegionModeValueInTerminalDB( std::set<int>& a_setCode )
{
	a_setCode.clear();
